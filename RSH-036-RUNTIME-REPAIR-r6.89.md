# RSH-036 r6.89 — leftover g07 after world.group+outside is remaining empty-scene vs golden, not useLegacyLights, not debug.checkShaderErrors, not xr.enabled, not env

Candidate, 12 September 2026. Base f213792
(r6.88 tnc isolation). No merge or freeze; 35/67 accepted;
32 remain. r6.17, r6.18 and r6.27 exact-head failures are retained. Do not
relabel them as passes. r6.88 exact-head CI is not claimed. GFX-01, GFX-03,
...
## AUD-98: leftover g07 empty-scene vs golden is not useLegacyLights vs debug.checkShaderErrors vs xr.enabled

r6.88 leftover g07 after world.group+outside hidden is remaining empty-scene
vs golden residual (leftover 0.2027), not shadowMap.type, not shadowMap.needsUpdate,
not clippingPlanes and not env. The new probe isolates leftover after
world.group+outside as remaining empty-scene vs golden independently of
useLegacyLights vs debug.checkShaderErrors vs xr.enabled after
...
or shadowMap.type/needsUpdate/clippingPlanes.
Product renderer stays on the leftover path (useLegacyLights unset /
debug.checkShaderErrors true / xr.enabled false). Isolation uses
`renderer.useLegacyLights = true`, `debug.checkShaderErrors = false`, and
`xr.enabled = true`. Deltas use the empty-scene leftover buffer as baseline.

Live leftover after world.group+outside hidden, rest chase 7.4/1.92,
1280×800 p=1 (product hex `0xd0d4d8`, exposure 0.56):

| pose | leftover % | lights Δ | debug Δ | xr Δ | env Δ | both Δ | pack Δ |
|---|---:|---:|---:|---:|---:|---:|---:|
...
geometry for a useLegacyLights/checkShaderErrors/xr.enabled change to occupy
remaining g07. Product renderer stays. Counts: leftover 2082.

Local unique cases: 28 in `scripts/world-ldx.test.mjs`. Previous r6.88
expected 2,989 + 28 = 3,017. Pixel 0/4, immutable CDN bytes, rendering
