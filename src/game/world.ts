...
export async function createWorld(def: TrackDef, built: BuiltTrack, shadows: boolean, night: boolean, weather: Weather = "clear") {
  ...
  const desertNight = def.theme === "desert" || def.theme === "snow" || def.theme === "stone" || def.theme === "jaffa" || def.theme === "carmel" || def.id === "ramon" || def.id === "hermon" || def.id === "scopus" || def.id === "golan" || def.id === "hw1";
