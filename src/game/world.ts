...
export async function createWorld(def: TrackDef, built: BuiltTrack, shadows: boolean, night: boolean, weather: Weather = "clear") {
  ...
  const crowdN = natureTrack || def.id === "ayalon" || def.id === "rothschild" || def.id === "hayarkon" || def.id === "oldjaffa" || def.id === "jerusalem" || def.id === "rishon" || def.id === "ramla" || def.id === "acre" || def.id === "caesarea" ? 0 : shadows ? 72 : 28;
  if (crowdN) {
