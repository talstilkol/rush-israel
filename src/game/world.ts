...
export async function createWorld(def: TrackDef, built: BuiltTrack, shadows: boolean, night: boolean, weather: Weather = "clear") {
  ...
  const postSpots = [];
  if (def.theme !== "desert" && def.theme !== "snow" && def.theme !== "carmel" && def.theme !== "bauhaus" && def.theme !== "jaffa" && def.theme !== "port" && def.id !== "ayalon") for (let i = 0; i < built.samples.length; i += 5) {
    const s = built.samples[i];
  ...
  const ficusStreet = (def.theme === "bauhaus" || def.id === "hayarkon") && def.id !== "ayalon" && def.id !== "rothschild" && def.id !== "holon" && def.id !== "ksaba" && def.id !== "rehovot" && def.id !== "petah" && def.id !== "telaviv" && def.id !== "namal";
