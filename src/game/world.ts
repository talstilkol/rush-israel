...
export async function createWorld(def: TrackDef, built: BuiltTrack, shadows: boolean, night: boolean, weather: Weather = "clear") {
  ...
  if (def.theme !== "desert" && def.theme !== "snow" && def.theme !== "carmel" && def.theme !== "stone") {
    paintAt(0.012, false);
    paintAt(0.022, true);
  }
  if (urban) {
  ...
  if (def.id === "ayalon" || (def.theme !== "port" && def.theme !== "jaffa" && def.theme !== "stone" && def.theme !== "bauhaus" && def.theme !== "highway")) {
