...
export async function createWorld(def: TrackDef, built: BuiltTrack, shadows: boolean, night: boolean, weather: Weather = "clear") {
  ...
  if (def.theme !== "desert" && def.theme !== "snow" && def.id !== "rothschild" && def.id !== "telaviv" && def.id !== "petah" && def.id !== "raanana" && def.id !== "ksaba" && def.theme !== "stone" && def.theme !== "jaffa" && def.theme !== "carmel") {
