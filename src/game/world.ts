...
export async function createWorld(def: TrackDef, built: BuiltTrack, shadows: boolean, night: boolean, weather: Weather = "clear") {
  ...
  group.add(new THREE.Mesh(keep(buildEdgeLine(built, -1, 0.16, 0.46)), edgeMat));
  {
    ...
    const nDash = Math.min(700, Math.floor(built.samples.length / 2) * (lanes - 1) * offs.length);
  ...
  group.add(new THREE.Mesh(keep(buildShoulder(built, -1)), shoulderMat));
  {
    const skipSigns = def.theme === "desert" || def.theme === "snow" || def.theme === "stone" || def.theme === "carmel" || def.theme === "bauhaus" || (def.theme === "highway" && def.id !== "ayalon") || def.id === "ramon" || def.id === "hermon" || def.id === "masada" || def.id === "deadsea";
    if (!skipSigns) {
  ...
  const tanks = new THREE.InstancedMesh(tankGeo, tankMat, def.theme === "jaffa" || def.theme === "carmel" || def.theme === "stone" ? 0 : Math.min(placements.length, nyc ? 40 : 40));
  ...
  const lampCount = def.id === "ramon" || def.id === "hermon" ? Math.floor(built.samples.length / 12) : def.id === "ayalon" ? Math.floor(built.samples.length / 8) : def.id === "hw1" || def.id === "hw2" || def.id === "hw6" ? Math.floor(built.samples.length / 16) : def.theme === "carmel" ? Math.floor(built.samples.length / 18) : Math.floor(built.samples.length / 28);
  ...
  const natureTrack = def.id === "ramon" || def.id === "hermon" || def.theme === "carmel" || def.theme === "desert" || def.theme === "snow" || def.id === "hw1" || def.id === "hw2" || def.id === "hw6" || def.id === "golan";
  const crowdN = def.id === "manhattan" || def.id === "timessquare" || def.id === "centralpark" || def.id === "brooklynbridge" ? shadows ? 72 : 28 : 0;
  if (crowdN) {
