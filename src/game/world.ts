...
function addLandmarks(group, def, bag, shadows, isNight, glows, emitList, colliders, movers, ramps, streets, built) {
  ...
  if (def.id === "acre") {
    const hb = acr(32.9195, 35.0708);
    const wall = new THREE.Mesh(new THREE.BoxGeometry(92, 11, 5.2), stone);
    wall.position.set(hb.x, 5.6, hb.z);
    add(wall);
    const wall2 = new THREE.Mesh(new THREE.BoxGeometry(5.2, 11, 54), stone);
    wall2.position.set(hb.x - 46, 5.6, hb.z + 24);
    add(wall2);
    for (let i = 0; i < 12; i++) {
      const merlon = new THREE.Mesh(new THREE.BoxGeometry(2.6, 2.1, 5.6), stone);
      merlon.position.set(hb.x - 42 + i * 8, 12.2, hb.z);
      add(merlon);
    }
    const burj = new THREE.Mesh(new THREE.CylinderGeometry(5.4, 6.2, 16, 12), stone);
    burj.position.set(hb.x - 46, 9, hb.z);
    add(burj);
    const burjCap = new THREE.Mesh(new THREE.CylinderGeometry(6.6, 5.2, 1.8, 12), stone);
    burjCap.position.set(hb.x - 46, 17.6, hb.z);
    add(burjCap);
    const ochreH = new THREE.MeshStandardMaterial({ color: 12093784, roughness: 0.88, envMapIntensity: 0.28 });
    bag.push(ochreH);
    for (let i = 0; i < 8; i++) {
      const hx = hb.x - 28 + i % 4 * 12;
      const hz = hb.z + 16 + Math.floor(i / 4) * 14;
      const house = new THREE.Mesh(new THREE.BoxGeometry(7.4, 5.6 + i % 3, 8.2), i % 2 ? ochreH : stone);
      house.position.set(hx, 3.2 + i % 3 * 0.4, hz);
      add(house);
      const rf = new THREE.Mesh(new THREE.BoxGeometry(8, 0.4, 8.8), terracotta);
      rf.position.set(hx, 6.4 + i % 3, hz);
      add(rf);
    }
    const kh = acr(32.9208, 35.0695);
    const khan = new THREE.Mesh(new THREE.BoxGeometry(24, 7.2, 24), stone);
    khan.position.set(kh.x, 3.6, kh.z);
    add(khan);
    const court = new THREE.Mesh(new THREE.BoxGeometry(12, 0.2, 12), cream);
    court.position.set(kh.x, 0.2, kh.z);
    add(court);
    for (const [dx, dz] of [[-8, -8], [8, -8], [-8, 8], [8, 8]]) {
      const col = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 0.85, 11, 8), stone);
      col.position.set(kh.x + dx, 7.5, kh.z + dz);
      add(col);
    }
    const clock = new THREE.Mesh(new THREE.CylinderGeometry(2.05, 2.5, 28, 10), stone);
    clock.position.set(kh.x, 16, kh.z);
    add(clock);
    const clockBox = new THREE.Mesh(new THREE.BoxGeometry(4.2, 4.2, 4.2), cream);
    clockBox.position.set(kh.x, 30, kh.z);
    add(clockBox);
    const clockCap = new THREE.Mesh(new THREE.ConeGeometry(2.8, 3.4, 4), terracotta);
    clockCap.rotation.y = Math.PI / 4;
    clockCap.position.set(kh.x, 33.8, kh.z);
    add(clockCap);
    const ms = acr(32.9228, 35.0708);
    ...
    add(citT);
    for (let i = 0; i < 8; i++) {
      const hx = hb.x - 22 - i % 2 * 7;
      const hz = hb.z - 18 + i * 5.4;
      const hull = new THREE.Mesh(new THREE.BoxGeometry(2.1, 0.7, 7.4), i % 2 ? white : cream);
      hull.position.set(hx, 0.4, hz);
      add(hull);
      const mast = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.08, 8, 5), wood);
      mast.position.set(hx, 4.6, hz);
      add(mast);
    }
  ...
  if (def.id === "nazareth") {
    ...
    bag.push(darkStone);
    const basilica = new THREE.Mesh(new THREE.BoxGeometry(28, 16, 20), darkStone);
    basilica.position.set(ba.x, 9, ba.z);
    add(basilica);
    const nave = new THREE.Mesh(new THREE.BoxGeometry(18, 10, 14), cream);
    nave.position.set(ba.x, 20, ba.z);
    add(nave);
    const lantern = new THREE.Mesh(new THREE.CylinderGeometry(6.2, 7.2, 12, 8), cream);
    lantern.position.set(ba.x, 30, ba.z);
    add(lantern);
    for (let i = 0; i < 8; i++) {
      const a = i / 8 * Math.PI * 2 + Math.PI / 8;
      const col = new THREE.Mesh(new THREE.BoxGeometry(1.2, 11, 0.8), cream);
      col.position.set(ba.x + Math.cos(a) * 6.6, 30, ba.z + Math.sin(a) * 6.6);
      add(col);
      const win = new THREE.Mesh(new THREE.BoxGeometry(1.6, 3.4, 0.3), darkGlass);
      win.position.set(ba.x + Math.cos(a) * 6.15, 30, ba.z + Math.sin(a) * 6.15);
      win.lookAt(ba.x, 30, ba.z);
      add(win);
    }
    const bDome = new THREE.Mesh(new THREE.ConeGeometry(7.6, 10, 8), darkStone);
    bDome.position.set(ba.x, 41, ba.z);
    add(bDome);
    const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.32, 4.2, 0.32), cream);
    crossV.position.set(ba.x, 47, ba.z);
    add(crossV);
    const crossH = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.32, 0.32), cream);
    crossH.position.set(ba.x, 46.2, ba.z);
    add(crossH);
    const mw = naz(32.706, 35.298);
    const well = new THREE.Mesh(new THREE.CylinderGeometry(2.6, 3, 2.2, 12), stone);
    well.position.set(mw.x, 1.3, mw.z);
    add(well);
    const wellRoof = new THREE.Mesh(new THREE.ConeGeometry(3.6, 2.8, 4), terracotta);
    wellRoof.position.set(mw.x, 3.8, mw.z);
    add(wellRoof);
    glowAt(ba.x, 42, ba.z, 16771264, 40, 32);
    hit(ba.x, ba.z, 16);
    hit(mw.x, mw.z, 6);
  }
  if (def.id === "tzfat") {
