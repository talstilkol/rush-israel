...
export function createWorld(def, built, shadows, night, weather = "clear") {
  ...
  const bag = [];
  const keep = (d) => {
    if (d === getFoliage() || d === getBark() || d === getSkyDay() || d === getSkyNight() || d === getJaffaClock() || d === getIsraelFlag() || d === getHerodian() || d === getCurb("city") || d === getCurb("stone") || d === getCurb("dirt") || d === getCurb("sand") || d === getCurtain("blue") || d === getCurtain("teal") || d === getCurtain("dark") || d === getCurtain("gold") || d === getCurtain("white") || d === getSidewalk()) return d;
    bag.push(d);
    return d;
  };
