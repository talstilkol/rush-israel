...
export function TouchControls({ engine, langHe }: Props) {
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex items-end justify-between gap-4 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] md:hidden">
      <div
        className="pointer-events-auto h-28 w-40 touch-none rounded-xl border border-border bg-surface/80"
        onPointerDown={(e) => {
        ...
        onPointerUp={() => engine?.setTouch({ steer: 0 })}
        onPointerCancel={() => engine?.setTouch({ steer: 0 })}
        onPointerLeave={() => engine?.setTouch({ steer: 0 })}
      >
        <div className="flex h-full items-center justify-center text-xs font-medium text-muted">{t("הגה", "Steer")}</div>
      </div>
