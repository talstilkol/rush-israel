...
1. Remaining 0/4 is sun intensity luma plus bakeEnv 0x3a9ae0 hue plus leftover
   ...
   independently of useLegacyLights vs debug.checkShaderErrors vs
   xr.enabled
   without changing product color, exposure or
   refreshing PNGs.
   Smoke must keep verifyWorldLayers through verifyWorldLdx. Live rest chase 7.4/1.92 must stay. Remaining pixel 0/4
   ...
   g07 empty-scene vs golden after useLegacyLights/debug.checkShaderErrors/xr.enabled neutralization. PNG refresh, threshold drift, color
...
2. Preserve the r6.18 Chromium install retry through the r6.84 lcc isolation,
   the r6.85 acd isolation, the r6.87 dsa isolation, the r6.88 tnc isolation and the r6.89 ldx isolation.
...
AUD-99 is the next RSH-036 isolation: leftover g07 empty-scene vs golden
...
Master plan r6.89 retains 67 units, 42 historical findings, 6 bundles and 98
