...
9. The standing owner instruction to finish the master plan authorised remaining deferred units serially. RSH-036 is consumed on validated merge; RSH-037 is not authorised until that merge.

## 5. Post-RSH-036 program state
...
| Active PR | none |
| RSH-036 one-unit authority | 1/1 — consumed |
...
Ghosts remain byte-preserved and remain assigned outside this unit. The RSH-036 one-unit authority is consumed on merge. RSH-037 remains deferred and cannot start without a new explicit owner instruction after this merge.
