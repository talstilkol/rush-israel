...
9. The standing owner instruction to finish the master plan authorised remaining deferred units serially. RSH-042 is consumed on validated merge; RSH-043 is not authorised until that merge.

## 5. Post-RSH-042 program state
...
| Active PR | none |
| RSH-042 one-unit authority | 1/1 — consumed |
...
| G6 | RSH-037–043 | 7 | Performance and reliability | IN PROGRESS |
...
Ghosts remain byte-preserved and remain assigned outside this unit. The 30-minute soak contract is locked at `src/game/thirty-soak/` with duration 1800 s; required-CI soak-smoke stays 2 cycles and does not substitute. P1-12 stays OPEN. The RSH-042 one-unit authority is consumed on merge. RSH-043 remains deferred and cannot start without a new explicit owner instruction after this merge.
