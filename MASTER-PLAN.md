...
9. The plain `next` / `המשך` authority covers exactly RSH-028 and is consumed on its validated merge; RSH-029 is not authorised.

## 5. Post-RSH-028 program state
...
| Active PR | none |
| RSH-028 one-unit authority | 1/1 — consumed |
...
RSH-021 through RSH-028 are accepted on validated merge. Save schema version 3 and the deterministic 0→1→2→3 migration graph remain unchanged. Production builds stay exactly `vite build`. Ayalon V1 remains a fictional one-carriageway highway (id `ayalon`, width 28, 8 lanes of 3.5, 27 hashed spline samples, 50 interchange ramps, 8 open checkpoints, 9 inspired POIs). The opposite carriageway stays visual-only. Owner freeze and GIS claims remain forbidden. GitHub still does not require production checks through branch settings.

Ghosts remain byte-preserved and remain assigned outside this unit. The RSH-028 one-unit `next` / `המשך` authority is consumed on merge. RSH-029 remains deferred and cannot start without a new explicit owner instruction.
