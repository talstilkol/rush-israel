...
1.2.2. כביש: albedo לבן × PNG + envMap מהסצנה + `uWet` + רמפות — **DONE** (tiny: IBL/clearcoat; אין SSR).  
1.2.8. עצים עם נפח (כדורים/חרוטים, לא מטוסים) — **DONE** (tiny).  
...
**1.2.8** עצים עם נפח במקום כרטיסי קרטון. Freeze עדיין חסום.
