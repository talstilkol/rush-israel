...
//#endregion
//#region server/middleware/game-cache.ts
async function gameCacheMiddleware(event, next) {
...
//#endregion
//#region scripts/rush-pwa.mjs
/** @typedef {import("./grok-pwa-shared.mjs").OgSite} OgSite */
var RUSH_PWA_NAME = "RUSH Israel";
var RUSH_PWA_DESCRIPTION = "Private owner-controlled Three.js WebGL simcade driving game on fictional routes inspired by Israeli places.";
/**
* @param {OgSite | null | undefined} site
* @returns {string}
*/
function productPwaName(site) {
	return typeof site?.title === "string" && site.title.trim() ? site.title.trim() : RUSH_PWA_NAME;
}
/**
...
* @returns {string}
*/
function renderRushWebManifest(host, site) {
	...
	return JSON.stringify({
		...
		name,
		short_name: name,
		description: typeof site?.description === "string" && site.description.trim() ? site.description.trim() : RUSH_PWA_DESCRIPTION,
		start_url: "/",
		scope: "/"
	});
}
...
* @returns {string}
*/
function renderRushInstallPageHtml(template, { host, url, site }) {
	...
	return renderInstallPageHtml(template, {
		host,
		url
	}).replaceAll(genericName, productName);
}
//#endregion
...
async function grokPwaMiddleware(event, next) {
	...
	if (path === "/__grok/manifest.webmanifest" || path === "/__grok/manifest.json") return new Response(renderRushWebManifest(requestHost(event), grokOgIdentity.site), { headers: {
		"content-type": "application/manifest+json; charset=utf-8",
		"cache-control": "no-cache"
	} });
	if (isInstallQuery(urlWithQuery) && isDocumentPath(path) && acceptsHtml(event.req.headers.get("accept"))) {
		const html = renderRushInstallPageHtml(install_page_default, {
...
var _lazy_w1TEse = defineLazyEventHandler(() => import("./_chunks/ssr-renderer.mjs"));
var findRoute = /* @__PURE__ */ (() => {
	const data = {
		route: "/**",
		handler: _lazy_w1TEse
	};
	return ((_m, p) => {
		return {
			data,
...
var globalMiddleware = [toEventHandler(gameCacheMiddleware), toEventHandler(grokPwaMiddleware)].filter(Boolean);
//#endregion
