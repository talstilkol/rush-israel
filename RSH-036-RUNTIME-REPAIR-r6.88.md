# RSH-036 r6.88 — leftover g07 after world.group+outside is remaining empty-scene vs golden, not shadowMap.type, not shadowMap.needsUpdate, not clippingPlanes, not env

Candidate, 12 September 2026. Base 79389fd
(r6.87 dsa isolation). No merge or freeze; 35/67 accepted;
32 remain. r6.17, r6.18 and r6.27 exact-head failures are retained. Do not
relabel them as passes. r6.87 exact-head CI is not claimed. GFX-01, GFX-03,
...
## AUD-97: leftover g07 empty-scene vs golden is not shadowMap.type vs shadowMap.needsUpdate vs clippingPlanes

r6.87 leftover g07 after world.group+outside hidden is remaining empty-scene
vs golden residual (leftover 0.2027), not dithering, not shadowMap.autoUpdate,
not info.autoReset and not env. The new probe isolates leftover after
world.group+outside as remaining empty-scene vs golden independently of
shadowMap.type vs shadowMap.needsUpdate vs clippingPlanes after
...
autoClearColor/autoClearDepth/autoClearStencil or dithering/autoUpdate/autoReset.
Product renderer stays on the leftover path (shadowMap.type PCFShadowMap /
shadowMap.needsUpdate false / clippingPlanes empty). Isolation uses
`BasicShadowMap`, `shadowMap.needsUpdate = true`, and a Y-up clipping plane.
...
| pose | leftover % | type Δ | needs Δ | clip Δ | env Δ | both Δ | pack Δ |
|---|---:|---:|---:|---:|---:|---:|---:|
...
geometry for a shadowMap.type/needsUpdate/clippingPlanes change to occupy
remaining g07. Product renderer stays. Counts: leftover 2082.

Local unique cases: 28 in `scripts/world-tnc.test.mjs`. Previous r6.87
expected 2,961 + 28 = 2,989. Pixel 0/4, immutable CDN bytes, rendering
