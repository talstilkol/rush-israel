...
**Version:** 1.2.0  
**Verified main baseline:** `6932a74ca23f125e212f37fa0db73da377eaabe3`  
**Date:** 29 August 2026  
**Total:** 42 findings — 12 P0, 18 P1, 12 P2  
**Status:** 30 OPEN, 9 MITIGATED, 3 CLOSED

## Status rules

- `OPEN`: verified issue without an accepted remediation.
- `MITIGATED`: accepted policy or implementation exists, but final enforcement or runtime evidence is missing.
- `CLOSED`: owner-accepted remediation is merged and its applicable validation passes.

No finding is closed merely because a plan or unexecuted script exists. RSH-007 is
required for clean-clone GitHub-hosted execution evidence.
...
| P0-02 | P0 | **OPEN** | `main` is not protected | No protection, required checks or ruleset exists. | RSH-008 |
| P0-03 | P0 | **MITIGATED** | No branch-and-PR delivery workflow exists | RSH-001–RSH-006 use unit branches and PRs; policy and templates are merged, but technical enforcement is pending. | RSH-008 |
| P0-04 | P0 | **OPEN** | No GitHub Actions CI is present | No tracked workflow or status-check evidence exists. | RSH-007 |
| P0-05 | P0 | **MITIGATED** | The QA command is not self-contained | RSH-006 implements automatic server startup, readiness, timeout and cleanup; clean-clone execution remains unverified. | RSH-007 |
| P0-06 | P0 | **CLOSED** | Automation contains absolute `/workspace` paths | RSH-005 migrated executable automation to module-relative roots; exact-branch search returned zero and `check:paths` prevents recurrence. | RSH-005 |
| P0-07 | P0 | **OPEN** | Secret scanning is too narrow | The current scan still covers only `src/game` and a small pattern set. | RSH-024 |
| P0-08 | P0 | **MITIGATED** | Operational contracts are ignored by Git | Governance, contribution, security, toolchain and QA contracts are versioned; ignored AGENTS/startup files still require a decision. | RSH-002, RSH-004 |
...
| P0-11 | P0 | **MITIGATED** | No version, tag, release or rollback baseline exists | Immutable baseline, milestone, label and release registers now exist; tags, releases and rollback proof remain absent. | RSH-063, RSH-064, RSH-067 |
...
| P1-01 | P1 | **OPEN** | Production QA-hook removal is not enforced by GitHub | A local check exists but no required GitHub CI check executes it. | RSH-007, RSH-024 |
...
| P1-05 | P1 | **OPEN** | The build command performs database migration | `npm run build` still invokes `db:migrate`. | RSH-024 |
...
| P1-09 | P1 | **MITIGATED** | The toolchain cannot be reproduced exactly | RSH-004 pins Node/npm across nvm, node-version, Volta and package metadata with fail-closed verification; clean CI install is pending. | RSH-007 |
| P1-10 | P1 | **OPEN** | Lint is not part of `qa:ci` | The lint command is still outside the aggregate gate. | RSH-007 |
| P1-11 | P1 | **OPEN** | The complete test suite is not part of `qa:ci` | The aggregate command still selects tests rather than executing the complete test command. | RSH-007 |
| P1-12 | P1 | **OPEN** | Important validation scripts are outside the main gate | Golden, damage, Ayalon lock, HaShalom, auth and long-soak checks are not consistently required. | RSH-007, RSH-008, RSH-035, RSH-042 |
...
| P2-04 | P2 | **OPEN** | Commit cadence and direct-main history reduce reviewability | New work uses PRs, but historical rapid direct-main changes remain and technical enforcement is pending. | RSH-008 |
...
| P2-08 | P2 | **MITIGATED** | README does not document reproducible setup and validation | `TOOLCHAIN.md` and `QA-HARNESS.md` now define exact setup and QA contracts; README integration remains pending. | RSH-012 |
