...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of shadowMap.type vs shadowMap.needsUpdate vs clippingPlanes. */
export const TNC_FRAMES = REGION_FRAMES;
export const TNC_LAYERS = Object.freeze([
  'type', 'needs', 'clip', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = TNC_LAYERS;
export const TYPE_IDS = Object.freeze(['type']);
export const NEEDS_IDS = Object.freeze(['needs']);
...
export const TNC_L2_MIN = 8;
export const TNC_LUMA_MIN = 8;
export const TNC_BLUE_MIN = 8;
export const TNC_BAND_MIN = 0.02;
...
export const PRODUCT_NEEDS = false;
export const PRODUCT_CLIP = 0;
...
export function dominantTnc(layers) {
  if (!Array.isArray(layers) || layers.length !== TNC_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotTncMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== TNC_LAYERS.length
    || TNC_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !TNC_LAYERS.includes(layer.id)
    ...
    || !frame.dominantTnc
    || !frame.dominant
    || !Array.isArray(frame.typeAxis)
    || !Array.isArray(frame.envAxis)
    || !Array.isArray(frame.bothAxis)
    || !Array.isArray(frame.needsAxis)
...
export function tncBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-tnc-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { type: 2083, needs: 2083, clip: 2083, env: 2083, both: 2084, pack: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'type' ? 18 : id === 'both' ? 20 : id === 'pack' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'type' ? -0.10 : id === 'both' ? -0.08 : id === 'pack' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = TNC_FRAMES.find(row => row.id === id);
  ...
  const layers = TNC_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantTnc: 'type',
    contributingTnc: ['type', 'pack'],
    typeAxis: ['type'],
    envAxis: [],
    bothAxis: ['both'],
    needsAxis: [],
...
export function worldTncFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, typeCount: 2083, needsCount: 2083, clipCount: 2083, envCount: 2083, bothCount: 2086, packCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    typeClears: true, typeHidesLeftover: true, typeKeepsEnv: true, typeKeepsHemi: true, typeKeepsNeeds: true, typeKeepsClip: true,
    needsClears: true, needsHidesLeftover: true, needsKeepsEnv: true, needsKeepsHemi: true, needsKeepsType: true, needsKeepsClip: true,
    clipClears: true, clipHidesLeftover: true, clipKeepsEnv: true, clipKeepsHemi: true, clipKeepsType: true, clipKeepsNeeds: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsType: true, envKeepsNeeds: true, envKeepsClip: true, envClearsHue: false,
    packClearsType: true, packClearsNeeds: true, packClearsClip: true, packHidesLeftover: true, packKeepsEnv: true, packKeepsHemi: true,
    bothSetsGray: true, bothClearsType: true, bothClearsNeeds: true, bothClearsClip: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    tncDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-tnc-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: TNC_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldTncResults(r) {
  assert.ok(r && typeof r === 'object', 'world-tnc evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-tnc frames missing');
  assert.equal(leftoverIsNotTncMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of shadowMap.type vs shadowMap.needsUpdate vs clippingPlanes mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'tnc isolation still combined with hemi');
  assert.equal(r.typeClears, true, 'type isolation did not force BasicShadowMap');
  assert.equal(r.typeHidesLeftover, true, 'type isolation did not hide leftover empty-scene meshes');
  assert.equal(r.typeKeepsEnv, true, 'type isolation clears scene.environment');
  assert.equal(r.typeKeepsHemi, true, 'hemi.color left product during type isolation');
  assert.equal(r.typeKeepsNeeds, true, 'shadowMap.needsUpdate left product during type isolation');
  assert.equal(r.typeKeepsClip, true, 'clippingPlanes left product during type isolation');
  assert.equal(r.needsClears, true, 'needs isolation did not force shadowMap.needsUpdate on');
  assert.equal(r.needsHidesLeftover, true, 'needs isolation did not hide leftover empty-scene meshes');
  assert.equal(r.needsKeepsEnv, true, 'needs isolation clears scene.environment');
  assert.equal(r.needsKeepsHemi, true, 'hemi.color left product during needs isolation');
  assert.equal(r.needsKeepsType, true, 'shadowMap.type left product during needs isolation');
  assert.equal(r.needsKeepsClip, true, 'clippingPlanes left product during needs isolation');
  assert.equal(r.clipClears, true, 'clip isolation did not force clippingPlanes');
  ...
  assert.equal(r.clipKeepsType, true, 'shadowMap.type left product during clip isolation');
  assert.equal(r.clipKeepsNeeds, true, 'shadowMap.needsUpdate left product during clip isolation');
  ...
  assert.equal(r.envKeepsType, true, 'shadowMap.type left product during env isolation');
  assert.equal(r.envKeepsNeeds, true, 'shadowMap.needsUpdate left product during env isolation');
  assert.equal(r.envKeepsClip, true, 'clippingPlanes left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsType, true, 'pack isolation did not force BasicShadowMap');
  assert.equal(r.packClearsNeeds, true, 'pack isolation did not force shadowMap.needsUpdate on');
  assert.equal(r.packClearsClip, true, 'pack isolation did not force clippingPlanes');
  ...
  assert.equal(r.bothClearsType, true, 'both isolation did not force BasicShadowMap');
  assert.equal(r.bothClearsNeeds, true, 'both isolation did not force shadowMap.needsUpdate on');
  assert.equal(r.bothClearsClip, true, 'both isolation did not force clippingPlanes');
  ...
  assert.equal(r.intensityClearsIbl, false, 'tnc isolation clears scene.environment via sun');
  assert.equal(r.tncDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after tnc restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during tnc probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during tnc probe');
  ...
  assert.ok(r.typeCount >= 151, 'type isolation missing');
  assert.ok(r.needsCount >= 151, 'needs isolation missing');
  assert.ok(r.clipCount >= 151, 'clip isolation missing');
  assert.ok(r.packCount >= 151, 'combined shadowMap.type-shadowMap.needsUpdate-clippingPlanes isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both shadowMap.type-shadowMap.needsUpdate-clippingPlanes+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during tnc probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during tnc probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during tnc probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during tnc probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during tnc probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during tnc probe');
  ...
  assert.equal(tncBufferIsNotOriginalGolden(r), false, 'tnc buffer is not original-golden');
  ...
  for (const spec of TNC_FRAMES) {
    ...
    if (!row.dominantTnc || !row.dominant || row.layers.length !== TNC_LAYERS.length
      || TNC_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.typeAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.needsAxis) || !Array.isArray(row.clipAxis) || !Array.isArray(row.packAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || TNC_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of shadowMap.type vs shadowMap.needsUpdate vs clippingPlanes attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present tnc axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'shadowMap.type vs shadowMap.needsUpdate vs clippingPlanes independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'tnc-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldTnc(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldTncResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldTnc(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let typeClears = false;
        let typeHidesLeftover = false;
        let typeKeepsEnv = true;
        let typeKeepsHemi = true;
        let typeKeepsNeeds = true;
        let typeKeepsClip = true;
        let needsClears = false;
        let needsHidesLeftover = false;
        let needsKeepsEnv = true;
        let needsKeepsHemi = true;
        let needsKeepsType = true;
        let needsKeepsClip = true;
        let clipClears = false;
        let clipHidesLeftover = false;
        let clipKeepsEnv = true;
        let clipKeepsHemi = true;
        let clipKeepsType = true;
        let clipKeepsNeeds = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsType = true;
        let envKeepsNeeds = true;
        let envKeepsClip = true;
        let packClearsType = false;
        let packClearsNeeds = false;
        let packClearsClip = false;
        let packHidesLeftover = false;
        let packKeepsEnv = true;
        let packKeepsHemi = true;
        let bothSetsGray = false;
        let bothClearsType = false;
        let bothClearsNeeds = false;
        let bothClearsClip = false;
        ...
        const setType = () => {
          const prev = engine.renderer.shadowMap.type;
          engine.renderer.shadowMap.type = THREE.BasicShadowMap;
          return [() => { engine.renderer.shadowMap.type = prev; }];
        };
        const setNeeds = () => {
          const prev = engine.renderer.shadowMap.needsUpdate;
          engine.renderer.shadowMap.needsUpdate = true;
          return [() => { engine.renderer.shadowMap.needsUpdate = prev; }];
        };
        const setClip = () => {
          const prev = engine.renderer.clippingPlanes;
          engine.renderer.clippingPlanes = [new THREE.Plane(new THREE.Vector3(0, 1, 0), 0)];
          return [() => { engine.renderer.clippingPlanes = prev; }];
        };
        const isolate = {
          type() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevNeeds = engine.renderer.shadowMap.needsUpdate;
            const prevClip = engine.renderer.clippingPlanes;
            const restorers = [...hideMeshes(leftoverOf()), ...setType()];
            typeClears = true;
            if (leftoverHidden()) typeHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) typeKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) typeKeepsHemi = false;
            if (engine.renderer.shadowMap.needsUpdate !== prevNeeds) typeKeepsNeeds = false;
            if (engine.renderer.clippingPlanes !== prevClip) typeKeepsClip = false;
            return restorers;
          },
          needs() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevType = engine.renderer.shadowMap.type;
            const prevClip = engine.renderer.clippingPlanes;
            const restorers = [...hideMeshes(leftoverOf()), ...setNeeds()];
            needsClears = true;
            if (leftoverHidden()) needsHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) needsKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) needsKeepsHemi = false;
            if (engine.renderer.shadowMap.type !== prevType) needsKeepsType = false;
            if (engine.renderer.clippingPlanes !== prevClip) needsKeepsClip = false;
            return restorers;
          },
          clip() {
            const prevHemi = hemiRgb();
            ...
            const prevType = engine.renderer.shadowMap.type;
            const prevNeeds = engine.renderer.shadowMap.needsUpdate;
            const restorers = [...hideMeshes(leftoverOf()), ...setClip()];
            clipClears = true;
            ...
            if (engine.renderer.shadowMap.type !== prevType) clipKeepsType = false;
            if (engine.renderer.shadowMap.needsUpdate !== prevNeeds) clipKeepsNeeds = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.shadowMap.type !== prevType) envKeepsType = false;
            if (engine.renderer.shadowMap.needsUpdate !== prevNeeds) envKeepsNeeds = false;
            if (engine.renderer.clippingPlanes !== prevClip) envKeepsClip = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
        ...
        const bothIsCombined = !typeKeepsHemi || !needsKeepsHemi || !clipKeepsHemi || !envKeepsHemi || !packKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.type();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.needs();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.clip();
      ...
      frames: TNC_FRAMES,
      layers: ['type', 'needs', 'clip', 'env', 'both', 'pack'],
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = TNC_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantTnc: dominantTnc(layers),
        contributingTnc: layers.filter(layer => layer.bandDelta <= -TNC_BAND_MIN).map(layer => layer.id),
        typeAxis: layers.filter(layer => TYPE_IDS.includes(layer.id) && layer.bandDelta <= -TNC_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -TNC_BAND_MIN || layer.blueDelta <= -TNC_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -TNC_BAND_MIN).map(layer => layer.id),
        needsAxis: layers.filter(layer => NEEDS_IDS.includes(layer.id) && layer.bandDelta <= -TNC_BAND_MIN).map(layer => layer.id),
        clipAxis: layers.filter(layer => CLIP_IDS.includes(layer.id) && layer.bandDelta <= -TNC_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -TNC_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      typeClears: report.typeClears,
      typeHidesLeftover: report.typeHidesLeftover,
      typeKeepsEnv: report.typeKeepsEnv,
      typeKeepsHemi: report.typeKeepsHemi,
      typeKeepsNeeds: report.typeKeepsNeeds,
      typeKeepsClip: report.typeKeepsClip,
      needsClears: report.needsClears,
      needsHidesLeftover: report.needsHidesLeftover,
      needsKeepsEnv: report.needsKeepsEnv,
      needsKeepsHemi: report.needsKeepsHemi,
      needsKeepsType: report.needsKeepsType,
      needsKeepsClip: report.needsKeepsClip,
      clipClears: report.clipClears,
      clipHidesLeftover: report.clipHidesLeftover,
      clipKeepsEnv: report.clipKeepsEnv,
      clipKeepsHemi: report.clipKeepsHemi,
      clipKeepsType: report.clipKeepsType,
      clipKeepsNeeds: report.clipKeepsNeeds,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsType: report.envKeepsType,
      envKeepsNeeds: report.envKeepsNeeds,
      envKeepsClip: report.envKeepsClip,
      envClearsHue: report.envClearsHue,
      packClearsType: report.packClearsType,
      packClearsNeeds: report.packClearsNeeds,
      packClearsClip: report.packClearsClip,
      ...
      bothClearsType: report.bothClearsType,
      bothClearsNeeds: report.bothClearsNeeds,
      bothClearsClip: report.bothClearsClip,
      bothHidesLeftover: report.bothHidesLeftover,
      bothKeepsHemi: report.bothKeepsHemi,
      intensityClearsIbl: report.intensityClearsIbl,
      tncDeltasUseEmptyBaseline: report.tncDeltasUseEmptyBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-tnc-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldTnc(browser, url) {
  const report = await measureWorldTnc(browser, url);
  const out = process.env.WORLD_TNC_OUTPUT ?? fromRoot('artifacts', 'world-tnc');
  const results = await retainWorldTnc(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldTnc(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
