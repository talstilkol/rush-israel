...
import {
  readWorldBuilderInputs,
  validateWorldBuilders,
} from "./check-world-builders.mjs";

function baseline() {
  return structuredClone(readWorldBuilderInputs());
}
...
test("missing, substituted or reordered builders fail closed", () => {
  ...
  [manifest.extraction.modules[0], manifest.extraction.modules[1]] = [
    manifest.extraction.modules[1],
    manifest.extraction.modules[0],
  ];
...
test("track ID branching and cross-builder imports fail closed", () => {
  ...
  assert.match(
    ...
    /identity changed|runtime ID branching|imports another builder/,
  );
});

test("world facade bypass or monolithic ownership regrowth fails closed", () => {
  const input = baseline();
  input.worldSource = input.worldSource.replace(
...
test("track, physics, asset and dependency identities remain closed", () => {
  ...
  assert.match(
    messages(validateWorldBuilders(tracks)),
    /track-module manifest|track definition digest/,
  );

  const physics = baseline();
  physics.preservedSources["src/game/physics.ts"] =
    physics.preservedSources["src/game/physics.ts"].replace(
      "PHYSICS_HZ = 120",
      "PHYSICS_HZ = 60",
    );
  ...
  assets.assetSource = assets.assetSource.replace(
    '"unverified_asset_files": 66',
    '"unverified_asset_files": 65',
  );
  assert.match(
    messages(validateWorldBuilders(assets)),
    /asset|distribution|release/,
  );
...
test("@ts-nocheck is forbidden in generated builders", () => {
  ...
  assert.match(
    messages(validateWorldBuilders(input)),
    /@ts-nocheck is forbidden|identity changed/,
  );
});
