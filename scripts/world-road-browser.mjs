...
/** Combined leftover gray cubemap vs IBL-off is not road vs all-standard vs car envMapIntensity attribution. */
export const ROAD_FRAMES = REGION_FRAMES;
export const ROAD_LAYERS = Object.freeze([
  'road', 'std', 'car', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['road']);
export const STD_IDS = Object.freeze(['std']);
export const CAR_IDS = Object.freeze(['car']);
export const HUE_IDS = Object.freeze(['hue']);
export const GRAY_BASELINE_IDS = Object.freeze(['road', 'std', 'car']);
export const ROAD_L2_MIN = 8;
export const ROAD_LUMA_MIN = 8;
export const ROAD_BLUE_MIN = 8;
...
export function dominantRoad(layers) {
  if (!Array.isArray(layers) || layers.length !== ROAD_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function lodIsNotRoadMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== ROAD_LAYERS.length
    || ROAD_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !ROAD_LAYERS.includes(layer.id)
    ...
    || !Array.isArray(frame.stdAxis)
    || !Array.isArray(frame.carAxis)
...
export function roadBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-road-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { road: 1, std: 1, car: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'car' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'car' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = ROAD_FRAMES.find(row => row.id === id);
  ...
  const layers = ROAD_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantRoad: 'hue',
    contributingRoad: ['car', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    roadAxis: [],
    stdAxis: [],
    carAxis: ['car'],
...
export function worldRoadFixture(extra = {}) {
  return {
    ...
    roadCount: 1, stdCount: 1, carCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, roadIsCombined: false, roadClearsOff: false,
    roadClearsHue: false, stdClearsRoad: false, hueClearsOff: false, intensityClearsIbl: false,
    roadDeltasUseGrayBaseline: true,
    roadZerosRoadEnvMapIntensity: true, stdZerosAllEnvMapIntensity: true, carZerosHeroEnvMapIntensity: true,
    sizeStaysProductPmrem: true, lodKeepsGroundRoughness: true, envKeepsGroundEnvMapIntensity: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-road-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: ROAD_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldRoadResults(r) {
  assert.ok(r && typeof r === 'object', 'world-road evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-road frames missing');
  assert.equal(lodIsNotRoadMismatch(r), false, 'combined leftover gray cubemap vs IBL-off is not road vs all-standard vs car envMapIntensity mismatch');
  ...
  assert.equal(r.roadIsCombined, false, 'road isolation still combined with hemi or disc');
  assert.equal(r.roadClearsOff, false, 'road isolation clears IBL-off');
  assert.equal(r.roadClearsHue, false, 'road isolation clears gray cubemap hue');
  assert.equal(r.stdClearsRoad, false, 'std isolation uses road-only envMapIntensity');
  ...
  assert.equal(r.roadDeltasUseGrayBaseline, true, 'road vs std vs car still uses product present as baseline');
  assert.equal(r.roadZerosRoadEnvMapIntensity, true, 'road envMapIntensity stays product during road isolation');
  assert.equal(r.stdZerosAllEnvMapIntensity, true, 'all-standard envMapIntensity stays product during std isolation');
  assert.equal(r.carZerosHeroEnvMapIntensity, true, 'hero envMapIntensity stays product during car isolation');
  assert.equal(r.sizeStaysProductPmrem, true, 'PMREM size left product 256 during road probe');
  assert.equal(r.lodKeepsGroundRoughness, true, 'ground roughness left product during road probe');
  assert.equal(r.envKeepsGroundEnvMapIntensity, true, 'ground envMapIntensity left product during road probe');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.roadCount >= 1, 'road envMapIntensity isolation missing');
  assert.ok(r.stdCount >= 1, 'all-standard envMapIntensity isolation missing');
  assert.ok(r.carCount >= 1, 'hero-car envMapIntensity isolation missing');
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during road probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during road probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during road probe');
  ...
  assert.equal(roadBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of ROAD_FRAMES) {
    ...
    if (!row.dominantRoad || !row.dominant || row.layers.length !== ROAD_LAYERS.length
      || ROAD_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.roadAxis) || !Array.isArray(row.stdAxis)
      || !Array.isArray(row.carAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || ROAD_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover gray cubemap vs IBL-off is not road vs all-standard vs car envMapIntensity attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present road axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'road vs all-standard vs car envMapIntensity independently of PMREM size vs CubeUV lod vs ground envMapIntensity vs 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'road-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldRoad(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldRoadResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldRoad(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let roadZerosRoadEnvMapIntensity = false;
        let stdZerosAllEnvMapIntensity = false;
        let carZerosHeroEnvMapIntensity = false;
        ...
        const groundMaterials = () => {
          const mats = [];
          for (const mesh of worldMeshes) {
            if (!isGroundMesh(mesh)) continue;
            for (const mat of materialsOf(mesh)) mats.push(mat);
          }
          return mats;
        };
        const bakeGray = () => {
        ...
        const zeroEnv = (mats, flag) => {
          const restorers = [];
          for (const mat of mats) {
            if (mat.envMapIntensity == null) continue;
            const intensity = mat.envMapIntensity;
            mat.envMapIntensity = 0;
            if (mat.envMapIntensity === 0) flag();
            restorers.push(() => { mat.envMapIntensity = intensity; });
          }
          return restorers;
        };
        const isolate = {
          road() {
            ...
            const restorers = zeroEnv(
              materialsMatching((_, mat) => isRoadMaterial(mat)),
              () => { roadZerosRoadEnvMapIntensity = true; },
            );
          ...
          std() {
            const [restoreEnv] = swapEnv(bakeGray());
            const restorers = zeroEnv(
              materialsMatching((_, mat) => !!(mat && (mat.isMeshStandardMaterial || mat.isMeshPhysicalMaterial))),
              () => { stdZerosAllEnvMapIntensity = true; },
            );
            return [() => { restorers.forEach(restore => restore()); restoreEnv(); }];
          },
          car() {
            ...
            const restorers = zeroEnv(
              materialsMatching((object) => !!(heroRoot && under(object, heroRoot))),
              () => { carZerosHeroEnvMapIntensity = true; },
            );
            return [() => { restorers.forEach(restore => restore()); restoreEnv(); }];
          },
          hue() {
        ...
        const groundBefore = groundMaterials().map(mat => ({ roughness: mat.roughness, envMapIntensity: mat.envMapIntensity }));
        const roadRestorers = isolate.road();
        const roadTex = engine.scene.environment;
        const roadIsCombined = roadRestorers.length !== 1;
        const roadClearsOff = roadTex == null;
        const roadClearsHue = roadTex === hueProbe.texture;
        roadRestorers.forEach(restore => restore());
        const stdRestorers = isolate.std();
        const stdClearsRoad = engine.scene.environment == null
          || engine.scene.environment === roadTex;
        stdRestorers.forEach(restore => restore());
        ...
        const sizeStaysProductPmrem = true;
        const lodKeepsGroundRoughness = groundMaterials().every((mat, i) => mat.roughness === groundBefore[i]?.roughness);
        const envKeepsGroundEnvMapIntensity = groundMaterials().every((mat, i) => mat.envMapIntensity === groundBefore[i]?.envMapIntensity);
        ...
        return {
          ...
          intensityClearsIbl,
          roadDeltasUseGrayBaseline: true,
          roadZerosRoadEnvMapIntensity,
          stdZerosAllEnvMapIntensity,
          carZerosHeroEnvMapIntensity,
          sizeStaysProductPmrem,
          lodKeepsGroundRoughness,
          envKeepsGroundEnvMapIntensity,
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = ROAD_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantRoad: dominantRoad(layers),
        contributingRoad: layers.filter(layer => layer.l2Delta <= -ROAD_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -ROAD_LUMA_MIN).map(layer => layer.id),
        roadAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -ROAD_BLUE_MIN).map(layer => layer.id),
        stdAxis: layers.filter(layer => STD_IDS.includes(layer.id) && (layer.l2Delta <= -ROAD_L2_MIN || layer.blueDelta <= -ROAD_BLUE_MIN)).map(layer => layer.id),
        carAxis: layers.filter(layer => CAR_IDS.includes(layer.id) && layer.blueDelta <= -ROAD_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -ROAD_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      roadIsCombined: report.roadIsCombined,
      roadClearsOff: report.roadClearsOff,
      roadClearsHue: report.roadClearsHue,
      stdClearsRoad: report.stdClearsRoad,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      roadDeltasUseGrayBaseline: report.roadDeltasUseGrayBaseline,
      roadZerosRoadEnvMapIntensity: report.roadZerosRoadEnvMapIntensity,
      stdZerosAllEnvMapIntensity: report.stdZerosAllEnvMapIntensity,
      carZerosHeroEnvMapIntensity: report.carZerosHeroEnvMapIntensity,
      sizeStaysProductPmrem: report.sizeStaysProductPmrem,
      lodKeepsGroundRoughness: report.lodKeepsGroundRoughness,
      envKeepsGroundEnvMapIntensity: report.envKeepsGroundEnvMapIntensity,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-road-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldRoad(browser, url) {
  const report = await measureWorldRoad(browser, url);
  const out = process.env.WORLD_ROAD_OUTPUT ?? fromRoot('artifacts', 'world-road');
  const results = await retainWorldRoad(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldRoad(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
