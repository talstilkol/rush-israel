...
const cam0 = await page.evaluate(() => window.__controlsTest?.getCamMode?.());
await page.keyboard.press("KeyC");
await page.waitForTimeout(80);
const cam1 = await page.evaluate(() => window.__controlsTest?.getCamMode?.());
if (cam1 !== ((cam0 ?? 0) + 1) % 4) throw new Error(`C did not cycle camera ${cam0} -> ${cam1}`);
...
await page.evaluate(() => window.__controlsTest?.finishNow?.());
await page.waitForTimeout(400);
