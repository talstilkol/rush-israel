...
/** Combined leftover gray IBL is not bare 0x808080 background vs baked hemi vs baked disc attribution. */
export const BARE_FRAMES = REGION_FRAMES;
export const BARE_LAYERS = Object.freeze([
  'background', 'hemi', 'disc', 'gray', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['background']);
export const GRAY_IDS = Object.freeze(['gray']);
export const HUE_IDS = Object.freeze(['hue']);
export const DISC_IDS = Object.freeze(['disc']);
export const HEMI_IDS = Object.freeze(['hemi']);
export const GRAY_BASELINE_IDS = Object.freeze(['background', 'hemi', 'disc', 'gray']);
export const BARE_L2_MIN = 8;
export const BARE_LUMA_MIN = 8;
export const BARE_BLUE_MIN = 8;
...
export function dominantBare(layers) {
  if (!Array.isArray(layers) || layers.length !== BARE_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function grayIsNotBareMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== BARE_LAYERS.length
    || BARE_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !BARE_LAYERS.includes(layer.id)
    ...
    || !Array.isArray(frame.grayAxis)
    || !Array.isArray(frame.hueAxis)
    || !Array.isArray(frame.hemiAxis)
    || !Array.isArray(frame.discAxis)
...
export function bareBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-bare-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { background: 1, hemi: 1, disc: 1, gray: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'background' || id === 'gray' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'background' || id === 'gray' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = BARE_FRAMES.find(row => row.id === id);
  ...
  const layers = BARE_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantBare: 'hue',
    contributingBare: ['background', 'gray', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: ['background'],
    grayAxis: ['gray'],
    hueAxis: ['hue'],
...
export function worldBareFixture(extra = {}) {
  return {
    ...
    backgroundCount: 1, hemiCount: 1, discCount: 1, grayCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, backgroundIsCombined: false, backgroundClearsGray: false,
    backgroundClearsHue: false, hemiClearsBackground: false, hueClearsGray: false, intensityClearsIbl: false,
    backgroundDeltasUseGrayBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-bare-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: BARE_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldBareResults(r) {
  assert.ok(r && typeof r === 'object', 'world-bare evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-bare frames missing');
  assert.equal(grayIsNotBareMismatch(r), false, 'combined leftover gray IBL is not bare 0x808080 background vs baked hemi vs baked disc mismatch');
  ...
  assert.equal(r.backgroundIsCombined, false, 'background isolation still combined with hemi or disc');
  assert.equal(r.backgroundClearsGray, false, 'background isolation clears IBL-off');
  assert.equal(r.backgroundClearsHue, false, 'background isolation clears gray cubemap hue');
  assert.equal(r.hemiClearsBackground, false, 'hemi isolation clears bare 0x808080 background');
  assert.equal(r.hueClearsGray, false, 'hue isolation clears IBL-off');
  assert.equal(r.intensityClearsIbl, false, 'intensity isolation clears scene.environment');
  assert.equal(r.backgroundDeltasUseGrayBaseline, true, 'background vs hemi vs disc still uses product present as baseline');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.backgroundCount >= 1, 'bare 0x808080 background missing');
  assert.ok(r.hemiCount >= 1, 'gray PMREM hemi missing');
  assert.ok(r.discCount >= 1, 'gray PMREM disc missing');
  assert.ok(r.grayCount >= 1, 'IBL-off gray missing');
  ...
  assert.equal(bareBufferIsNotOriginalGolden(r), false, 'bare buffer is not original-golden');
  ...
  for (const spec of BARE_FRAMES) {
    ...
    if (!row.dominantBare || !row.dominant || row.layers.length !== BARE_LAYERS.length
      || BARE_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.grayAxis)
      || !Array.isArray(row.hueAxis) || !Array.isArray(row.hemiAxis) || !Array.isArray(row.discAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || BARE_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover gray IBL is not bare 0x808080 background vs baked hemi vs baked disc attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present bare axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'gray IBL 0x808080 background vs hemi vs disc independently of IBL-off vs 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'bare-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldBare(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldBareResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldBare(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const isolate = {
          background() {
            return swapEnv(bakeVariant({ skyHex: 0x808080, background: true }));
          },
          hemi() {
            return swapEnv(bakeVariant({ skyHex: 0x808080, hemi: true }));
          },
          disc() {
            return swapEnv(bakeVariant({ skyHex: 0x808080, disc: true }));
          },
          gray() {
        ...
        const bgProbe = bakeVariant({ skyHex: 0x808080, background: true });
        const bgRestorers = isolate.background();
        const backgroundIsCombined = bgRestorers.length !== 1;
        const backgroundClearsGray = engine.scene.environment == null;
        const backgroundClearsHue = engine.scene.environment === hueProbe.texture;
        bgRestorers.forEach(restore => restore());
        const hemiRestorers = isolate.hemi();
        const hemiClearsBackground = engine.scene.environment == null
          || engine.scene.environment === bgProbe.texture;
        hemiRestorers.forEach(restore => restore());
        const hueRestorers = isolate.hue();
        const hueClearsGray = engine.scene.environment == null;
        ...
        return {
          ...
          backgroundIsCombined,
          backgroundClearsGray,
          backgroundClearsHue,
          hemiClearsBackground,
          hueClearsGray,
          intensityClearsIbl,
          backgroundDeltasUseGrayBaseline: true,
          ...counts,
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = BARE_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantBare: dominantBare(layers),
        contributingBare: layers.filter(layer => layer.l2Delta <= -BARE_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -BARE_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -BARE_BLUE_MIN).map(layer => layer.id),
        grayAxis: layers.filter(layer => GRAY_IDS.includes(layer.id) && layer.blueDelta <= -BARE_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -BARE_BLUE_MIN).map(layer => layer.id),
        hemiAxis: layers.filter(layer => HEMI_IDS.includes(layer.id) && (layer.l2Delta <= -BARE_L2_MIN || layer.blueDelta <= -BARE_BLUE_MIN)).map(layer => layer.id),
        discAxis: layers.filter(layer => DISC_IDS.includes(layer.id) && (layer.l2Delta <= -BARE_L2_MIN || layer.blueDelta <= -BARE_BLUE_MIN)).map(layer => layer.id),
      };
    });
    return {
      ...
      backgroundIsCombined: report.backgroundIsCombined,
      backgroundClearsGray: report.backgroundClearsGray,
      backgroundClearsHue: report.backgroundClearsHue,
      hemiClearsBackground: report.hemiClearsBackground,
      hueClearsGray: report.hueClearsGray,
      intensityClearsIbl: report.intensityClearsIbl,
      backgroundDeltasUseGrayBaseline: report.backgroundDeltasUseGrayBaseline,
      backgroundCount: report.backgroundCount,
      hemiCount: report.hemiCount,
      discCount: report.discCount,
      grayCount: report.grayCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-bare-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldBare(browser, url) {
  const report = await measureWorldBare(browser, url);
  const out = process.env.WORLD_BARE_OUTPUT ?? fromRoot('artifacts', 'world-bare');
  const results = await retainWorldBare(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldBare(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
