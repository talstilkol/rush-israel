...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of xr.setFoveation vs xr.setReferenceSpaceType vs PCFSoftShadowMap. */
export const FRS_FRAMES = REGION_FRAMES;
export const FRS_LAYERS = Object.freeze([
  'lights', 'debug', 'xr', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = FRS_LAYERS;
...
export const FRS_L2_MIN = 8;
export const FRS_LUMA_MIN = 8;
export const FRS_BLUE_MIN = 8;
export const FRS_BAND_MIN = 0.02;
...
export function dominantFrs(layers) {
  if (!Array.isArray(layers) || layers.length !== FRS_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotFrsMismatch(report) {
  if (!report?.frames?.length) return true;
  if (report.protocol !== 'world-frs-buffer-not-original-golden') return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== FRS_LAYERS.length
    || FRS_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !FRS_LAYERS.includes(layer.id)
...
export function frsBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-frs-buffer-not-original-golden'
...
function frame(id, extra = {}) {
  const spec = FRS_FRAMES.find(row => row.id === id);
  ...
  const layers = FRS_LAYERS.map(layerId => layerRow(layerId));
  return {
...
export function worldFrsFixture(extra = {}) {
  return {
    ...
    frsDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-frs-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: FRS_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldFrsResults(r) {
  assert.ok(r && typeof r === 'object', 'world-frs evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-frs frames missing');
  assert.equal(leftoverIsNotFrsMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of xr.setFoveation vs xr.setReferenceSpaceType vs PCFSoftShadowMap mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'frs isolation still combined with hemi');
  assert.equal(r.lightsClears, true, 'type isolation did not force setFoveation 0');
  ...
  assert.equal(r.lightsKeepsDebug, true, 'referenceSpaceType left product during type isolation');
  assert.equal(r.lightsKeepsXr, true, 'shadowMap.type left product during type isolation');
  assert.equal(r.debugClears, true, "needs isolation did not force setReferenceSpaceType('viewer')");
  ...
  assert.equal(r.debugKeepsLights, true, 'foveation left product during needs isolation');
  assert.equal(r.debugKeepsXr, true, 'shadowMap.type left product during needs isolation');
  assert.equal(r.xrClears, true, 'clip isolation did not force PCFSoftShadowMap');
  ...
  assert.equal(r.xrKeepsLights, true, 'foveation left product during clip isolation');
  assert.equal(r.xrKeepsDebug, true, 'referenceSpaceType left product during clip isolation');
  ...
  assert.equal(r.envKeepsLights, true, 'foveation left product during env isolation');
  assert.equal(r.envKeepsDebug, true, 'referenceSpaceType left product during env isolation');
  ...
  assert.equal(r.packClearsLights, true, 'pack isolation did not force setFoveation 0');
  assert.equal(r.packClearsDebug, true, "pack isolation did not force setReferenceSpaceType('viewer')");
  assert.equal(r.packClearsXr, true, 'pack isolation did not force PCFSoftShadowMap');
  ...
  assert.equal(r.bothClearsLights, true, 'both isolation did not force setFoveation 0');
  assert.equal(r.bothClearsDebug, true, "both isolation did not force setReferenceSpaceType('viewer')");
  assert.equal(r.bothClearsXr, true, 'both isolation did not force PCFSoftShadowMap');
  ...
  assert.equal(r.intensityClearsIbl, false, 'frs isolation clears scene.environment via setFoveation');
  assert.equal(r.frsDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after tnc restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during frs probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during frs probe');
  ...
  assert.ok(r.packCount >= 151, 'combined setFoveation-setReferenceSpaceType-PCFSoftShadowMap isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both setFoveation-setReferenceSpaceType-PCFSoftShadowMap+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during frs probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during frs probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during frs probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during frs probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during frs probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during frs probe');
  ...
  assert.equal(frsBufferIsNotOriginalGolden(r), false, 'ldx buffer is not original-golden');
  ...
  for (const spec of FRS_FRAMES) {
    ...
    if (!row.dominantFrs || !row.dominant || row.layers.length !== FRS_LAYERS.length
      || FRS_LAYERS.some(id => !byId[id])
      ...
      || FRS_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of xr.setFoveation vs xr.setReferenceSpaceType vs PCFSoftShadowMap attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present frs axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'xr.setFoveation vs xr.setReferenceSpaceType vs PCFSoftShadowMap independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'frs-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldFrs(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldFrsResults(report);
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const setLights = () => {
          engine.renderer.xr.setFoveation(0);
          return [() => { engine.renderer.xr.setFoveation(1); }];
        };
        const setDebug = () => {
          engine.renderer.xr.setReferenceSpaceType('viewer');
          return [() => { engine.renderer.xr.setReferenceSpaceType('local-floor'); }];
        };
        const setXr = () => {
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = FRS_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantFrs: dominantFrs(layers),
        contributingFrs: layers.filter(layer => layer.bandDelta <= -FRS_BAND_MIN).map(layer => layer.id),
        lightsAxis: layers.filter(layer => LIGHTS_IDS.includes(layer.id) && layer.bandDelta <= -FRS_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -FRS_BAND_MIN || layer.blueDelta <= -FRS_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -FRS_BAND_MIN).map(layer => layer.id),
        debugAxis: layers.filter(layer => DEBUG_IDS.includes(layer.id) && layer.bandDelta <= -FRS_BAND_MIN).map(layer => layer.id),
        xrAxis: layers.filter(layer => XR_IDS.includes(layer.id) && layer.bandDelta <= -FRS_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -FRS_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      frsDeltasUseEmptyBaseline: report.frsDeltasUseEmptyBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-frs-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldFrs(browser, url) {
  const report = await measureWorldFrs(browser, url);
  const out = process.env.WORLD_FRS_OUTPUT ?? fromRoot('artifacts', 'world-frs');
  const results = await retainWorldFrs(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldFrs(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
