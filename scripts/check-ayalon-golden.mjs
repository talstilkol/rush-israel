...
export function validateAyalonGolden(overrides = {}) {
  ...
  if (
    ...
    || manifest.deferred_boundary?.rsh_036_authorized !== true || manifest.deferred_boundary?.rsh_037_authorized !== false
    || manifest.deferred_boundary?.rsh_036_started !== true || manifest.deferred_boundary?.rsh_037_started !== false
  ) {
...
if (isMainModule(import.meta.url)) {
  ...
  console.log(`ayalon-golden ok: ${result.uniqueCount} unique frames; ${result.placeholderCount} placeholders; approved ${result.approved}; RSH-037 deferred`);
}
