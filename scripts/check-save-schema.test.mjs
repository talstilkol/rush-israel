...
test("the RSH-021 schema remains exact beneath the accepted RSH-022 recovery overlay", () => {
  ...
  assert.equal(manifest.deferred_boundary.queue_head, "RSH-026");
  assert.equal(manifest.deferred_boundary.rsh_026_started, false);
  assert.equal(manifest.deferred_boundary.rsh_026_authorized, false);
});
