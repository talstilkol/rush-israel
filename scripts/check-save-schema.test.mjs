...
test("the RSH-021 schema remains exact beneath the accepted RSH-022 recovery overlay", () => {
  ...
  assert.equal(manifest.deferred_boundary.queue_head, "RSH-030");
  ...
  assert.equal(manifest.deferred_boundary.rsh_030_started, false);
  assert.equal(manifest.deferred_boundary.rsh_030_authorized, false);
});
