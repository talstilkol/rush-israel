...
import {
  BAND_PIXELS, BEAM_BLUE_MIN, BEAM_FRAMES, BEAM_L2_MIN, BEAM_LAYERS, BEAM_LUMA_MIN,
  FAILURE_LIMIT, IBL_IDS, LUMA_IDS, PIXEL_THRESHOLD, PRODUCT_EXPOSURE,
  beamBufferIsNotOriginalGolden, dominantBeam, luma, retainWorldBeam,
  termIsNotBeamMismatch, worldBeamFixture, worldBeamResults,
} from './world-beam-browser.mjs';
import { worldTermFixture } from './world-term-browser.mjs';
...
test('dominant 200px term report without sun vs near is not beam mismatch', () => {
  assert.equal(termIsNotBeamMismatch({ frames: [] }), true);
  assert.equal(termIsNotBeamMismatch(worldTermFixture()), true);
  assert.equal(termIsNotBeamMismatch(worldBeamFixture()), false);
  assert.throws(() => worldBeamResults(worldTermFixture()), /combined dir luma is not sun vs near mismatch/);
});

test('beam buffer without original-golden protocol identity is not original-golden', () => {
  assert.equal(beamBufferIsNotOriginalGolden(worldBeamFixture()), false);
  assert.equal(beamBufferIsNotOriginalGolden({
    ...worldBeamFixture(), originalGoldenComparisons: 4, protocol: 'original-golden',
  }), true);
  assert.equal(PIXEL_THRESHOLD, 0.12);
  assert.equal(FAILURE_LIMIT, 0.08);
  assert.equal(BEAM_L2_MIN, 8);
  assert.equal(BEAM_LUMA_MIN, 8);
  assert.equal(BEAM_BLUE_MIN, 8);
  assert.equal(PRODUCT_EXPOSURE, 0.56);
  assert.equal(BAND_PIXELS, 1280 * 200);
  assert.deepEqual(BEAM_LAYERS, ['sun', 'near', 'fill', 'environment']);
  assert.deepEqual([...LUMA_IDS], ['sun', 'near', 'fill']);
  assert.deepEqual([...IBL_IDS], ['environment']);
  assert.equal(GROUND_ALBEDO.hex, 0xd0d4d8);
  assert.equal(luma({ r: 0, g: 0, b: 0 }), 0);
  assert.throws(() => worldBeamResults({ ...worldBeamFixture(), productGroundHex: 0 }), /product ground color retuned/);
  assert.throws(() => worldBeamResults({ ...worldBeamFixture(), productExposure: 1 }), /product exposure retuned/);
  assert.throws(() => worldBeamResults({ ...worldBeamFixture(), lumaTermsIncludeHemi: true }), /luma terms include hemi/);
  assert.throws(() => worldBeamResults({ ...worldBeamFixture(), dirIsCombined: true }), /sun and near still isolated together/);
  assert.throws(() => worldBeamResults({ ...worldBeamFixture(), sunClearsNear: true }), /sun isolation clears near/);
  assert.throws(() => worldBeamResults({ ...worldBeamFixture(), nearClearsSun: true }), /near isolation clears sun/);
});
...
test('locked beam poses match original-golden cameras and PNG hashes', () => {
  assert.deepEqual(BEAM_FRAMES.map(row => row.id), ['g01', 'g05', 'g07', 'g08']);
  assert.deepEqual(BEAM_FRAMES.map(row => row.file), ORIGINAL_GOLDEN_FILES);
  assert.equal(BEAM_FRAMES[3].night, true);
  assert.equal(dominantBeam(worldBeamFixture().frames[0].layers), 'sun');
});

test('beam-attribution evidence yields five protocol passes', () => {
  assert.deepEqual(worldBeamResults(worldBeamFixture()).map(row => row.status), Array(5).fill('passed'));
});
...
]) test(`world-beam evidence fails closed: ${name}`, () => {
  const r = worldBeamFixture();
  mutate(r);
  assert.throws(() => worldBeamResults(r));
});

test('baseline PNG hash drift fails closed', () => {
  const r = worldBeamFixture();
  r.baselineHashes['ayalon-day-g01.png'] = '0'.repeat(64);
  assert.throws(() => worldBeamResults(r), /baseline hash drift/);
});

test('zero full-frame present mismatch remains failed', () => {
  const r = worldBeamFixture();
  r.frames[0] = { ...r.frames[0], presentPct: 0, presentMismatched: 0 };
  const row = worldBeamResults(r).find(item => item.case.includes('still mismatches'));
  assert.equal(row.status, 'failed');
  assert.equal(row.failures, 1);
});

test('a rest-camera miss remains failed during beam sampling', () => {
  const r = worldBeamFixture();
  r.frames[1] = { ...r.frames[1], follow: 9.2, height: 2.28 };
  assert.throws(() => worldBeamResults(r));
});

test('invalid world-beam report is retained before validation throws', async () => {
  const out = await mkdtemp(join(tmpdir(), 'world-beam-invalid-'));
  try {
    const r = worldBeamFixture({ originalGoldenComparisons: 4 });
    await assert.rejects(retainWorldBeam(r, out));
...
test('smoke keeps world-layer residual mismatch bias scene region column slice extra material factor rgb shade tone term and beam probes', () => {
