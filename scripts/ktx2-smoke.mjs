...
if (existsSync("/workspace/public/game/blob.ktx2")) {
  throw new Error("blob.ktx2 must not ship — it was uncompressed RGBA, not UASTC");
}
const ktx = readdirSync("/workspace/public/game").filter((f) => f.endsWith(".ktx2"));
if (ktx.length) throw new Error("unexpected ktx2 " + ktx.join(","));
if (!existsSync("/workspace/public/game/blob.png")) throw new Error("missing blob.png");

const blobSrc = readFileSync("/workspace/src/game/blob-assets.ts", "utf8");
