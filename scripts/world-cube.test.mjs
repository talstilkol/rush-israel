...
import {
  BAND_PIXELS, CONTENT_IDS, CUBE_BLUE_MIN, CUBE_FRAMES, CUBE_L2_MIN, CUBE_LAYERS, CUBE_LUMA_MIN,
  FAILURE_LIMIT, IBL_IDS, LUMA_IDS, PIXEL_THRESHOLD, PRODUCT_EXPOSURE,
  cubeBufferIsNotOriginalGolden, dominantCube, luma, retainWorldCube,
  gainIsNotCubeMismatch, worldCubeFixture, worldCubeResults,
} from './world-cube-browser.mjs';
import { worldGainFixture } from './world-gain-browser.mjs';
...
test('dominant 200px gain report without product cubemap content vs gray is not cube mismatch', () => {
  assert.equal(gainIsNotCubeMismatch({ frames: [] }), true);
  assert.equal(gainIsNotCubeMismatch(worldGainFixture()), true);
  assert.equal(gainIsNotCubeMismatch(worldCubeFixture()), false);
  assert.throws(() => worldCubeResults(worldGainFixture()), /combined IBL is not product cubemap content vs a neutral gray cubemap mismatch/);
});

test('cube buffer without original-golden protocol identity is not original-golden', () => {
  assert.equal(cubeBufferIsNotOriginalGolden(worldCubeFixture()), false);
  assert.equal(cubeBufferIsNotOriginalGolden({
    ...worldCubeFixture(), originalGoldenComparisons: 4, protocol: 'original-golden',
  }), true);
  assert.equal(PIXEL_THRESHOLD, 0.12);
  assert.equal(FAILURE_LIMIT, 0.08);
  assert.equal(CUBE_L2_MIN, 8);
  assert.equal(CUBE_LUMA_MIN, 8);
  assert.equal(CUBE_BLUE_MIN, 8);
  assert.equal(PRODUCT_EXPOSURE, 0.56);
  assert.equal(BAND_PIXELS, 1280 * 200);
  assert.deepEqual(CUBE_LAYERS, ['content', 'ibl', 'intensity', 'fill']);
  assert.deepEqual([...LUMA_IDS], ['intensity', 'fill']);
  assert.deepEqual([...IBL_IDS], ['ibl']);
  assert.deepEqual([...CONTENT_IDS], ['content']);
  assert.equal(GROUND_ALBEDO.hex, 0xd0d4d8);
  assert.equal(luma({ r: 0, g: 0, b: 0 }), 0);
  assert.throws(() => worldCubeResults({ ...worldCubeFixture(), productGroundHex: 0 }), /product ground color retuned/);
  assert.throws(() => worldCubeResults({ ...worldCubeFixture(), productExposure: 1 }), /product exposure retuned/);
  assert.throws(() => worldCubeResults({ ...worldCubeFixture(), lumaTermsIncludeHemi: true }), /luma terms include hemi/);
  assert.throws(() => worldCubeResults({ ...worldCubeFixture(), contentIsCombined: true }), /content isolation still combined/);
  assert.throws(() => worldCubeResults({ ...worldCubeFixture(), contentClearsGain: true }), /content isolation clears environmentIntensity/);
  assert.throws(() => worldCubeResults({ ...worldCubeFixture(), contentClearsIntensity: true }), /content isolation clears intensity/);
  assert.throws(() => worldCubeResults({ ...worldCubeFixture(), iblClearsIntensity: true }), /ibl isolation clears intensity/);
  assert.throws(() => worldCubeResults({ ...worldCubeFixture(), intensityClearsIbl: true }), /intensity isolation clears scene.environment/);
});
...
test('locked cube poses match original-golden cameras and PNG hashes', () => {
  assert.deepEqual(CUBE_FRAMES.map(row => row.id), ['g01', 'g05', 'g07', 'g08']);
  assert.deepEqual(CUBE_FRAMES.map(row => row.file), ORIGINAL_GOLDEN_FILES);
  assert.equal(CUBE_FRAMES[3].night, true);
  assert.equal(dominantCube(worldCubeFixture().frames[0].layers), 'content');
});

test('cube-attribution evidence yields five protocol passes', () => {
  assert.deepEqual(worldCubeResults(worldCubeFixture()).map(row => row.status), Array(5).fill('passed'));
});
...
]) test(`world-cube evidence fails closed: ${name}`, () => {
  const r = worldCubeFixture();
  mutate(r);
  assert.throws(() => worldCubeResults(r));
});

test('baseline PNG hash drift fails closed', () => {
  const r = worldCubeFixture();
  r.baselineHashes['ayalon-day-g01.png'] = '0'.repeat(64);
  assert.throws(() => worldCubeResults(r), /baseline hash drift/);
});

test('zero full-frame present mismatch remains failed', () => {
  const r = worldCubeFixture();
  r.frames[0] = { ...r.frames[0], presentPct: 0, presentMismatched: 0 };
  const row = worldCubeResults(r).find(item => item.case.includes('still mismatches'));
  assert.equal(row.status, 'failed');
  assert.equal(row.failures, 1);
});

test('a rest-camera miss remains failed during cube sampling', () => {
  const r = worldCubeFixture();
  r.frames[1] = { ...r.frames[1], follow: 9.2, height: 2.28 };
  assert.throws(() => worldCubeResults(r));
});

test('invalid world-cube report is retained before validation throws', async () => {
  const out = await mkdtemp(join(tmpdir(), 'world-cube-invalid-'));
  try {
    const r = worldCubeFixture({ originalGoldenComparisons: 4 });
    await assert.rejects(retainWorldCube(r, out));
...
test('smoke keeps world-layer residual mismatch bias scene region column slice extra material factor rgb shade tone term beam ray ibl gain and cube probes', () => {
