...
/** Leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of bloom vs SMAA vs grade. */
export const FX_FRAMES = REGION_FRAMES;
export const FX_LAYERS = Object.freeze([
  'bloom', 'smaa', 'grade', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = FX_LAYERS;
export const BLOOM_IDS = Object.freeze(['bloom']);
export const SMAA_IDS = Object.freeze(['smaa']);
export const GRADE_IDS = Object.freeze(['grade']);
...
export const FX_L2_MIN = 8;
export const FX_LUMA_MIN = 8;
export const FX_BLUE_MIN = 8;
export const FX_BAND_MIN = 0.02;
...
export function dominantFx(layers) {
  if (!Array.isArray(layers) || layers.length !== FX_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotFxMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== FX_LAYERS.length
    || FX_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !FX_LAYERS.includes(layer.id)
    ...
    || !frame.dominantFx
    || !frame.dominant
    || !Array.isArray(frame.bloomAxis)
    || !Array.isArray(frame.envAxis)
    || !Array.isArray(frame.bothAxis)
    || !Array.isArray(frame.smaaAxis)
    || !Array.isArray(frame.gradeAxis)
...
export function fxBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-fx-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { bloom: 2083, smaa: 2083, grade: 2083, env: 2083, both: 2084, pack: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'bloom' ? 18 : id === 'both' ? 20 : id === 'pack' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'bloom' ? -0.10 : id === 'both' ? -0.08 : id === 'pack' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = FX_FRAMES.find(row => row.id === id);
  ...
  const layers = FX_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantFx: 'bloom',
    contributingFx: ['bloom', 'pack'],
    bloomAxis: ['bloom'],
    envAxis: [],
    bothAxis: ['both'],
    smaaAxis: [],
...
export function worldFxFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, bloomCount: 2083, smaaCount: 2083, gradeCount: 2083, envCount: 2083, bothCount: 2086, packCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    bloomSetsWide: true, bloomHidesLeftover: true, bloomKeepsEnv: true, bloomKeepsHemi: true, bloomKeepsSmaa: true, bloomKeepsGrade: true,
    smaaClears: true, smaaHidesLeftover: true, smaaKeepsEnv: true, smaaKeepsHemi: true, smaaKeepsBloom: true, smaaKeepsGrade: true,
    gradeClears: true, gradeHidesLeftover: true, gradeKeepsEnv: true, gradeKeepsHemi: true, gradeKeepsBloom: true, gradeKeepsSmaa: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsBloom: true, envKeepsSmaa: true, envKeepsGrade: true, envClearsHue: false,
    packSetsWide: true, packClearsSmaa: true, packClearsGrade: true, packHidesLeftover: true, packKeepsEnv: true, packKeepsHemi: true,
    bothSetsGray: true, bothSetsWide: true, bothClearsSmaa: true, bothClearsGrade: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    fxDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-fx-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: FX_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldFxResults(r) {
  assert.ok(r && typeof r === 'object', 'world-fx evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-fx frames missing');
  assert.equal(leftoverIsNotFxMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of bloom vs SMAA vs grade mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'fx isolation still combined with hemi');
  assert.equal(r.bloomSetsWide, true, 'bloom isolation did not set strength 0.8');
  assert.equal(r.bloomHidesLeftover, true, 'bloom isolation did not hide leftover empty-scene meshes');
  assert.equal(r.bloomKeepsEnv, true, 'bloom isolation clears scene.environment');
  assert.equal(r.bloomKeepsHemi, true, 'hemi.color left product during bloom isolation');
  assert.equal(r.bloomKeepsSmaa, true, 'SMAA left product during bloom isolation');
  assert.equal(r.bloomKeepsGrade, true, 'grade left product during bloom isolation');
  assert.equal(r.smaaClears, true, 'smaa isolation did not disable SMAA');
  assert.equal(r.smaaHidesLeftover, true, 'buffer isolation did not hide leftover empty-scene meshes');
  assert.equal(r.smaaKeepsEnv, true, 'buffer isolation clears scene.environment');
  assert.equal(r.smaaKeepsHemi, true, 'hemi.color left product during buffer isolation');
  assert.equal(r.smaaKeepsBloom, true, 'bloom left product during smaa isolation');
  assert.equal(r.smaaKeepsGrade, true, 'grade left product during smaa isolation');
  assert.equal(r.gradeClears, true, 'grade isolation did not disable grade');
  assert.equal(r.gradeHidesLeftover, true, 'size isolation did not hide leftover empty-scene meshes');
  assert.equal(r.gradeKeepsEnv, true, 'size isolation clears scene.environment');
  assert.equal(r.gradeKeepsHemi, true, 'hemi.color left product during size isolation');
  assert.equal(r.gradeKeepsBloom, true, 'bloom left product during grade isolation');
  assert.equal(r.gradeKeepsSmaa, true, 'SMAA left product during grade isolation');
  ...
  assert.equal(r.envKeepsBloom, true, 'bloom left product during env isolation');
  assert.equal(r.envKeepsSmaa, true, 'SMAA left product during env isolation');
  assert.equal(r.envKeepsGrade, true, 'grade left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packSetsWide, true, 'pack isolation did not set bloom strength 0.8');
  assert.equal(r.packClearsSmaa, true, 'pack isolation did not disable SMAA');
  assert.equal(r.packClearsGrade, true, 'pack isolation did not disable grade');
  ...
  assert.equal(r.bothSetsWide, true, 'both isolation did not set bloom strength 0.8');
  assert.equal(r.bothClearsSmaa, true, 'both isolation did not disable SMAA');
  assert.equal(r.bothClearsGrade, true, 'both isolation did not disable grade');
  ...
  assert.equal(r.intensityClearsIbl, false, 'fx isolation clears scene.environment via sun');
  assert.equal(r.fxDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after fx restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during fx probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during fx probe');
  ...
  assert.ok(r.bloomCount >= 151, 'bloom isolation missing');
  assert.ok(r.smaaCount >= 151, 'smaa isolation missing');
  assert.ok(r.gradeCount >= 151, 'grade isolation missing');
  assert.ok(r.packCount >= 151, 'combined bloom-smaa-grade isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both bloom-smaa-grade+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during fx probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during fx probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during fx probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during fx probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during fx probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during fx probe');
  ...
  assert.equal(fxBufferIsNotOriginalGolden(r), false, 'fx buffer is not original-golden');
  ...
  for (const spec of FX_FRAMES) {
    ...
    if (!row.dominantFx || !row.dominant || row.layers.length !== FX_LAYERS.length
      || FX_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.bloomAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.smaaAxis) || !Array.isArray(row.gradeAxis) || !Array.isArray(row.packAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || FX_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of bloom vs SMAA vs grade attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present fx axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'bloom vs SMAA vs grade independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'fx-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldFx(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldFxResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldFx(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let bloomSetsWide = false;
        let bloomHidesLeftover = false;
        let bloomKeepsEnv = true;
        let bloomKeepsHemi = true;
        let bloomKeepsSmaa = true;
        let bloomKeepsGrade = true;
        let smaaClears = false;
        let smaaHidesLeftover = false;
        let smaaKeepsEnv = true;
        let smaaKeepsHemi = true;
        let smaaKeepsBloom = true;
        let smaaKeepsGrade = true;
        let gradeClears = false;
        let gradeHidesLeftover = false;
        let gradeKeepsEnv = true;
        let gradeKeepsHemi = true;
        let gradeKeepsBloom = true;
        let gradeKeepsSmaa = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsBloom = true;
        let envKeepsSmaa = true;
        let envKeepsGrade = true;
        let packSetsWide = false;
        let packClearsSmaa = false;
        let packClearsGrade = false;
        let packHidesLeftover = false;
        let packKeepsEnv = true;
        let packKeepsHemi = true;
        let bothSetsGray = false;
        let bothSetsWide = false;
        let bothClearsSmaa = false;
        let bothClearsGrade = false;
        ...
        const setBloom = () => {
          const prevEnabled = engine.post.bloom.enabled;
          const prevStrength = engine.post.bloom.strength;
          engine.post.bloom.enabled = true;
          engine.post.bloom.strength = 0.8;
          return [() => {
            engine.post.bloom.enabled = prevEnabled;
            engine.post.bloom.strength = prevStrength;
          }];
        };
        const setSmaa = () => {
          const smaa = engine.post.smaa;
          const prev = smaa ? smaa.enabled : true;
          if (smaa) smaa.enabled = false;
          return [() => { if (smaa) smaa.enabled = prev; }];
        };
        const setGrade = () => {
          const prev = engine.post.grade.enabled;
          engine.post.grade.enabled = false;
          return [() => { engine.post.grade.enabled = prev; }];
        };
        const isolate = {
          bloom() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevSmaa = engine.post.smaa ? engine.post.smaa.enabled : true;
            const prevGrade = engine.post.grade.enabled;
            const restorers = [...hideMeshes(leftoverOf()), ...setBloom()];
            bloomSetsWide = true;
            if (leftoverHidden()) bloomHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) bloomKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) bloomKeepsHemi = false;
            if (engine.post.smaa && engine.post.smaa.enabled !== prevSmaa) bloomKeepsSmaa = false;
            if (engine.post.grade.enabled !== prevGrade) bloomKeepsGrade = false;
            return restorers;
          },
          smaa() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevBloom = engine.post.bloom.enabled;
            const prevGrade = engine.post.grade.enabled;
            const restorers = [...hideMeshes(leftoverOf()), ...setSmaa()];
            smaaClears = true;
            if (leftoverHidden()) smaaHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) smaaKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) smaaKeepsHemi = false;
            if (engine.post.bloom.enabled !== prevBloom) smaaKeepsBloom = false;
            if (engine.post.grade.enabled !== prevGrade) smaaKeepsGrade = false;
            return restorers;
          },
          grade() {
            const prevHemi = hemiRgb();
            ...
            const prevBloom = engine.post.bloom.enabled;
            const prevSmaa = engine.post.smaa ? engine.post.smaa.enabled : true;
            const restorers = [...hideMeshes(leftoverOf()), ...setGrade()];
            gradeClears = true;
            if (leftoverHidden()) gradeHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) gradeKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) gradeKeepsHemi = false;
            if (engine.post.bloom.enabled !== prevBloom) gradeKeepsBloom = false;
            if (engine.post.smaa && engine.post.smaa.enabled !== prevSmaa) gradeKeepsSmaa = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.post.bloom.enabled !== prevBloom) envKeepsBloom = false;
            if (engine.post.smaa && engine.post.smaa.enabled !== prevSmaa) envKeepsSmaa = false;
            if (engine.post.grade.enabled !== prevGrade) envKeepsGrade = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
            ...
            if (engine.post.bloom.enabled === true) packSetsWide = true;
            packClearsSmaa = true;
            packClearsGrade = true;
        ...
        const bothIsCombined = !bloomKeepsHemi || !smaaKeepsHemi || !gradeKeepsHemi || !envKeepsHemi || !packKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.bloom();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.smaa();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.grade();
      ...
      frames: FX_FRAMES,
      layers: ['bloom', 'smaa', 'grade', 'env', 'both', 'pack'],
    ...
    const frames = report.frames.map(row => {
      ...
      return {
        ...
        layers,
        dominantFx: dominantFx(layers),
        contributingFx: layers.filter(layer => layer.bandDelta <= -FX_BAND_MIN).map(layer => layer.id),
        bloomAxis: layers.filter(layer => BLOOM_IDS.includes(layer.id) && layer.bandDelta <= -FX_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -FX_BAND_MIN || layer.blueDelta <= -FX_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -FX_BAND_MIN).map(layer => layer.id),
        smaaAxis: layers.filter(layer => SMAA_IDS.includes(layer.id) && layer.bandDelta <= -FX_BAND_MIN).map(layer => layer.id),
        gradeAxis: layers.filter(layer => GRADE_IDS.includes(layer.id) && layer.bandDelta <= -FX_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -FX_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      bloomSetsWide: report.bloomSetsWide,
      bloomHidesLeftover: report.bloomHidesLeftover,
      bloomKeepsEnv: report.bloomKeepsEnv,
      bloomKeepsHemi: report.bloomKeepsHemi,
      bloomKeepsSmaa: report.bloomKeepsSmaa,
      bloomKeepsGrade: report.bloomKeepsGrade,
      smaaClears: report.smaaClears,
      smaaHidesLeftover: report.smaaHidesLeftover,
      smaaKeepsEnv: report.smaaKeepsEnv,
      smaaKeepsHemi: report.smaaKeepsHemi,
      smaaKeepsBloom: report.smaaKeepsBloom,
      smaaKeepsGrade: report.smaaKeepsGrade,
      gradeClears: report.gradeClears,
      gradeHidesLeftover: report.gradeHidesLeftover,
      gradeKeepsEnv: report.gradeKeepsEnv,
      gradeKeepsHemi: report.gradeKeepsHemi,
      gradeKeepsBloom: report.gradeKeepsBloom,
      gradeKeepsSmaa: report.gradeKeepsSmaa,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsBloom: report.envKeepsBloom,
      envKeepsSmaa: report.envKeepsSmaa,
      envKeepsGrade: report.envKeepsGrade,
      envClearsHue: report.envClearsHue,
      packSetsWide: report.packSetsWide,
      packClearsSmaa: report.packClearsSmaa,
      packClearsGrade: report.packClearsGrade,
      ...
      bothClearsSmaa: report.bothClearsSmaa,
      bothClearsGrade: report.bothClearsGrade,
      bothHidesLeftover: report.bothHidesLeftover,
      bothKeepsHemi: report.bothKeepsHemi,
      intensityClearsIbl: report.intensityClearsIbl,
      fxDeltasUseEmptyBaseline: report.fxDeltasUseEmptyBaseline,
      ...
      bloomCount: report.bloomCount,
      smaaCount: report.smaaCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-fx-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldFx(browser, url) {
  const report = await measureWorldFx(browser, url);
  const out = process.env.WORLD_FX_OUTPUT ?? fromRoot('artifacts', 'world-fx');
  const results = await retainWorldFx(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldFx(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
