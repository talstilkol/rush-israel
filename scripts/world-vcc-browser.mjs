...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of setViewport vs setClearColor vs renderer.compile. */
export const VCC_FRAMES = REGION_FRAMES;
export const VCC_LAYERS = Object.freeze([
  'lights', 'debug', 'xr', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = VCC_LAYERS;
...
export const VCC_L2_MIN = 8;
export const VCC_LUMA_MIN = 8;
export const VCC_BLUE_MIN = 8;
export const VCC_BAND_MIN = 0.02;
...
export function dominantVcc(layers) {
  if (!Array.isArray(layers) || layers.length !== VCC_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotVccMismatch(report) {
  if (!report?.frames?.length) return true;
  if (report.protocol !== 'world-vcc-buffer-not-original-golden') return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== VCC_LAYERS.length
    || VCC_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !VCC_LAYERS.includes(layer.id)
...
export function vccBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-vcc-buffer-not-original-golden'
...
function frame(id, extra = {}) {
  const spec = VCC_FRAMES.find(row => row.id === id);
  ...
  const layers = VCC_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantVcc: 'lights',
    contributingVcc: ['lights', 'pack'],
    lightsAxis: ['lights'],
    envAxis: [],
    bothAxis: ['both'],
    debugAxis: [],
...
export function worldVccFixture(extra = {}) {
  return {
    ...
    vccDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-vcc-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: VCC_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldVccResults(r) {
  assert.ok(r && typeof r === 'object', 'world-vcc evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-vcc frames missing');
  assert.equal(leftoverIsNotVccMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of setViewport vs setClearColor vs renderer.compile mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'vcc isolation still combined with hemi');
  assert.equal(r.lightsClears, true, 'viewport isolation did not force setViewport 640x400');
  assert.equal(r.lightsHidesLeftover, true, 'viewport isolation did not hide leftover empty-scene meshes');
  assert.equal(r.lightsKeepsEnv, true, 'viewport isolation clears scene.environment');
  assert.equal(r.lightsKeepsHemi, true, 'hemi.color left product during viewport isolation');
  assert.equal(r.lightsKeepsDebug, true, 'clearColor left product during viewport isolation');
  assert.equal(r.lightsKeepsXr, true, 'compile left product during viewport isolation');
  assert.equal(r.debugClears, true, "clearColor isolation did not force setClearColor 0x808080");
  assert.equal(r.debugHidesLeftover, true, 'clearColor isolation did not hide leftover empty-scene meshes');
  assert.equal(r.debugKeepsEnv, true, 'clearColor isolation clears scene.environment');
  assert.equal(r.debugKeepsHemi, true, 'hemi.color left product during clearColor isolation');
  assert.equal(r.debugKeepsLights, true, 'viewport left product during clearColor isolation');
  assert.equal(r.debugKeepsXr, true, 'compile left product during clearColor isolation');
  assert.equal(r.xrClears, true, 'compile isolation did not force renderer.compile');
  assert.equal(r.xrHidesLeftover, true, 'compile isolation did not hide leftover empty-scene meshes');
  assert.equal(r.xrKeepsEnv, true, 'compile isolation clears scene.environment');
  assert.equal(r.xrKeepsHemi, true, 'hemi.color left product during compile isolation');
  assert.equal(r.xrKeepsLights, true, 'viewport left product during compile isolation');
  assert.equal(r.xrKeepsDebug, true, 'clearColor left product during compile isolation');
  ...
  assert.equal(r.packClearsLights, true, 'pack isolation did not force setViewport 640x400');
  assert.equal(r.packClearsDebug, true, "pack isolation did not force setClearColor 0x808080");
  assert.equal(r.packClearsXr, true, 'pack isolation did not force renderer.compile');
  ...
  assert.equal(r.bothClearsLights, true, 'both isolation did not force setViewport 640x400');
  assert.equal(r.bothClearsDebug, true, "both isolation did not force setClearColor 0x808080");
  assert.equal(r.bothClearsXr, true, 'both isolation did not force renderer.compile');
  ...
  assert.equal(r.intensityClearsIbl, false, 'vcc isolation clears scene.environment via setViewport');
  assert.equal(r.vccDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after tnc restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during vcc probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during vcc probe');
  ...
  assert.ok(r.debugCount >= 151, 'clearColor isolation missing');
  assert.ok(r.xrCount >= 151, 'compile isolation missing');
  assert.ok(r.packCount >= 151, 'combined setViewport-setClearColor-renderer.compile isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both setViewport-setClearColor-renderer.compile+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during vcc probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during vcc probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during vcc probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during vcc probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during vcc probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during vcc probe');
  ...
  assert.equal(vccBufferIsNotOriginalGolden(r), false, 'vcc buffer is not original-golden');
  ...
  for (const spec of VCC_FRAMES) {
    ...
    if (!row.dominantVcc || !row.dominant || row.layers.length !== VCC_LAYERS.length
      || VCC_LAYERS.some(id => !byId[id])
      ...
      || VCC_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of setViewport vs setClearColor vs renderer.compile attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present vcc axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'setViewport vs setClearColor vs renderer.compile independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'vcc-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldVcc(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldVccResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldVcc(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const viewportOf = () => {
          ...
          engine.renderer.getViewport(v);
          return { x: v.x, y: v.y, width: v.z, height: v.w };
        };
        const clearHexOf = () => {
          const c = new THREE.Color();
          engine.renderer.getClearColor(c);
          return c.getHex();
        };
        let compileCalled = false;
        const setLights = () => {
          const prev = viewportOf();
          engine.renderer.setViewport(0, 0, 640, 400);
          return [() => { engine.renderer.setViewport(prev.x, prev.y, prev.width, prev.height); }];
        };
        const setDebug = () => {
          const prev = new THREE.Color();
          engine.renderer.getClearColor(prev);
          const prevA = engine.renderer.getClearAlpha();
          engine.renderer.setClearColor(0x808080, prevA);
          return [() => { engine.renderer.setClearColor(prev, prevA); }];
        };
        const setXr = () => {
        ...
        const isolate = {
          lights() {
            ...
            const prevCompile = compileCalled;
            const restorers = [...hideMeshes(leftoverOf()), ...setLights()];
            const vp = viewportOf();
            lightsClears = vp.width === 640 && vp.height === 400;
            ...
            if (clearHexOf() !== prevClear) lightsKeepsDebug = false;
            if (compileCalled !== prevCompile) lightsKeepsXr = false;
            return restorers;
          },
          debug() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevVp = viewportOf();
            const prevCompile = compileCalled;
            const restorers = [...hideMeshes(leftoverOf()), ...setDebug()];
            debugClears = clearHexOf() === 0x808080;
            ...
            const vp = viewportOf();
            if (vp.width !== prevVp.width || vp.height !== prevVp.height) debugKeepsLights = false;
            if (compileCalled !== prevCompile) debugKeepsXr = false;
            return restorers;
          },
          xr() {
            const prevHemi = hemiRgb();
            ...
            const vp = viewportOf();
            if (vp.width !== prevVp.width || vp.height !== prevVp.height) xrKeepsLights = false;
            if (clearHexOf() !== prevClear) xrKeepsDebug = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            const vp = viewportOf();
            if (vp.width !== prevVp.width || vp.height !== prevVp.height) envKeepsLights = false;
            if (clearHexOf() !== prevClear) envKeepsDebug = false;
            if (compileCalled !== prevCompile) envKeepsXr = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
            ...
            const vp = viewportOf();
            packClearsLights = vp.width === 640 && vp.height === 400;
            packClearsDebug = clearHexOf() === 0x808080;
            packClearsXr = compileCalled === true;
          ...
          both() {
            ...
            const vp = viewportOf();
            bothClearsLights = vp.width === 640 && vp.height === 400;
            bothClearsDebug = clearHexOf() === 0x808080;
            bothClearsXr = compileCalled === true;
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = VCC_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantVcc: dominantVcc(layers),
        contributingVcc: layers.filter(layer => layer.bandDelta <= -VCC_BAND_MIN).map(layer => layer.id),
        lightsAxis: layers.filter(layer => LIGHTS_IDS.includes(layer.id) && layer.bandDelta <= -VCC_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -VCC_BAND_MIN || layer.blueDelta <= -VCC_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -VCC_BAND_MIN).map(layer => layer.id),
        debugAxis: layers.filter(layer => DEBUG_IDS.includes(layer.id) && layer.bandDelta <= -VCC_BAND_MIN).map(layer => layer.id),
        xrAxis: layers.filter(layer => XR_IDS.includes(layer.id) && layer.bandDelta <= -VCC_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -VCC_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      vccDeltasUseEmptyBaseline: report.vccDeltasUseEmptyBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-vcc-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldVcc(browser, url) {
  const report = await measureWorldVcc(browser, url);
  const out = process.env.WORLD_VCC_OUTPUT ?? fromRoot('artifacts', 'world-vcc');
  const results = await retainWorldVcc(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldVcc(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
