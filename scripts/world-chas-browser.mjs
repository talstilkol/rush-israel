...
/** Leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of rest-chase follow vs height vs look-ahead. */
export const CHAS_FRAMES = REGION_FRAMES;
export const CHAS_LAYERS = Object.freeze([
  'follow', 'height', 'look', 'env', 'both', 'pose',
]);
export const CAPTURE_LAYERS = CHAS_LAYERS;
export const FOLLOW_IDS = Object.freeze(['follow']);
export const HEIGHT_IDS = Object.freeze(['height']);
export const LOOK_IDS = Object.freeze(['look']);
export const IBL_IDS = Object.freeze(['env']);
export const BOTH_IDS = Object.freeze(['both']);
export const POSE_IDS = Object.freeze(['pose']);
export const PRODUCT_HEMI_SKY_HEX = 0xa8c8e8;
export const PRODUCT_HEMI_NIGHT_HEX = 0x6a88b0;
export const CHAS_L2_MIN = 8;
export const CHAS_LUMA_MIN = 8;
export const CHAS_BLUE_MIN = 8;
export const CHAS_BAND_MIN = 0.02;
...
export const WIDE_FOLLOW = 14;
export const HIGH_HEIGHT = 4;
...
export function dominantChas(layers) {
  if (!Array.isArray(layers) || layers.length !== CHAS_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotChasMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== CHAS_LAYERS.length
    || CHAS_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !CHAS_LAYERS.includes(layer.id)
    ...
    || !frame.dominantChas
    || !frame.dominant
    || !Array.isArray(frame.followAxis)
    || !Array.isArray(frame.envAxis)
    || !Array.isArray(frame.bothAxis)
    || !Array.isArray(frame.heightAxis)
    || !Array.isArray(frame.lookAxis)
    || !Array.isArray(frame.poseAxis)
...
export function chasBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-chas-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { follow: 2083, height: 2083, look: 2083, env: 2083, both: 2084, pose: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'follow' ? 18 : id === 'both' ? 20 : id === 'pose' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'follow' ? -0.10 : id === 'both' ? -0.08 : id === 'pose' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = CHAS_FRAMES.find(row => row.id === id);
  ...
  const layers = CHAS_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantChas: 'follow',
    contributingChas: ['follow', 'pose'],
    followAxis: ['follow'],
    envAxis: [],
    bothAxis: ['both'],
    heightAxis: [],
...
export function worldChasFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, followCount: 2083, heightCount: 2083, lookCount: 2083, envCount: 2083, bothCount: 2086, poseCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    followSetsWide: true, followHidesLeftover: true, followKeepsEnv: true, followKeepsHemi: true, followKeepsHeight: true, followKeepsLook: true,
    heightSetsHigh: true, heightHidesLeftover: true, heightKeepsEnv: true, heightKeepsHemi: true, heightKeepsFollow: true, heightKeepsLook: true,
    lookSetsAhead: true, lookHidesLeftover: true, lookKeepsEnv: true, lookKeepsHemi: true, lookKeepsFollow: true, lookKeepsHeight: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsFollow: true, envKeepsHeight: true, envKeepsLook: true, envClearsHue: false,
    poseSetsWide: true, poseSetsHigh: true, poseSetsAhead: true, poseHidesLeftover: true, poseKeepsEnv: true, poseKeepsHemi: true,
    bothSetsGray: true, bothSetsWide: true, bothSetsHigh: true, bothSetsAhead: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    chasDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-chas-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: CHAS_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldChasResults(r) {
  assert.ok(r && typeof r === 'object', 'world-chas evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-chas frames missing');
  assert.equal(leftoverIsNotChasMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of rest-chase follow vs height vs look-ahead mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'chas isolation still combined with hemi');
  assert.equal(r.followSetsWide, true, 'follow isolation did not set wide follow');
  assert.equal(r.followHidesLeftover, true, 'follow isolation did not hide leftover empty-scene meshes');
  assert.equal(r.followKeepsEnv, true, 'follow isolation clears scene.environment');
  assert.equal(r.followKeepsHemi, true, 'hemi.color left product during follow isolation');
  assert.equal(r.followKeepsHeight, true, 'camera.near left product during follow isolation');
  assert.equal(r.followKeepsLook, true, 'camera.far left product during follow isolation');
  assert.equal(r.heightSetsHigh, true, 'height isolation did not set high height');
  assert.equal(r.heightHidesLeftover, true, 'height isolation did not hide leftover empty-scene meshes');
  assert.equal(r.heightKeepsEnv, true, 'height isolation clears scene.environment');
  assert.equal(r.heightKeepsHemi, true, 'hemi.color left product during height isolation');
  assert.equal(r.heightKeepsFollow, true, 'camera.fov left product during height isolation');
  assert.equal(r.heightKeepsLook, true, 'camera.far left product during height isolation');
  assert.equal(r.lookSetsAhead, true, 'look isolation did not set look-ahead');
  assert.equal(r.lookHidesLeftover, true, 'look isolation did not hide leftover empty-scene meshes');
  assert.equal(r.lookKeepsEnv, true, 'look isolation clears scene.environment');
  assert.equal(r.lookKeepsHemi, true, 'hemi.color left product during look isolation');
  assert.equal(r.lookKeepsFollow, true, 'camera.fov left product during look isolation');
  assert.equal(r.lookKeepsHeight, true, 'camera.near left product during look isolation');
  ...
  assert.equal(r.envKeepsFollow, true, 'camera.fov left product during env isolation');
  assert.equal(r.envKeepsHeight, true, 'camera.near left product during env isolation');
  assert.equal(r.envKeepsLook, true, 'camera.far left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.poseSetsWide, true, 'pose isolation did not set wide follow');
  assert.equal(r.poseSetsHigh, true, 'pose isolation did not set high height');
  assert.equal(r.poseSetsAhead, true, 'pose isolation did not set look-ahead');
  assert.equal(r.poseHidesLeftover, true, 'pose isolation did not hide leftover empty-scene meshes');
  assert.equal(r.poseKeepsEnv, true, 'pose isolation clears scene.environment');
  assert.equal(r.poseKeepsHemi, true, 'hemi.color left product during pose isolation');
  assert.equal(r.bothSetsGray, true, 'both isolation did not swap gray cubemap');
  assert.equal(r.bothSetsWide, true, 'both isolation did not set wide follow');
  assert.equal(r.bothSetsHigh, true, 'both isolation did not set high height');
  assert.equal(r.bothSetsAhead, true, 'both isolation did not set look-ahead');
  ...
  assert.equal(r.intensityClearsIbl, false, 'chas isolation clears scene.environment via sun');
  assert.equal(r.chasDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after chas restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during chas probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during chas probe');
  ...
  assert.ok(r.followCount >= 151, 'camera follow isolation missing');
  assert.ok(r.heightCount >= 151, 'camera height isolation missing');
  assert.ok(r.lookCount >= 151, 'camera look isolation missing');
  assert.ok(r.poseCount >= 151, 'combined chase-pose isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both chase-pose+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during chas probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during chas probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during chas probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during chas probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during chas probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during chas probe');
  ...
  assert.equal(chasBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of CHAS_FRAMES) {
    ...
    if (!row.dominantChas || !row.dominant || row.layers.length !== CHAS_LAYERS.length
      || CHAS_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.followAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.heightAxis) || !Array.isArray(row.lookAxis) || !Array.isArray(row.poseAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || CHAS_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of rest-chase follow vs height vs look-ahead attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present chas axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'rest-chase follow vs height vs look-ahead independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'chas-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldChas(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldChasResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldChas(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let followSetsWide = false;
        let followHidesLeftover = false;
        let followKeepsEnv = true;
        let followKeepsHemi = true;
        let followKeepsHeight = true;
        let followKeepsLook = true;
        let heightSetsHigh = false;
        let heightHidesLeftover = false;
        let heightKeepsEnv = true;
        let heightKeepsHemi = true;
        let heightKeepsFollow = true;
        let heightKeepsLook = true;
        let lookSetsAhead = false;
        let lookHidesLeftover = false;
        let lookKeepsEnv = true;
        let lookKeepsHemi = true;
        let lookKeepsFollow = true;
        let lookKeepsHeight = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsFollow = true;
        let envKeepsHeight = true;
        let envKeepsLook = true;
        let poseSetsWide = false;
        let poseSetsHigh = false;
        let poseSetsAhead = false;
        let poseHidesLeftover = false;
        let poseKeepsEnv = true;
        let poseKeepsHemi = true;
        let bothSetsGray = false;
        let bothSetsWide = false;
        let bothSetsHigh = false;
        let bothSetsAhead = false;
        ...
        const chaseHeight = () => engine.camera.position.y - engine.player.y;
        const setFollow = (dist) => {
          const prev = engine.camera.position.clone();
          const p = engine.player;
          const dx = prev.x - p.x;
          const dz = prev.z - p.z;
          const len = Math.hypot(dx, dz) || 1;
          engine.camera.position.x = p.x + dx * (dist / len);
          engine.camera.position.z = p.z + dz * (dist / len);
          engine.camera.updateMatrixWorld(true);
          return [() => { engine.camera.position.copy(prev); engine.camera.updateMatrixWorld(true); }];
        };
        const setHeight = (h) => {
          const prevY = engine.camera.position.y;
          engine.camera.position.y = engine.player.y + h;
          engine.camera.updateMatrixWorld(true);
          return [() => { engine.camera.position.y = prevY; engine.camera.updateMatrixWorld(true); }];
        };
        const setLook = () => {
          const prevQuat = engine.camera.quaternion.clone();
          const p = engine.player;
          engine.camera.lookAt(p.x, p.y + 20, p.z);
          engine.camera.updateMatrixWorld(true);
          return [() => { engine.camera.quaternion.copy(prevQuat); engine.camera.updateMatrixWorld(true); }];
        };
        const isolate = {
          follow() {
            ...
            const prevY = engine.camera.position.y;
            const prevQuat = engine.camera.quaternion.clone();
            const restorers = [...hideMeshes(leftoverOf()), ...setFollow(14)];
            if (Math.abs(chaseFollow() - 14) <= 0.2) followSetsWide = true;
            if (leftoverHidden()) followHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) followKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) followKeepsHemi = false;
            if (engine.camera.position.y !== prevY) followKeepsHeight = false;
            if (!engine.camera.quaternion.equals(prevQuat)) followKeepsLook = false;
            return restorers;
          },
          height() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevFollow = chaseFollow();
            const prevQuat = engine.camera.quaternion.clone();
            const restorers = [...hideMeshes(leftoverOf()), ...setHeight(4)];
            if (Math.abs(chaseHeight() - 4) <= 0.05) heightSetsHigh = true;
            if (leftoverHidden()) heightHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) heightKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) heightKeepsHemi = false;
            if (Math.abs(chaseFollow() - prevFollow) > 0.05) heightKeepsFollow = false;
            if (!engine.camera.quaternion.equals(prevQuat)) heightKeepsLook = false;
            return restorers;
          },
          look() {
            const prevHemi = hemiRgb();
            ...
            const prevFollow = chaseFollow();
            const prevY = engine.camera.position.y;
            const restorers = [...hideMeshes(leftoverOf()), ...setLook()];
            if (Math.abs(engine.camera.quaternion.x) + Math.abs(engine.camera.quaternion.y) > 0) lookSetsAhead = true;
            if (leftoverHidden()) lookHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) lookKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) lookKeepsHemi = false;
            if (Math.abs(chaseFollow() - prevFollow) > 0.05) lookKeepsFollow = false;
            if (engine.camera.position.y !== prevY) lookKeepsHeight = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (Math.abs(chaseFollow() - prevFollow) > 0.05) envKeepsFollow = false;
            if (engine.camera.position.y !== prevY) envKeepsHeight = false;
            if (!engine.camera.quaternion.equals(prevQuat)) envKeepsLook = false;
            return restorers;
          },
          pose() {
            const prevHemi = hemiRgb();
            ...
            if (Math.abs(chaseFollow() - 14) <= 0.2) poseSetsWide = true;
            if (Math.abs(chaseHeight() - 4) <= 0.05) poseSetsHigh = true;
            if (Math.abs(engine.camera.quaternion.x) + Math.abs(engine.camera.quaternion.y) > 0) poseSetsAhead = true;
            if (leftoverHidden()) poseHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) poseKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) poseKeepsHemi = false;
            return restorers;
          },
          both() {
            const prevHemi = hemiRgb();
            ...
            if (Math.abs(chaseFollow() - 14) <= 0.2) bothSetsWide = true;
            if (Math.abs(chaseHeight() - 4) <= 0.05) bothSetsHigh = true;
            if (Math.abs(engine.camera.quaternion.x) + Math.abs(engine.camera.quaternion.y) > 0) bothSetsAhead = true;
        ...
        const bothIsCombined = !followKeepsHemi || !heightKeepsHemi || !lookKeepsHemi || !envKeepsHemi || !poseKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.follow();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.height();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.look();
        mgmtRestorers.forEach(restore => restore());
        const encRestorers = isolate.pose();
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = CHAS_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantChas: dominantChas(layers),
        contributingChas: layers.filter(layer => layer.bandDelta <= -CHAS_BAND_MIN).map(layer => layer.id),
        followAxis: layers.filter(layer => FOLLOW_IDS.includes(layer.id) && layer.bandDelta <= -CHAS_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -CHAS_BAND_MIN || layer.blueDelta <= -CHAS_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -CHAS_BAND_MIN).map(layer => layer.id),
        heightAxis: layers.filter(layer => HEIGHT_IDS.includes(layer.id) && layer.bandDelta <= -CHAS_BAND_MIN).map(layer => layer.id),
        lookAxis: layers.filter(layer => LOOK_IDS.includes(layer.id) && layer.bandDelta <= -CHAS_BAND_MIN).map(layer => layer.id),
        poseAxis: layers.filter(layer => POSE_IDS.includes(layer.id) && layer.bandDelta <= -CHAS_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      followSetsWide: report.followSetsWide,
      followHidesLeftover: report.followHidesLeftover,
      followKeepsEnv: report.followKeepsEnv,
      followKeepsHemi: report.followKeepsHemi,
      followKeepsHeight: report.followKeepsHeight,
      followKeepsLook: report.followKeepsLook,
      heightSetsHigh: report.heightSetsHigh,
      heightHidesLeftover: report.heightHidesLeftover,
      heightKeepsEnv: report.heightKeepsEnv,
      heightKeepsHemi: report.heightKeepsHemi,
      heightKeepsFollow: report.heightKeepsFollow,
      heightKeepsLook: report.heightKeepsLook,
      lookSetsAhead: report.lookSetsAhead,
      lookHidesLeftover: report.lookHidesLeftover,
      lookKeepsEnv: report.lookKeepsEnv,
      lookKeepsHemi: report.lookKeepsHemi,
      lookKeepsFollow: report.lookKeepsFollow,
      lookKeepsHeight: report.lookKeepsHeight,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsFollow: report.envKeepsFollow,
      envKeepsHeight: report.envKeepsHeight,
      envKeepsLook: report.envKeepsLook,
      envClearsHue: report.envClearsHue,
      poseSetsWide: report.poseSetsWide,
      poseSetsHigh: report.poseSetsHigh,
      poseSetsAhead: report.poseSetsAhead,
      poseHidesLeftover: report.poseHidesLeftover,
      poseKeepsEnv: report.poseKeepsEnv,
      poseKeepsHemi: report.poseKeepsHemi,
      bothSetsGray: report.bothSetsGray,
      bothSetsWide: report.bothSetsWide,
      bothSetsHigh: report.bothSetsHigh,
      bothSetsAhead: report.bothSetsAhead,
      bothHidesLeftover: report.bothHidesLeftover,
      bothKeepsHemi: report.bothKeepsHemi,
      intensityClearsIbl: report.intensityClearsIbl,
      chasDeltasUseEmptyBaseline: report.chasDeltasUseEmptyBaseline,
      ...
      followCount: report.followCount,
      heightCount: report.heightCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-chas-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldChas(browser, url) {
  const report = await measureWorldChas(browser, url);
  const out = process.env.WORLD_CHAS_OUTPUT ?? fromRoot('artifacts', 'world-chas');
  const results = await retainWorldChas(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldChas(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
