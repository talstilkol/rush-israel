...
/** Combined remaining scene.environment with gray hemi is not environment=null vs environmentIntensity=0 vs both attribution. */
export const EINT_FRAMES = REGION_FRAMES;
export const EINT_LAYERS = Object.freeze([
  'env', 'eint', 'both', 'hue', 'intensity', 'fill',
]);
export const CAPTURE_LAYERS = Object.freeze([
  'env', 'eint', 'both', 'hgray', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['env']);
export const EINT_IDS = Object.freeze(['eint']);
export const BOTH_IDS = Object.freeze(['both']);
export const HUE_IDS = Object.freeze(['hue']);
export const HGRAY_BASELINE_IDS = Object.freeze(['env', 'eint', 'both']);
export const PRODUCT_HEMI_SKY_HEX = 0xa8c8e8;
export const PRODUCT_HEMI_NIGHT_HEX = 0x6a88b0;
export const EINT_L2_MIN = 8;
export const EINT_LUMA_MIN = 8;
export const EINT_BLUE_MIN = 8;
...
export function dominantEint(layers) {
  if (!Array.isArray(layers) || layers.length !== EINT_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function envIsNotEintMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== EINT_LAYERS.length
    || EINT_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !EINT_LAYERS.includes(layer.id)
...
export function eintBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-eint-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { env: 1, eint: 1, both: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'env' ? 70 : id === 'eint' ? 68 : id === 'both' ? 65 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'env' ? 110 : id === 'eint' ? 108 : id === 'both' ? 105 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = EINT_FRAMES.find(row => row.id === id);
  ...
  const layers = EINT_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantEint: 'hue',
    contributingEint: ['env', 'eint', 'both', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    envAxis: ['env'],
    eintAxis: ['eint'],
    bothAxis: ['both'],
...
export function worldEintFixture(extra = {}) {
  return {
    ...
    envCount: 1, eintCount: 1, bothCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, envIsCombined: false, envClearsEnv: true,
    envSetsGray: true, envKeepsMaps: true, envKeepsEint: true, envClearsHue: false,
    eintZerosIntensity: true, eintSetsGray: true, eintKeepsEnv: true, eintKeepsMaps: true,
    bothClearsEnv: true, bothZerosIntensity: true, bothSetsGray: true, bothKeepsMaps: true,
    hueClearsOff: false, intensityClearsIbl: false,
    eintDeltasUseHgrayBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-eint-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: EINT_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldEintResults(r) {
  assert.ok(r && typeof r === 'object', 'world-eint evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-eint frames missing');
  assert.equal(envIsNotEintMismatch(r), false, 'combined remaining scene.environment with gray hemi is not environment=null vs environmentIntensity=0 vs both mismatch');
  ...
  assert.equal(r.envClearsEnv, true, 'scene.environment isolation did not clear scene.environment');
  assert.equal(r.envSetsGray, true, 'hemi.color stays product during scene.environment isolation');
  assert.equal(r.envKeepsMaps, true, 'material.envMap left product during scene.environment isolation');
  assert.equal(r.envKeepsEint, true, 'environmentIntensity left product during scene.environment isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses gray cubemap hue');
  assert.equal(r.eintZerosIntensity, true, 'environmentIntensity isolation did not zero environmentIntensity');
  assert.equal(r.eintSetsGray, true, 'hemi.color stays product during environmentIntensity isolation');
  assert.equal(r.eintKeepsEnv, true, 'environmentIntensity isolation clears scene.environment');
  assert.equal(r.eintKeepsMaps, true, 'material.envMap left product during environmentIntensity isolation');
  assert.equal(r.bothClearsEnv, true, 'both isolation did not clear scene.environment');
  assert.equal(r.bothZerosIntensity, true, 'both isolation did not zero environmentIntensity');
  assert.equal(r.bothSetsGray, true, 'hemi.color stays product during both isolation');
  assert.equal(r.bothKeepsMaps, true, 'material.envMap left product during both isolation');
  ...
  assert.equal(r.eintDeltasUseHgrayBaseline, true, 'env vs eint vs both still uses product present as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product during eint probe');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during eint probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during eint probe');
  ...
  assert.ok(r.eintCount >= 1, 'environmentIntensity isolation missing');
  assert.ok(r.bothCount >= 1, 'environment=null plus environmentIntensity=0 isolation missing');
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during eint probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during eint probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during eint probe');
  ...
  assert.equal(eintBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of EINT_FRAMES) {
    ...
    if (!row.dominantEint || !row.dominant || row.layers.length !== EINT_LAYERS.length
      || EINT_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.eintAxis)
      || !Array.isArray(row.bothAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || EINT_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined remaining scene.environment with gray hemi is not environment=null vs environmentIntensity=0 vs both attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present eint axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'environment=null vs environmentIntensity=0 vs both independently of material.envMap vs 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'eint-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldEint(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldEintResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldEint(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let envClearsEnv = false;
        let envSetsGray = false;
        let envKeepsMaps = true;
        let envKeepsEint = true;
        let eintZerosIntensity = false;
        let eintSetsGray = false;
        let eintKeepsEnv = true;
        let eintKeepsMaps = true;
        let bothClearsEnv = false;
        let bothZerosIntensity = false;
        let bothSetsGray = false;
        let bothKeepsMaps = true;
        const bakeGray = () => {
          const tmp = new THREE.Scene();
        ...
        const envMapMaterials = () => {
          const mats = [];
          engine.scene.traverse(object => {
            for (const mat of materialsOf(object)) {
              if (mat && 'envMap' in mat) mats.push(mat);
            }
          });
          return mats;
        };
        const isolate = {
          env() {
            const maps = envMapMaterials().map(mat => mat.envMap);
            const prevEint = engine.scene.environmentIntensity;
            const [restoreEnv] = swapNull();
            const hemiRestorers = grayHemi(() => { envSetsGray = true; });
            if (engine.scene.environment == null) envClearsEnv = true;
            if (engine.scene.environmentIntensity !== prevEint) envKeepsEint = false;
            if (envMapMaterials().some((mat, i) => mat.envMap !== maps[i])) envKeepsMaps = false;
            return [() => { hemiRestorers.forEach(restore => restore()); restoreEnv(); }];
          },
          eint() {
            const maps = envMapMaterials().map(mat => mat.envMap);
            ...
            if (engine.scene.environmentIntensity === 0) eintZerosIntensity = true;
            const hemiRestorers = grayHemi(() => { eintSetsGray = true; });
            if (engine.scene.environment == null) eintKeepsEnv = false;
            if (envMapMaterials().some((mat, i) => mat.envMap !== maps[i])) eintKeepsMaps = false;
            return [() => {
              engine.scene.environmentIntensity = prevEint;
          ...
          both() {
            ...
            const [restoreEnv] = swapNull();
            engine.scene.environmentIntensity = 0;
            if (engine.scene.environmentIntensity === 0) bothZerosIntensity = true;
            const hemiRestorers = grayHemi(() => { bothSetsGray = true; });
            if (engine.scene.environment == null) bothClearsEnv = true;
            if (envMapMaterials().some((mat, i) => mat.envMap !== maps[i])) bothKeepsMaps = false;
            return [() => {
              engine.scene.environmentIntensity = prevEint;
              hemiRestorers.forEach(restore => restore());
              restoreEnv();
        ...
        const eintRestorers = isolate.eint();
        eintRestorers.forEach(restore => restore());
    ...
    const frames = report.frames.map(row => {
      ...
      const hgrayLive = decodeDataUrlPng(layerPngs.hgray);
      const hgrayBand = bandPixelmatch(hgrayLive, goldPngs[row.id], dominantSpec);
      const hgraySample = sampleRgb(hgrayLive, goldPngs[row.id], dominantSpec);
      const hgrayLuma = luma(hgraySample.live);
      const layers = EINT_LAYERS.map(id => {
        ...
        const compareL2 = HGRAY_BASELINE_IDS.includes(id) ? hgraySample.l2 : baselineSample.l2;
        const compareLuma = HGRAY_BASELINE_IDS.includes(id) ? hgrayLuma : baselineLuma;
        const compareBlue = HGRAY_BASELINE_IDS.includes(id) ? hgraySample.live.b : baselineSample.live.b;
        const comparePct = HGRAY_BASELINE_IDS.includes(id) ? Number(hgrayBand.pct.toFixed(4)) : Number(baselineBand.pct.toFixed(4));
      ...
      return {
        ...
        layers,
        dominantEint: dominantEint(layers),
        contributingEint: layers.filter(layer => layer.l2Delta <= -EINT_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -EINT_LUMA_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -EINT_BLUE_MIN).map(layer => layer.id),
        eintAxis: layers.filter(layer => EINT_IDS.includes(layer.id) && layer.blueDelta <= -EINT_BLUE_MIN).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && (layer.l2Delta <= -EINT_L2_MIN || layer.blueDelta <= -EINT_BLUE_MIN)).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -EINT_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      envClearsEnv: report.envClearsEnv,
      envSetsGray: report.envSetsGray,
      envKeepsMaps: report.envKeepsMaps,
      envKeepsEint: report.envKeepsEint,
      envClearsHue: report.envClearsHue,
      eintZerosIntensity: report.eintZerosIntensity,
      eintSetsGray: report.eintSetsGray,
      eintKeepsEnv: report.eintKeepsEnv,
      eintKeepsMaps: report.eintKeepsMaps,
      bothClearsEnv: report.bothClearsEnv,
      bothZerosIntensity: report.bothZerosIntensity,
      bothSetsGray: report.bothSetsGray,
      bothKeepsMaps: report.bothKeepsMaps,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      eintDeltasUseHgrayBaseline: report.eintDeltasUseHgrayBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-eint-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldEint(browser, url) {
  const report = await measureWorldEint(browser, url);
  const out = process.env.WORLD_EINT_OUTPUT ?? fromRoot('artifacts', 'world-eint');
  const results = await retainWorldEint(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldEint(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
