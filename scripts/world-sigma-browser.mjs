...
/** Combined leftover fromScene 0x808080 is not sigma vs mesh sky vs bake color space attribution. */
export const SIGMA_FRAMES = REGION_FRAMES;
export const SIGMA_LAYERS = Object.freeze([
  'sigma', 'mesh', 'space', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['sigma']);
export const MESH_IDS = Object.freeze(['mesh']);
export const SPACE_IDS = Object.freeze(['space']);
export const HUE_IDS = Object.freeze(['hue']);
export const GRAY_BASELINE_IDS = Object.freeze(['sigma', 'mesh', 'space']);
export const SIGMA_L2_MIN = 8;
export const SIGMA_LUMA_MIN = 8;
export const SIGMA_BLUE_MIN = 8;
...
export function dominantSigma(layers) {
  if (!Array.isArray(layers) || layers.length !== SIGMA_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function convIsNotSigmaMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== SIGMA_LAYERS.length
    || SIGMA_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !SIGMA_LAYERS.includes(layer.id)
...
export function sigmaBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-sigma-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { sigma: 1, mesh: 1, space: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'space' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'space' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = SIGMA_FRAMES.find(row => row.id === id);
  ...
  const layers = SIGMA_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantSigma: 'hue',
    contributingSigma: ['space', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: [],
    meshAxis: [],
    spaceAxis: ['space'],
...
export function worldSigmaFixture(extra = {}) {
  return {
    ...
    sigmaCount: 1, meshCount: 1, spaceCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, sigmaIsCombined: false, sigmaClearsOff: false,
    sigmaClearsHue: false, meshClearsSigma: false, hueClearsOff: false, intensityClearsIbl: false,
    sigmaDeltasUseGrayBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-sigma-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: SIGMA_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldSigmaResults(r) {
  assert.ok(r && typeof r === 'object', 'world-sigma evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-sigma frames missing');
  assert.equal(convIsNotSigmaMismatch(r), false, 'combined leftover fromScene 0x808080 is not sigma vs mesh sky vs bake color space mismatch');
  ...
  assert.equal(r.sigmaIsCombined, false, 'sigma isolation still combined with hemi or disc');
  assert.equal(r.sigmaClearsOff, false, 'sigma isolation clears IBL-off');
  assert.equal(r.sigmaClearsHue, false, 'sigma isolation clears gray cubemap hue');
  assert.equal(r.meshClearsSigma, false, 'mesh isolation uses Color background');
  ...
  assert.equal(r.sigmaDeltasUseGrayBaseline, true, 'sigma vs mesh vs space still uses product present as baseline');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.sigmaCount >= 1, 'fromScene sigma 0 missing');
  assert.ok(r.meshCount >= 1, 'mesh sky fromScene missing');
  assert.ok(r.spaceCount >= 1, 'linear bake color space missing');
  ...
  assert.equal(sigmaBufferIsNotOriginalGolden(r), false, 'sigma buffer is not original-golden');
  ...
  for (const spec of SIGMA_FRAMES) {
    ...
    if (!row.dominantSigma || !row.dominant || row.layers.length !== SIGMA_LAYERS.length
      || SIGMA_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.meshAxis)
      || !Array.isArray(row.spaceAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || SIGMA_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover fromScene 0x808080 is not sigma vs mesh sky vs bake color space attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present sigma axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'fromScene sigma vs mesh sky vs linear bake independently of 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'sigma-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldSigma(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldSigmaResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldSigma(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const isolate = {
          sigma() {
            return swapEnv(bakeVariant({ skyHex: 0x808080, hemi: true, disc: true, sigma: 0 }));
          },
          mesh() {
            const tmp = new THREE.Scene();
          ...
          space() {
            const tmp = new THREE.Scene();
            tmp.background = new THREE.Color(0x808080).convertSRGBToLinear();
            const pmrem = new THREE.PMREMGenerator(engine.renderer);
            try {
              const env = pmrem.fromScene(tmp, 0.04);
              owned.push(env);
        ...
        const sigmaProbe = bakeVariant({ skyHex: 0x808080, hemi: true, disc: true, sigma: 0 });
        const sigmaRestorers = isolate.sigma();
        const sigmaIsCombined = sigmaRestorers.length !== 1;
        const sigmaClearsOff = engine.scene.environment == null;
        const sigmaClearsHue = engine.scene.environment === hueProbe.texture;
        sigmaRestorers.forEach(restore => restore());
        const meshRestorers = isolate.mesh();
        const meshClearsSigma = engine.scene.environment == null
          || engine.scene.environment === sigmaProbe.texture;
        meshRestorers.forEach(restore => restore());
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = SIGMA_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantSigma: dominantSigma(layers),
        contributingSigma: layers.filter(layer => layer.l2Delta <= -SIGMA_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -SIGMA_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -SIGMA_BLUE_MIN).map(layer => layer.id),
        meshAxis: layers.filter(layer => MESH_IDS.includes(layer.id) && (layer.l2Delta <= -SIGMA_L2_MIN || layer.blueDelta <= -SIGMA_BLUE_MIN)).map(layer => layer.id),
        spaceAxis: layers.filter(layer => SPACE_IDS.includes(layer.id) && layer.blueDelta <= -SIGMA_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -SIGMA_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      sigmaIsCombined: report.sigmaIsCombined,
      sigmaClearsOff: report.sigmaClearsOff,
      sigmaClearsHue: report.sigmaClearsHue,
      meshClearsSigma: report.meshClearsSigma,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      sigmaDeltasUseGrayBaseline: report.sigmaDeltasUseGrayBaseline,
      sigmaCount: report.sigmaCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-sigma-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldSigma(browser, url) {
  const report = await measureWorldSigma(browser, url);
  const out = process.env.WORLD_SIGMA_OUTPUT ?? fromRoot('artifacts', 'world-sigma');
  const results = await retainWorldSigma(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldSigma(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
