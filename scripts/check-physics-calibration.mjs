...
export function validatePhysicsCalibration(overrides = {}) {
  ...
  if (
    manifest.deferred_boundary?.queue_head !== "RSH-034"
    || manifest.deferred_boundary?.rsh_034_authorized !== false
    || manifest.deferred_boundary?.rsh_034_started !== false
  ) {
    errors.push("RSH-034 deferred boundary changed");
  }
...
if (isMainModule(import.meta.url)) {
  ...
  console.log(`physics-calibration ok: PHYSICS_VERSION ${result.physicsVersion}; claims ${result.claims.join("/")}; RSH-034 deferred`);
}
