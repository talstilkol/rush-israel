...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of preserveDrawingBuffer vs stencil vs reversedDepthBuffer. */
export const PSR_FRAMES = REGION_FRAMES;
export const PSR_LAYERS = Object.freeze([
  'preserve', 'stencil', 'reversed', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = PSR_LAYERS;
export const PRESERVE_IDS = Object.freeze(['preserve']);
export const STENCIL_IDS = Object.freeze(['stencil']);
export const REVERSED_IDS = Object.freeze(['reversed']);
...
export const PSR_L2_MIN = 8;
export const PSR_LUMA_MIN = 8;
export const PSR_BLUE_MIN = 8;
export const PSR_BAND_MIN = 0.02;
...
export const PRODUCT_PRESERVE = false;
export const PRODUCT_STENCIL = false;
export const PRODUCT_REVERSED = false;
...
export function dominantPsr(layers) {
  if (!Array.isArray(layers) || layers.length !== PSR_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotPsrMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== PSR_LAYERS.length
    || PSR_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !PSR_LAYERS.includes(layer.id)
    ...
    || !frame.dominantPsr
    || !frame.dominant
    || !Array.isArray(frame.preserveAxis)
    || !Array.isArray(frame.envAxis)
    || !Array.isArray(frame.bothAxis)
    || !Array.isArray(frame.stencilAxis)
    || !Array.isArray(frame.reversedAxis)
...
export function psrBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-psr-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { preserve: 2083, stencil: 2083, reversed: 2083, env: 2083, both: 2084, pack: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'preserve' ? 18 : id === 'both' ? 20 : id === 'pack' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'preserve' ? -0.10 : id === 'both' ? -0.08 : id === 'pack' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = PSR_FRAMES.find(row => row.id === id);
  ...
  const layers = PSR_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantPsr: 'preserve',
    contributingPsr: ['preserve', 'pack'],
    preserveAxis: ['preserve'],
    envAxis: [],
    bothAxis: ['both'],
    stencilAxis: [],
    reversedAxis: [],
...
export function worldPsrFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, preserveCount: 2083, stencilCount: 2083, reversedCount: 2083, envCount: 2083, bothCount: 2086, packCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    preserveClears: true, preserveHidesLeftover: true, preserveKeepsEnv: true, preserveKeepsHemi: true, preserveKeepsStencil: true, preserveKeepsReversed: true,
    stencilClears: true, stencilHidesLeftover: true, stencilKeepsEnv: true, stencilKeepsHemi: true, stencilKeepsPreserve: true, stencilKeepsReversed: true,
    reversedClears: true, reversedHidesLeftover: true, reversedKeepsEnv: true, reversedKeepsHemi: true, reversedKeepsPreserve: true, reversedKeepsStencil: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsPreserve: true, envKeepsStencil: true, envKeepsReversed: true, envClearsHue: false,
    packClearsPreserve: true, packClearsStencil: true, packClearsReversed: true, packHidesLeftover: true, packKeepsEnv: true, packKeepsHemi: true,
    bothSetsGray: true, bothClearsPreserve: true, bothClearsStencil: true, bothClearsReversed: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    psrDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-psr-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: PSR_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldPsrResults(r) {
  assert.ok(r && typeof r === 'object', 'world-psr evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-psr frames missing');
  assert.equal(leftoverIsNotPsrMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of preserveDrawingBuffer vs stencil vs reversedDepthBuffer mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'psr isolation still combined with hemi');
  assert.equal(r.preserveClears, true, 'preserve isolation did not force preserveDrawingBuffer on');
  assert.equal(r.preserveHidesLeftover, true, 'preserve isolation did not hide leftover empty-scene meshes');
  assert.equal(r.preserveKeepsEnv, true, 'preserve isolation clears scene.environment');
  assert.equal(r.preserveKeepsHemi, true, 'hemi.color left product during preserve isolation');
  assert.equal(r.preserveKeepsStencil, true, 'stencil left product during preserve isolation');
  assert.equal(r.preserveKeepsReversed, true, 'reversed left product during preserve isolation');
  assert.equal(r.stencilClears, true, 'stencil isolation did not force stencil on');
  assert.equal(r.stencilHidesLeftover, true, 'stencil isolation did not hide leftover empty-scene meshes');
  assert.equal(r.stencilKeepsEnv, true, 'stencil isolation clears scene.environment');
  assert.equal(r.stencilKeepsHemi, true, 'hemi.color left product during stencil isolation');
  assert.equal(r.stencilKeepsPreserve, true, 'preserve left product during stencil isolation');
  assert.equal(r.stencilKeepsReversed, true, 'reversed left product during stencil isolation');
  assert.equal(r.reversedClears, true, 'reversed isolation did not force reversedDepthBuffer on');
  assert.equal(r.reversedHidesLeftover, true, 'reversed isolation did not hide leftover empty-scene meshes');
  assert.equal(r.reversedKeepsEnv, true, 'reversed isolation clears scene.environment');
  assert.equal(r.reversedKeepsHemi, true, 'hemi.color left product during reversed isolation');
  assert.equal(r.reversedKeepsPreserve, true, 'preserve left product during reversed isolation');
  assert.equal(r.reversedKeepsStencil, true, 'stencil left product during reversed isolation');
  ...
  assert.equal(r.envKeepsPreserve, true, 'preserve left product during env isolation');
  assert.equal(r.envKeepsStencil, true, 'stencil left product during env isolation');
  assert.equal(r.envKeepsReversed, true, 'reversed left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsPreserve, true, 'pack isolation did not force preserveDrawingBuffer on');
  assert.equal(r.packClearsStencil, true, 'pack isolation did not force stencil on');
  assert.equal(r.packClearsReversed, true, 'pack isolation did not force reversedDepthBuffer on');
  ...
  assert.equal(r.bothClearsPreserve, true, 'both isolation did not force preserveDrawingBuffer on');
  assert.equal(r.bothClearsStencil, true, 'both isolation did not force stencil on');
  assert.equal(r.bothClearsReversed, true, 'both isolation did not force reversedDepthBuffer on');
  ...
  assert.equal(r.intensityClearsIbl, false, 'psr isolation clears scene.environment via sun');
  assert.equal(r.psrDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after psr restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during psr probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during psr probe');
  ...
  assert.ok(r.preserveCount >= 151, 'auto isolation missing');
  assert.ok(r.stencilCount >= 151, 'sort isolation missing');
  assert.ok(r.reversedCount >= 151, 'override isolation missing');
  assert.ok(r.packCount >= 151, 'combined preserveDrawingBuffer-stencil-reversedDepthBuffer isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both preserveDrawingBuffer-stencil-reversedDepthBuffer+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during psr probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during psr probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during psr probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during psr probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during psr probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during psr probe');
  ...
  assert.equal(psrBufferIsNotOriginalGolden(r), false, 'psr buffer is not original-golden');
  ...
  for (const spec of PSR_FRAMES) {
    ...
    if (!row.dominantPsr || !row.dominant || row.layers.length !== PSR_LAYERS.length
      || PSR_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.preserveAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.stencilAxis) || !Array.isArray(row.reversedAxis) || !Array.isArray(row.packAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || PSR_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of preserveDrawingBuffer vs stencil vs reversedDepthBuffer attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present psr axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'preserveDrawingBuffer vs stencil vs reversedDepthBuffer independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'psr-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldPsr(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldPsrResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldPsr(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let preserveClears = false;
        let preserveHidesLeftover = false;
        let preserveKeepsEnv = true;
        let preserveKeepsHemi = true;
        let preserveKeepsStencil = true;
        let preserveKeepsReversed = true;
        let stencilClears = false;
        let stencilHidesLeftover = false;
        let stencilKeepsEnv = true;
        let stencilKeepsHemi = true;
        let stencilKeepsPreserve = true;
        let stencilKeepsReversed = true;
        let reversedClears = false;
        let reversedHidesLeftover = false;
        let reversedKeepsEnv = true;
        let reversedKeepsHemi = true;
        let reversedKeepsPreserve = true;
        let reversedKeepsStencil = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsPreserve = true;
        let envKeepsStencil = true;
        let envKeepsReversed = true;
        let packClearsPreserve = false;
        let packClearsStencil = false;
        let packClearsReversed = false;
        let packHidesLeftover = false;
        let packKeepsEnv = true;
        let packKeepsHemi = true;
        let bothSetsGray = false;
        let bothClearsPreserve = false;
        let bothClearsStencil = false;
        let bothClearsReversed = false;
        ...
        const setPreserve = () => {
          const prev = engine.renderer.preserveDrawingBuffer;
          engine.renderer.preserveDrawingBuffer = true;
          return [() => { engine.renderer.preserveDrawingBuffer = prev; }];
        };
        const setStencil = () => {
          const prev = engine.renderer.stencil;
          engine.renderer.stencil = true;
          return [() => { engine.renderer.stencil = prev; }];
        };
        const setReversed = () => {
          const cap = engine.renderer.capabilities;
          const prev = cap.reversedDepthBuffer;
          cap.reversedDepthBuffer = true;
          return [() => { cap.reversedDepthBuffer = prev; }];
        };
        const isolate = {
          preserve() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevStencil = engine.renderer.stencil;
            const prevReversed = engine.renderer.capabilities.reversedDepthBuffer;
            const restorers = [...hideMeshes(leftoverOf()), ...setPreserve()];
            preserveClears = true;
            if (leftoverHidden()) preserveHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) preserveKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) preserveKeepsHemi = false;
            if (engine.renderer.stencil !== prevStencil) preserveKeepsStencil = false;
            if (engine.renderer.capabilities.reversedDepthBuffer !== prevReversed) preserveKeepsReversed = false;
            return restorers;
          },
          stencil() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevPreserve = engine.renderer.preserveDrawingBuffer;
            const prevReversed = engine.renderer.capabilities.reversedDepthBuffer;
            const restorers = [...hideMeshes(leftoverOf()), ...setStencil()];
            stencilClears = true;
            if (leftoverHidden()) stencilHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) stencilKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) stencilKeepsHemi = false;
            if (engine.renderer.preserveDrawingBuffer !== prevPreserve) stencilKeepsPreserve = false;
            if (engine.renderer.capabilities.reversedDepthBuffer !== prevReversed) stencilKeepsReversed = false;
            return restorers;
          },
          reversed() {
            const prevHemi = hemiRgb();
            ...
            const prevPreserve = engine.renderer.preserveDrawingBuffer;
            const prevStencil = engine.renderer.stencil;
            const restorers = [...hideMeshes(leftoverOf()), ...setReversed()];
            reversedClears = true;
            if (leftoverHidden()) reversedHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) reversedKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) reversedKeepsHemi = false;
            if (engine.renderer.preserveDrawingBuffer !== prevPreserve) reversedKeepsPreserve = false;
            if (engine.renderer.stencil !== prevStencil) reversedKeepsStencil = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.preserveDrawingBuffer !== prevPreserve) envKeepsPreserve = false;
            if (engine.renderer.stencil !== prevStencil) envKeepsStencil = false;
            if (engine.renderer.capabilities.reversedDepthBuffer !== prevReversed) envKeepsReversed = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
            ...
            packClearsPreserve = true;
            packClearsStencil = true;
            packClearsReversed = true;
          ...
          both() {
            ...
            bothClearsPreserve = true;
            bothClearsStencil = true;
            bothClearsReversed = true;
        ...
        const bothIsCombined = !preserveKeepsHemi || !stencilKeepsHemi || !reversedKeepsHemi || !envKeepsHemi || !packKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.preserve();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.stencil();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.reversed();
        ...
        return {
          ...
          bothIsCombined,
          preserveClears,
          preserveHidesLeftover,
          preserveKeepsEnv,
          preserveKeepsHemi,
          preserveKeepsStencil,
          preserveKeepsReversed,
          stencilClears,
          stencilHidesLeftover,
          stencilKeepsEnv,
          stencilKeepsHemi,
          stencilKeepsPreserve,
          stencilKeepsReversed,
          reversedClears,
          reversedHidesLeftover,
          reversedKeepsEnv,
          reversedKeepsHemi,
          reversedKeepsPreserve,
          reversedKeepsStencil,
          envSetsGray,
          envHidesLeftover,
          envKeepsHemi,
          envKeepsPreserve,
      ...
      frames: PSR_FRAMES,
      layers: ['preserve', 'stencil', 'reversed', 'env', 'both', 'pack'],
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = PSR_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantPsr: dominantPsr(layers),
        contributingPsr: layers.filter(layer => layer.bandDelta <= -PSR_BAND_MIN).map(layer => layer.id),
        preserveAxis: layers.filter(layer => PRESERVE_IDS.includes(layer.id) && layer.bandDelta <= -PSR_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -PSR_BAND_MIN || layer.blueDelta <= -PSR_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -PSR_BAND_MIN).map(layer => layer.id),
        stencilAxis: layers.filter(layer => STENCIL_IDS.includes(layer.id) && layer.bandDelta <= -PSR_BAND_MIN).map(layer => layer.id),
        reversedAxis: layers.filter(layer => REVERSED_IDS.includes(layer.id) && layer.bandDelta <= -PSR_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -PSR_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      preserveClears: report.preserveClears,
      preserveHidesLeftover: report.preserveHidesLeftover,
      preserveKeepsEnv: report.preserveKeepsEnv,
      preserveKeepsHemi: report.preserveKeepsHemi,
      preserveKeepsStencil: report.preserveKeepsStencil,
      preserveKeepsReversed: report.preserveKeepsReversed,
      stencilClears: report.stencilClears,
      stencilHidesLeftover: report.stencilHidesLeftover,
      stencilKeepsEnv: report.stencilKeepsEnv,
      stencilKeepsHemi: report.stencilKeepsHemi,
      stencilKeepsPreserve: report.stencilKeepsPreserve,
      stencilKeepsReversed: report.stencilKeepsReversed,
      reversedClears: report.reversedClears,
      reversedHidesLeftover: report.reversedHidesLeftover,
      reversedKeepsEnv: report.reversedKeepsEnv,
      reversedKeepsHemi: report.reversedKeepsHemi,
      reversedKeepsPreserve: report.reversedKeepsPreserve,
      reversedKeepsStencil: report.reversedKeepsStencil,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsPreserve: report.envKeepsPreserve,
      envKeepsStencil: report.envKeepsStencil,
      envKeepsReversed: report.envKeepsReversed,
      envClearsHue: report.envClearsHue,
      packClearsPreserve: report.packClearsPreserve,
      packClearsStencil: report.packClearsStencil,
      packClearsReversed: report.packClearsReversed,
      ...
      bothClearsPreserve: report.bothClearsPreserve,
      bothClearsStencil: report.bothClearsStencil,
      bothClearsReversed: report.bothClearsReversed,
      bothHidesLeftover: report.bothHidesLeftover,
      bothKeepsHemi: report.bothKeepsHemi,
      intensityClearsIbl: report.intensityClearsIbl,
      psrDeltasUseEmptyBaseline: report.psrDeltasUseEmptyBaseline,
      ...
      preserveCount: report.preserveCount,
      stencilCount: report.stencilCount,
      reversedCount: report.reversedCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-psr-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldPsr(browser, url) {
  const report = await measureWorldPsr(browser, url);
  const out = process.env.WORLD_PSR_OUTPUT ?? fromRoot('artifacts', 'world-psr');
  const results = await retainWorldPsr(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldPsr(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
