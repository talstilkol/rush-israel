...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of dithering vs shadowMap.autoUpdate vs info.autoReset. */
export const DSA_FRAMES = REGION_FRAMES;
export const DSA_LAYERS = Object.freeze([
  'dither', 'shadow', 'reset', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = DSA_LAYERS;
export const DITHER_IDS = Object.freeze(['dither']);
export const SHADOW_IDS = Object.freeze(['shadow']);
export const RESET_IDS = Object.freeze(['reset']);
...
export const DSA_L2_MIN = 8;
export const DSA_LUMA_MIN = 8;
export const DSA_BLUE_MIN = 8;
export const DSA_BAND_MIN = 0.02;
...
export const PRODUCT_DITHER = false;
export const PRODUCT_SHADOW = true;
export const PRODUCT_RESET = true;
...
export function dominantDsa(layers) {
  if (!Array.isArray(layers) || layers.length !== DSA_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotDsaMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== DSA_LAYERS.length
    || DSA_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !DSA_LAYERS.includes(layer.id)
...
export function dsaBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-dsa-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { dither: 2083, shadow: 2083, reset: 2083, env: 2083, both: 2084, pack: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'dither' ? 18 : id === 'both' ? 20 : id === 'pack' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'dither' ? -0.10 : id === 'both' ? -0.08 : id === 'pack' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = DSA_FRAMES.find(row => row.id === id);
  ...
  const layers = DSA_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantDsa: 'dither',
    contributingDsa: ['dither', 'pack'],
    ditherAxis: ['dither'],
    envAxis: [],
    bothAxis: ['both'],
    shadowAxis: [],
...
export function worldDsaFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, ditherCount: 2083, shadowCount: 2083, resetCount: 2083, envCount: 2083, bothCount: 2086, packCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    ditherClears: true, ditherHidesLeftover: true, ditherKeepsEnv: true, ditherKeepsHemi: true, ditherKeepsShadow: true, ditherKeepsReset: true,
    shadowClears: true, shadowHidesLeftover: true, shadowKeepsEnv: true, shadowKeepsHemi: true, shadowKeepsDither: true, shadowKeepsReset: true,
    resetClears: true, resetHidesLeftover: true, resetKeepsEnv: true, resetKeepsHemi: true, resetKeepsDither: true, resetKeepsShadow: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsDither: true, envKeepsShadow: true, envKeepsReset: true, envClearsHue: false,
    packClearsDither: true, packClearsShadow: true, packClearsReset: true, packHidesLeftover: true, packKeepsEnv: true, packKeepsHemi: true,
    bothSetsGray: true, bothClearsDither: true, bothClearsShadow: true, bothClearsReset: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    dsaDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-dsa-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: DSA_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldDsaResults(r) {
  assert.ok(r && typeof r === 'object', 'world-dsa evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-dsa frames missing');
  assert.equal(leftoverIsNotDsaMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of dithering vs shadowMap.autoUpdate vs info.autoReset mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'dsa isolation still combined with hemi');
  assert.equal(r.ditherClears, true, 'dither isolation did not force dithering on');
  assert.equal(r.ditherHidesLeftover, true, 'dither isolation did not hide leftover empty-scene meshes');
  assert.equal(r.ditherKeepsEnv, true, 'dither isolation clears scene.environment');
  assert.equal(r.ditherKeepsHemi, true, 'hemi.color left product during dither isolation');
  assert.equal(r.ditherKeepsShadow, true, 'shadowMap.autoUpdate left product during dither isolation');
  assert.equal(r.ditherKeepsReset, true, 'info.autoReset left product during dither isolation');
  assert.equal(r.shadowClears, true, 'shadow isolation did not force shadowMap.autoUpdate off');
  ...
  assert.equal(r.shadowKeepsDither, true, 'dithering left product during shadow isolation');
  assert.equal(r.shadowKeepsReset, true, 'info.autoReset left product during shadow isolation');
  assert.equal(r.resetClears, true, 'reset isolation did not force info.autoReset off');
  assert.equal(r.resetHidesLeftover, true, 'reset isolation did not hide leftover empty-scene meshes');
  assert.equal(r.resetKeepsEnv, true, 'reset isolation clears scene.environment');
  assert.equal(r.resetKeepsHemi, true, 'hemi.color left product during reset isolation');
  assert.equal(r.resetKeepsDither, true, 'dithering left product during reset isolation');
  assert.equal(r.resetKeepsShadow, true, 'shadowMap.autoUpdate left product during reset isolation');
  ...
  assert.equal(r.envKeepsDither, true, 'dithering left product during env isolation');
  assert.equal(r.envKeepsShadow, true, 'shadowMap.autoUpdate left product during env isolation');
  assert.equal(r.envKeepsReset, true, 'info.autoReset left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsDither, true, 'pack isolation did not force dithering on');
  assert.equal(r.packClearsShadow, true, 'pack isolation did not force shadowMap.autoUpdate off');
  assert.equal(r.packClearsReset, true, 'pack isolation did not force info.autoReset off');
  ...
  assert.equal(r.bothClearsDither, true, 'both isolation did not force dithering on');
  assert.equal(r.bothClearsShadow, true, 'both isolation did not force shadowMap.autoUpdate off');
  assert.equal(r.bothClearsReset, true, 'both isolation did not force info.autoReset off');
  ...
  assert.equal(r.intensityClearsIbl, false, 'dsa isolation clears scene.environment via sun');
  assert.equal(r.dsaDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after dsa restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during dsa probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during dsa probe');
  ...
  assert.ok(r.ditherCount >= 151, 'dither isolation missing');
  assert.ok(r.shadowCount >= 151, 'shadow isolation missing');
  assert.ok(r.resetCount >= 151, 'reset isolation missing');
  assert.ok(r.packCount >= 151, 'combined dithering-shadowMap.autoUpdate-info.autoReset isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both dithering-shadowMap.autoUpdate-info.autoReset+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during dsa probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during dsa probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during dsa probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during dsa probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during dsa probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during dsa probe');
  ...
  assert.equal(dsaBufferIsNotOriginalGolden(r), false, 'dsa buffer is not original-golden');
  ...
  for (const spec of DSA_FRAMES) {
    ...
    if (!row.dominantDsa || !row.dominant || row.layers.length !== DSA_LAYERS.length
      || DSA_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.ditherAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.shadowAxis) || !Array.isArray(row.resetAxis) || !Array.isArray(row.packAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || DSA_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of dithering vs shadowMap.autoUpdate vs info.autoReset attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present dsa axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'dithering vs shadowMap.autoUpdate vs info.autoReset independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'dsa-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldDsa(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldDsaResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldDsa(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let ditherClears = false;
        let ditherHidesLeftover = false;
        let ditherKeepsEnv = true;
        let ditherKeepsHemi = true;
        let ditherKeepsShadow = true;
        let ditherKeepsReset = true;
        let shadowClears = false;
        let shadowHidesLeftover = false;
        let shadowKeepsEnv = true;
        let shadowKeepsHemi = true;
        let shadowKeepsDither = true;
        let shadowKeepsReset = true;
        let resetClears = false;
        let resetHidesLeftover = false;
        let resetKeepsEnv = true;
        let resetKeepsHemi = true;
        let resetKeepsDither = true;
        let resetKeepsShadow = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsDither = true;
        let envKeepsShadow = true;
        let envKeepsReset = true;
        let packClearsDither = false;
        let packClearsShadow = false;
        let packClearsReset = false;
        let packHidesLeftover = false;
        let packKeepsEnv = true;
        let packKeepsHemi = true;
        let bothSetsGray = false;
        let bothClearsDither = false;
        let bothClearsShadow = false;
        let bothClearsReset = false;
        ...
        const setDither = () => {
          const prev = engine.renderer.dithering;
          engine.renderer.dithering = true;
          return [() => { engine.renderer.dithering = prev; }];
        };
        const setShadow = () => {
          const prev = engine.renderer.shadowMap.autoUpdate;
          engine.renderer.shadowMap.autoUpdate = false;
          return [() => { engine.renderer.shadowMap.autoUpdate = prev; }];
        };
        const setReset = () => {
          const prev = engine.renderer.info.autoReset;
          engine.renderer.info.autoReset = false;
          return [() => { engine.renderer.info.autoReset = prev; }];
        };
        const isolate = {
          dither() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevShadow = engine.renderer.shadowMap.autoUpdate;
            const prevReset = engine.renderer.info.autoReset;
            const restorers = [...hideMeshes(leftoverOf()), ...setDither()];
            ditherClears = true;
            if (leftoverHidden()) ditherHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) ditherKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) ditherKeepsHemi = false;
            if (engine.renderer.shadowMap.autoUpdate !== prevShadow) ditherKeepsShadow = false;
            if (engine.renderer.info.autoReset !== prevReset) ditherKeepsReset = false;
            return restorers;
          },
          shadow() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevDither = engine.renderer.dithering;
            const prevReset = engine.renderer.info.autoReset;
            ...
            if (engine.renderer.dithering !== prevDither) shadowKeepsDither = false;
            if (engine.renderer.info.autoReset !== prevReset) shadowKeepsReset = false;
            return restorers;
          },
          reset() {
            const prevHemi = hemiRgb();
            ...
            const prevDither = engine.renderer.dithering;
            const prevShadow = engine.renderer.shadowMap.autoUpdate;
            const restorers = [...hideMeshes(leftoverOf()), ...setReset()];
            resetClears = true;
            if (leftoverHidden()) resetHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) resetKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) resetKeepsHemi = false;
            if (engine.renderer.dithering !== prevDither) resetKeepsDither = false;
            if (engine.renderer.shadowMap.autoUpdate !== prevShadow) resetKeepsShadow = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.dithering !== prevDither) envKeepsDither = false;
            if (engine.renderer.shadowMap.autoUpdate !== prevShadow) envKeepsShadow = false;
            if (engine.renderer.info.autoReset !== prevReset) envKeepsReset = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
            ...
            packClearsDither = true;
            packClearsShadow = true;
            packClearsReset = true;
          ...
          both() {
            ...
            bothClearsDither = true;
            bothClearsShadow = true;
            bothClearsReset = true;
        ...
        const bothIsCombined = !ditherKeepsHemi || !shadowKeepsHemi || !resetKeepsHemi || !envKeepsHemi || !packKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.dither();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.shadow();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.reset();
      ...
      frames: DSA_FRAMES,
      layers: ['dither', 'shadow', 'reset', 'env', 'both', 'pack'],
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = DSA_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantDsa: dominantDsa(layers),
        contributingDsa: layers.filter(layer => layer.bandDelta <= -DSA_BAND_MIN).map(layer => layer.id),
        ditherAxis: layers.filter(layer => DITHER_IDS.includes(layer.id) && layer.bandDelta <= -DSA_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -DSA_BAND_MIN || layer.blueDelta <= -DSA_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -DSA_BAND_MIN).map(layer => layer.id),
        shadowAxis: layers.filter(layer => SHADOW_IDS.includes(layer.id) && layer.bandDelta <= -DSA_BAND_MIN).map(layer => layer.id),
        resetAxis: layers.filter(layer => RESET_IDS.includes(layer.id) && layer.bandDelta <= -DSA_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -DSA_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      ditherClears: report.ditherClears,
      ditherHidesLeftover: report.ditherHidesLeftover,
      ditherKeepsEnv: report.ditherKeepsEnv,
      ditherKeepsHemi: report.ditherKeepsHemi,
      ditherKeepsShadow: report.ditherKeepsShadow,
      ditherKeepsReset: report.ditherKeepsReset,
      ...
      shadowKeepsDither: report.shadowKeepsDither,
      shadowKeepsReset: report.shadowKeepsReset,
      resetClears: report.resetClears,
      resetHidesLeftover: report.resetHidesLeftover,
      resetKeepsEnv: report.resetKeepsEnv,
      resetKeepsHemi: report.resetKeepsHemi,
      resetKeepsDither: report.resetKeepsDither,
      resetKeepsShadow: report.resetKeepsShadow,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsDither: report.envKeepsDither,
      envKeepsShadow: report.envKeepsShadow,
      envKeepsReset: report.envKeepsReset,
      envClearsHue: report.envClearsHue,
      packClearsDither: report.packClearsDither,
      packClearsShadow: report.packClearsShadow,
      packClearsReset: report.packClearsReset,
      ...
      ditherCount: report.ditherCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-dsa-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldDsa(browser, url) {
  const report = await measureWorldDsa(browser, url);
  const out = process.env.WORLD_DSA_OUTPUT ?? fromRoot('artifacts', 'world-dsa');
  const results = await retainWorldDsa(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldDsa(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
