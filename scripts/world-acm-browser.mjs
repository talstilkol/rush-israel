...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of autoClear vs sortObjects vs overrideMaterial. */
export const ACM_FRAMES = REGION_FRAMES;
export const ACM_LAYERS = Object.freeze([
  'auto', 'sort', 'override', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = ACM_LAYERS;
export const AUTO_IDS = Object.freeze(['auto']);
export const SORT_IDS = Object.freeze(['sort']);
export const OVERRIDE_IDS = Object.freeze(['override']);
...
export const ACM_L2_MIN = 8;
export const ACM_LUMA_MIN = 8;
export const ACM_BLUE_MIN = 8;
export const ACM_BAND_MIN = 0.02;
...
export const PRODUCT_AUTOCLEAR = true;
export const PRODUCT_SORT = true;
export const PRODUCT_OVERRIDE = false;
...
export function dominantAcm(layers) {
  if (!Array.isArray(layers) || layers.length !== ACM_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotAcmMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== ACM_LAYERS.length
    || ACM_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !ACM_LAYERS.includes(layer.id)
    ...
    || !frame.dominantAcm
    || !frame.dominant
    || !Array.isArray(frame.autoAxis)
    || !Array.isArray(frame.envAxis)
    || !Array.isArray(frame.bothAxis)
    || !Array.isArray(frame.sortAxis)
    || !Array.isArray(frame.overrideAxis)
...
export function acmBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-acm-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { auto: 2083, sort: 2083, override: 2083, env: 2083, both: 2084, pack: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'auto' ? 18 : id === 'both' ? 20 : id === 'pack' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'auto' ? -0.10 : id === 'both' ? -0.08 : id === 'pack' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = ACM_FRAMES.find(row => row.id === id);
  ...
  const layers = ACM_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantAcm: 'auto',
    contributingAcm: ['auto', 'pack'],
    autoAxis: ['auto'],
    envAxis: [],
    bothAxis: ['both'],
    sortAxis: [],
...
export function worldAcmFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, autoCount: 2083, sortCount: 2083, overrideCount: 2083, envCount: 2083, bothCount: 2086, packCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    autoClears: true, autoHidesLeftover: true, autoKeepsEnv: true, autoKeepsHemi: true, autoKeepsSort: true, autoKeepsOverride: true,
    sortClears: true, sortHidesLeftover: true, sortKeepsEnv: true, sortKeepsHemi: true, sortKeepsAuto: true, sortKeepsOverride: true,
    overrideClears: true, overrideHidesLeftover: true, overrideKeepsEnv: true, overrideKeepsHemi: true, overrideKeepsAuto: true, overrideKeepsSort: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsAuto: true, envKeepsSort: true, envKeepsOverride: true, envClearsHue: false,
    packClearsAuto: true, packClearsSort: true, packClearsOverride: true, packHidesLeftover: true, packKeepsEnv: true, packKeepsHemi: true,
    bothSetsGray: true, bothClearsAuto: true, bothClearsSort: true, bothClearsOverride: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    acmDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-acm-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: ACM_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldAcmResults(r) {
  assert.ok(r && typeof r === 'object', 'world-acm evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-acm frames missing');
  assert.equal(leftoverIsNotAcmMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of autoClear vs sortObjects vs overrideMaterial mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'acm isolation still combined with hemi');
  assert.equal(r.autoClears, true, 'auto isolation did not force autoClear off');
  assert.equal(r.autoHidesLeftover, true, 'auto isolation did not hide leftover empty-scene meshes');
  assert.equal(r.autoKeepsEnv, true, 'auto isolation clears scene.environment');
  assert.equal(r.autoKeepsHemi, true, 'hemi.color left product during auto isolation');
  assert.equal(r.autoKeepsSort, true, 'sort left product during auto isolation');
  assert.equal(r.autoKeepsOverride, true, 'override left product during auto isolation');
  assert.equal(r.sortClears, true, 'sort isolation did not force sortObjects off');
  assert.equal(r.sortHidesLeftover, true, 'sort isolation did not hide leftover empty-scene meshes');
  assert.equal(r.sortKeepsEnv, true, 'sort isolation clears scene.environment');
  assert.equal(r.sortKeepsHemi, true, 'hemi.color left product during sort isolation');
  assert.equal(r.sortKeepsAuto, true, 'auto left product during sort isolation');
  assert.equal(r.sortKeepsOverride, true, 'override left product during sort isolation');
  assert.equal(r.overrideClears, true, 'override isolation did not set gray overrideMaterial');
  assert.equal(r.overrideHidesLeftover, true, 'override isolation did not hide leftover empty-scene meshes');
  assert.equal(r.overrideKeepsEnv, true, 'override isolation clears scene.environment');
  assert.equal(r.overrideKeepsHemi, true, 'hemi.color left product during override isolation');
  assert.equal(r.overrideKeepsAuto, true, 'auto left product during override isolation');
  assert.equal(r.overrideKeepsSort, true, 'sort left product during override isolation');
  ...
  assert.equal(r.envKeepsAuto, true, 'auto left product during env isolation');
  assert.equal(r.envKeepsSort, true, 'sort left product during env isolation');
  assert.equal(r.envKeepsOverride, true, 'override left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsAuto, true, 'pack isolation did not force autoClear off');
  assert.equal(r.packClearsSort, true, 'pack isolation did not force sortObjects off');
  assert.equal(r.packClearsOverride, true, 'pack isolation did not set gray overrideMaterial');
  ...
  assert.equal(r.bothClearsAuto, true, 'both isolation did not force autoClear off');
  assert.equal(r.bothClearsSort, true, 'both isolation did not force sortObjects off');
  assert.equal(r.bothClearsOverride, true, 'both isolation did not set gray overrideMaterial');
  ...
  assert.equal(r.intensityClearsIbl, false, 'acm isolation clears scene.environment via sun');
  assert.equal(r.acmDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after acm restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during acm probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during acm probe');
  ...
  assert.ok(r.autoCount >= 151, 'auto isolation missing');
  assert.ok(r.sortCount >= 151, 'sort isolation missing');
  assert.ok(r.overrideCount >= 151, 'override isolation missing');
  assert.ok(r.packCount >= 151, 'combined autoClear-sortObjects-overrideMaterial isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both autoClear-sortObjects-overrideMaterial+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during acm probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during acm probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during acm probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during acm probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during acm probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during acm probe');
  ...
  assert.equal(acmBufferIsNotOriginalGolden(r), false, 'acm buffer is not original-golden');
  ...
  for (const spec of ACM_FRAMES) {
    ...
    if (!row.dominantAcm || !row.dominant || row.layers.length !== ACM_LAYERS.length
      || ACM_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.autoAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.sortAxis) || !Array.isArray(row.overrideAxis) || !Array.isArray(row.packAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || ACM_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of autoClear vs sortObjects vs overrideMaterial attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present acm axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'autoClear vs sortObjects vs overrideMaterial independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'acm-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldAcm(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldAcmResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldAcm(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let autoClears = false;
        let autoHidesLeftover = false;
        let autoKeepsEnv = true;
        let autoKeepsHemi = true;
        let autoKeepsSort = true;
        let autoKeepsOverride = true;
        let sortClears = false;
        let sortHidesLeftover = false;
        let sortKeepsEnv = true;
        let sortKeepsHemi = true;
        let sortKeepsAuto = true;
        let sortKeepsOverride = true;
        let overrideClears = false;
        let overrideHidesLeftover = false;
        let overrideKeepsEnv = true;
        let overrideKeepsHemi = true;
        let overrideKeepsAuto = true;
        let overrideKeepsSort = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsAuto = true;
        let envKeepsSort = true;
        let envKeepsOverride = true;
        let packClearsAuto = false;
        let packClearsSort = false;
        let packClearsOverride = false;
        let packHidesLeftover = false;
        let packKeepsEnv = true;
        let packKeepsHemi = true;
        let bothSetsGray = false;
        let bothClearsAuto = false;
        let bothClearsSort = false;
        let bothClearsOverride = false;
        ...
        const setAuto = () => {
          const prev = engine.renderer.autoClear;
          engine.renderer.autoClear = false;
          return [() => { engine.renderer.autoClear = prev; }];
        };
        const setSort = () => {
          const prev = engine.renderer.sortObjects;
          engine.renderer.sortObjects = false;
          return [() => { engine.renderer.sortObjects = prev; }];
        };
        const setOverride = () => {
          const prev = engine.scene.overrideMaterial;
          const mat = new THREE.MeshBasicMaterial({ color: 0x808080 });
          engine.scene.overrideMaterial = mat;
          return [() => {
            engine.scene.overrideMaterial = prev;
            mat.dispose();
          }];
        };
        const isolate = {
          auto() {
            ...
            const prevSort = engine.renderer.sortObjects;
            const prevOverride = engine.scene.overrideMaterial;
            const restorers = [...hideMeshes(leftoverOf()), ...setAuto()];
            autoClears = true;
            if (leftoverHidden()) autoHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) autoKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) autoKeepsHemi = false;
            if (engine.renderer.sortObjects !== prevSort) autoKeepsSort = false;
            if (engine.scene.overrideMaterial !== prevOverride) autoKeepsOverride = false;
            return restorers;
          },
          sort() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevAuto = engine.renderer.autoClear;
            const prevOverride = engine.scene.overrideMaterial;
            const restorers = [...hideMeshes(leftoverOf()), ...setSort()];
            sortClears = true;
            if (leftoverHidden()) sortHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) sortKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) sortKeepsHemi = false;
            if (engine.renderer.autoClear !== prevAuto) sortKeepsAuto = false;
            if (engine.scene.overrideMaterial !== prevOverride) sortKeepsOverride = false;
            return restorers;
          },
          override() {
            const prevHemi = hemiRgb();
            ...
            const prevAuto = engine.renderer.autoClear;
            const prevSort = engine.renderer.sortObjects;
            const restorers = [...hideMeshes(leftoverOf()), ...setOverride()];
            overrideClears = true;
            if (leftoverHidden()) overrideHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) overrideKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) overrideKeepsHemi = false;
            if (engine.renderer.autoClear !== prevAuto) overrideKeepsAuto = false;
            if (engine.renderer.sortObjects !== prevSort) overrideKeepsSort = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.autoClear !== prevAuto) envKeepsAuto = false;
            if (engine.renderer.sortObjects !== prevSort) envKeepsSort = false;
            if (engine.scene.overrideMaterial !== prevOverride) envKeepsOverride = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
        ...
        const bothIsCombined = !autoKeepsHemi || !sortKeepsHemi || !overrideKeepsHemi || !envKeepsHemi || !packKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.auto();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.sort();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.override();
      ...
      frames: ACM_FRAMES,
      layers: ['auto', 'sort', 'override', 'env', 'both', 'pack'],
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = ACM_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantAcm: dominantAcm(layers),
        contributingAcm: layers.filter(layer => layer.bandDelta <= -ACM_BAND_MIN).map(layer => layer.id),
        autoAxis: layers.filter(layer => AUTO_IDS.includes(layer.id) && layer.bandDelta <= -ACM_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -ACM_BAND_MIN || layer.blueDelta <= -ACM_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -ACM_BAND_MIN).map(layer => layer.id),
        sortAxis: layers.filter(layer => SORT_IDS.includes(layer.id) && layer.bandDelta <= -ACM_BAND_MIN).map(layer => layer.id),
        overrideAxis: layers.filter(layer => OVERRIDE_IDS.includes(layer.id) && layer.bandDelta <= -ACM_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -ACM_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      autoClears: report.autoClears,
      autoHidesLeftover: report.autoHidesLeftover,
      autoKeepsEnv: report.autoKeepsEnv,
      autoKeepsHemi: report.autoKeepsHemi,
      autoKeepsSort: report.autoKeepsSort,
      autoKeepsOverride: report.autoKeepsOverride,
      sortClears: report.sortClears,
      sortHidesLeftover: report.sortHidesLeftover,
      sortKeepsEnv: report.sortKeepsEnv,
      sortKeepsHemi: report.sortKeepsHemi,
      sortKeepsAuto: report.sortKeepsAuto,
      sortKeepsOverride: report.sortKeepsOverride,
      overrideClears: report.overrideClears,
      overrideHidesLeftover: report.overrideHidesLeftover,
      overrideKeepsEnv: report.overrideKeepsEnv,
      overrideKeepsHemi: report.overrideKeepsHemi,
      overrideKeepsAuto: report.overrideKeepsAuto,
      overrideKeepsSort: report.overrideKeepsSort,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsAuto: report.envKeepsAuto,
      envKeepsSort: report.envKeepsSort,
      envKeepsOverride: report.envKeepsOverride,
      envClearsHue: report.envClearsHue,
      packClearsAuto: report.packClearsAuto,
      packClearsSort: report.packClearsSort,
      packClearsOverride: report.packClearsOverride,
      ...
      bothClearsAuto: report.bothClearsAuto,
      bothClearsSort: report.bothClearsSort,
      bothClearsOverride: report.bothClearsOverride,
      bothHidesLeftover: report.bothHidesLeftover,
      bothKeepsHemi: report.bothKeepsHemi,
      intensityClearsIbl: report.intensityClearsIbl,
      acmDeltasUseEmptyBaseline: report.acmDeltasUseEmptyBaseline,
      ...
      autoCount: report.autoCount,
      sortCount: report.sortCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-acm-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldAcm(browser, url) {
  const report = await measureWorldAcm(browser, url);
  const out = process.env.WORLD_ACM_OUTPUT ?? fromRoot('artifacts', 'world-acm');
  const results = await retainWorldAcm(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldAcm(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
