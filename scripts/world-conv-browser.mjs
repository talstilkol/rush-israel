...
/** Combined leftover PMREM-filtered 0x808080 is not fromScene vs fromCubemap vs environmentIntensity attribution. */
export const CONV_FRAMES = REGION_FRAMES;
export const CONV_LAYERS = Object.freeze([
  'scene', 'cube', 'gain', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['scene']);
export const CUBE_IDS = Object.freeze(['cube']);
export const GAIN_IDS = Object.freeze(['gain']);
export const HUE_IDS = Object.freeze(['hue']);
export const GRAY_BASELINE_IDS = Object.freeze(['scene', 'cube', 'gain']);
export const CONV_L2_MIN = 8;
export const CONV_LUMA_MIN = 8;
export const CONV_BLUE_MIN = 8;
...
export function dominantConv(layers) {
  if (!Array.isArray(layers) || layers.length !== CONV_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function flatIsNotConvMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== CONV_LAYERS.length
    || CONV_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !CONV_LAYERS.includes(layer.id)
    ...
    || !Array.isArray(frame.cubeAxis)
    || !Array.isArray(frame.gainAxis)
...
export function convBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-conv-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { scene: 1, cube: 1, gain: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'gain' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'gain' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = CONV_FRAMES.find(row => row.id === id);
  ...
  const layers = CONV_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantConv: 'hue',
    contributingConv: ['gain', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: [],
    cubeAxis: [],
    gainAxis: ['gain'],
...
export function worldConvFixture(extra = {}) {
  return {
    ...
    sceneCount: 1, cubeCount: 1, gainCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, sceneIsCombined: false, sceneClearsOff: false,
    sceneClearsHue: false, cubeClearsScene: false, hueClearsOff: false, intensityClearsIbl: false,
    sceneDeltasUseGrayBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-conv-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: CONV_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldConvResults(r) {
  assert.ok(r && typeof r === 'object', 'world-conv evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-conv frames missing');
  assert.equal(flatIsNotConvMismatch(r), false, 'combined leftover PMREM-filtered 0x808080 is not fromScene vs fromCubemap vs environmentIntensity mismatch');
  ...
  assert.equal(r.sceneIsCombined, false, 'scene isolation still combined with hemi or disc');
  assert.equal(r.sceneClearsOff, false, 'scene isolation clears IBL-off');
  assert.equal(r.sceneClearsHue, false, 'scene isolation clears gray cubemap hue');
  assert.equal(r.cubeClearsScene, false, 'cube isolation uses unfiltered CubeTexture');
  ...
  assert.equal(r.sceneDeltasUseGrayBaseline, true, 'scene vs cube vs gain still uses product present as baseline');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.sceneCount >= 1, 'fromScene 0x808080 PMREM missing');
  assert.ok(r.cubeCount >= 1, 'fromCubemap gray PMREM missing');
  assert.ok(r.gainCount >= 1, 'environmentIntensity missing');
  ...
  assert.equal(convBufferIsNotOriginalGolden(r), false, 'conv buffer is not original-golden');
  ...
  for (const spec of CONV_FRAMES) {
    ...
    if (!row.dominantConv || !row.dominant || row.layers.length !== CONV_LAYERS.length
      || CONV_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.cubeAxis)
      || !Array.isArray(row.gainAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || CONV_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover PMREM-filtered 0x808080 is not fromScene vs fromCubemap vs environmentIntensity attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present conv axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'fromScene vs fromCubemap vs environmentIntensity independently of 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'conv-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldConv(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldConvResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldConv(browser, url) {
  ...
  try {
      ...
      try {
        ...
        const isolate = {
          ...
          cube() {
            const pmrem = new THREE.PMREMGenerator(engine.renderer);
            try {
              const env = pmrem.fromCubemap(flatCube(0x808080));
              owned.push(env);
              return swapEnv(env);
            } finally {
              pmrem.dispose();
          ...
          gain() {
            const intensity = engine.scene.environmentIntensity;
            engine.scene.environmentIntensity = 0;
            return [() => { engine.scene.environmentIntensity = intensity; }];
          },
          hue() {
        ...
        const sceneRestorers = isolate.scene();
        const sceneIsCombined = sceneRestorers.length !== 1;
        const sceneClearsOff = engine.scene.environment == null;
        const sceneClearsHue = engine.scene.environment === hueProbe.texture;
        sceneRestorers.forEach(restore => restore());
        const cubeRestorers = isolate.cube();
        const cubeClearsScene = !engine.scene.environment || !!engine.scene.environment.isCubeTexture;
        cubeRestorers.forEach(restore => restore());
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = CONV_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantConv: dominantConv(layers),
        contributingConv: layers.filter(layer => layer.l2Delta <= -CONV_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -CONV_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -CONV_BLUE_MIN).map(layer => layer.id),
        cubeAxis: layers.filter(layer => CUBE_IDS.includes(layer.id) && (layer.l2Delta <= -CONV_L2_MIN || layer.blueDelta <= -CONV_BLUE_MIN)).map(layer => layer.id),
        gainAxis: layers.filter(layer => GAIN_IDS.includes(layer.id) && layer.blueDelta <= -CONV_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -CONV_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      sceneIsCombined: report.sceneIsCombined,
      sceneClearsOff: report.sceneClearsOff,
      sceneClearsHue: report.sceneClearsHue,
      cubeClearsScene: report.cubeClearsScene,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      sceneDeltasUseGrayBaseline: report.sceneDeltasUseGrayBaseline,
      sceneCount: report.sceneCount,
      cubeCount: report.cubeCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-conv-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldConv(browser, url) {
  const report = await measureWorldConv(browser, url);
  const out = process.env.WORLD_CONV_OUTPUT ?? fromRoot('artifacts', 'world-conv');
  const results = await retainWorldConv(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldConv(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
