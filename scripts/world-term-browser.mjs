...
/** Combined unlit luma / envmap B is not dir/fill/ambient vs environment/envIntensity. */
export const TERM_FRAMES = REGION_FRAMES;
export const TERM_LAYERS = Object.freeze([
  'dir', 'fill', 'ambient', 'environment', 'envIntensity',
]);
export const LUMA_IDS = Object.freeze(['dir', 'fill', 'ambient']);
export const IBL_IDS = Object.freeze(['environment', 'envIntensity']);
export const TERM_L2_MIN = 8;
export const TERM_LUMA_MIN = 8;
export const TERM_BLUE_MIN = 8;
...
export function dominantTerm(layers) {
  if (!Array.isArray(layers) || layers.length !== TERM_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function toneIsNotTermMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== TERM_LAYERS.length
    || TERM_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !TERM_LAYERS.includes(layer.id)
...
export function termBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-term-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { dir: 2, fill: 1, ambient: 1, environment: 1, envIntensity: 57 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'dir' ? 50 : id === 'fill' ? 90 : id === 'ambient' ? 100
    : id === 'envIntensity' ? 70 : 85;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'dir' ? 30 : 80) : 90;
  const blueVal = IBL_IDS.includes(id) ? (id === 'envIntensity' ? 70 : 90) : 120;
  return {
...
function frame(id, extra = {}) {
  const spec = TERM_FRAMES.find(row => row.id === id);
  ...
  const layers = TERM_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantTerm: 'dir',
    contributingTerm: ['dir', 'envIntensity', 'environment'],
    lumaAxis: ['dir'],
    iblAxis: ['environment', 'envIntensity'],
    ...extra,
  };
}

export function worldTermFixture(extra = {}) {
  return {
    ...
    dirCount: 2, fillCount: 1, ambientCount: 1, environmentCount: 1, envIntensityCount: 57,
    lumaTermsIncludeHemi: false, environmentClearsIntensity: false, envIntensityClearsEnvironment: false,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-term-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: TERM_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldTermResults(r) {
  assert.ok(r && typeof r === 'object', 'world-term evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-term frames missing');
  assert.equal(toneIsNotTermMismatch(r), false, 'combined unlit luma / envmap B is not dir/fill/ambient vs environment/envIntensity mismatch');
  ...
  assert.equal(r.environmentClearsIntensity, false, 'environment isolation clears envMapIntensity');
  assert.equal(r.envIntensityClearsEnvironment, false, 'envIntensity isolation clears scene.environment');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.dirCount >= 2, 'dir lights missing');
  ...
  assert.ok(r.envIntensityCount >= 1, 'envMapIntensity missing');
  ...
  assert.equal(termBufferIsNotOriginalGolden(r), false, 'term buffer is not original-golden');
  ...
  let cameraFail = 0, mismatchFail = 0, termFail = 0;
  for (const spec of TERM_FRAMES) {
    ...
    if (!row.dominantTerm || !row.dominant || row.layers.length !== TERM_LAYERS.length
      || TERM_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || TERM_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) termFail += 1;
  }
  return [
    {
      case: 'combined unlit luma / envmap B is not dir/fill/ambient vs environment/envIntensity attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present term axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'dir/fill/ambient and environment/envIntensity of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
      status: termFail ? 'failed' : 'passed',
      probes: 4,
      failures: termFail,
    },
    {
    ...
    {
      case: 'term-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldTerm(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldTermResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldTerm(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex }) => {
      ...
      try {
        ...
        const isolate = {
          ...
          ambient() {
            return lightsOf(object => object.isAmbientLight);
          },
          environment() {
        ...
        engine.world.group.traverse(object => {
          ...
          const intensity = object.intensity;
          const restorers = [...isolate.dir(), ...isolate.fill(), ...isolate.ambient()];
          if (object.intensity !== intensity) lumaTermsIncludeHemi = true;
          restorers.forEach(restore => restore());
        });
        let environmentClearsIntensity = false;
        const intensityBeforeEnv = [];
        engine.world.group.traverse(object => {
          for (const mat of materialsOf(object)) {
            if (mat && 'envMapIntensity' in mat) intensityBeforeEnv.push(mat.envMapIntensity);
          }
        });
        const envRestorers = isolate.environment();
        const intensityAfterEnv = [];
        engine.world.group.traverse(object => {
          for (const mat of materialsOf(object)) {
            if (mat && 'envMapIntensity' in mat) intensityAfterEnv.push(mat.envMapIntensity);
          }
        });
        if (intensityBeforeEnv.length !== intensityAfterEnv.length
          || intensityBeforeEnv.some((value, i) => value !== intensityAfterEnv[i])) {
          environmentClearsIntensity = true;
        }
        envRestorers.forEach(restore => restore());
        let envIntensityClearsEnvironment = false;
        const environmentBefore = engine.scene.environment;
        const intensityRestorers = isolate.envIntensity();
        if (engine.scene.environment !== environmentBefore) envIntensityClearsEnvironment = true;
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = TERM_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantTerm: dominantTerm(layers),
        contributingTerm: layers.filter(layer => layer.l2Delta <= -TERM_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -TERM_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -TERM_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      envIntensityClearsEnvironment: report.envIntensityClearsEnvironment,
      ...
      envIntensityCount: report.envIntensityCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-term-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldTerm(browser, url) {
  const report = await measureWorldTerm(browser, url);
  const out = process.env.WORLD_TERM_OUTPUT ?? fromRoot('artifacts', 'world-term');
  const results = await retainWorldTerm(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldTerm(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
