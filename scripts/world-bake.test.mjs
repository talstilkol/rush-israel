...
import {
  BAND_PIXELS, DISC_IDS, BAKE_BLUE_MIN, BAKE_FRAMES, BAKE_L2_MIN, BAKE_LAYERS, BAKE_LUMA_MIN,
  FAILURE_LIMIT, HEMI_IDS, IBL_IDS, LUMA_IDS, PIXEL_THRESHOLD, PRODUCT_EXPOSURE,
  bakeBufferIsNotOriginalGolden, dominantBake, luma, retainWorldBake,
  probeIsNotBakeMismatch, worldBakeFixture, worldBakeResults,
} from './world-bake-browser.mjs';
import { worldProbeFixture } from './world-probe-browser.mjs';
...
test('dominant 200px probe report without background vs hemi vs disc is not bake mismatch', () => {
  assert.equal(probeIsNotBakeMismatch({ frames: [] }), true);
  assert.equal(probeIsNotBakeMismatch(worldProbeFixture()), true);
  assert.equal(probeIsNotBakeMismatch(worldBakeFixture()), false);
  assert.throws(() => worldBakeResults(worldProbeFixture()), /combined bakeEnv IBL is not background vs hemi vs sun disc mismatch/);
});

test('bake buffer without original-golden protocol identity is not original-golden', () => {
  assert.equal(bakeBufferIsNotOriginalGolden(worldBakeFixture()), false);
  assert.equal(bakeBufferIsNotOriginalGolden({
    ...worldBakeFixture(), originalGoldenComparisons: 4, protocol: 'original-golden',
  }), true);
  assert.equal(PIXEL_THRESHOLD, 0.12);
  assert.equal(FAILURE_LIMIT, 0.08);
  assert.equal(BAKE_L2_MIN, 8);
  assert.equal(BAKE_LUMA_MIN, 8);
  assert.equal(BAKE_BLUE_MIN, 8);
  assert.equal(PRODUCT_EXPOSURE, 0.56);
  assert.equal(BAND_PIXELS, 1280 * 200);
  assert.deepEqual(BAKE_LAYERS, ['background', 'hemi', 'disc', 'intensity', 'fill']);
  ...
  assert.throws(() => worldBakeResults({ ...worldBakeFixture(), productGroundHex: 0 }), /product ground color retuned/);
  assert.throws(() => worldBakeResults({ ...worldBakeFixture(), productExposure: 1 }), /product exposure retuned/);
  assert.throws(() => worldBakeResults({ ...worldBakeFixture(), lumaTermsIncludeHemi: true }), /luma terms include hemi/);
  assert.throws(() => worldBakeResults({ ...worldBakeFixture(), backgroundIsCombined: true }), /background isolation still combined/);
  assert.throws(() => worldBakeResults({ ...worldBakeFixture(), backgroundClearsGain: true }), /background isolation clears environmentIntensity/);
  assert.throws(() => worldBakeResults({ ...worldBakeFixture(), backgroundClearsIntensity: true }), /background isolation clears intensity/);
  assert.throws(() => worldBakeResults({ ...worldBakeFixture(), hemiClearsIntensity: true }), /hemi isolation clears intensity/);
  assert.throws(() => worldBakeResults({ ...worldBakeFixture(), intensityClearsIbl: true }), /intensity isolation clears scene.environment/);
});
...
test('locked bake poses match original-golden cameras and PNG hashes', () => {
  assert.deepEqual(BAKE_FRAMES.map(row => row.id), ['g01', 'g05', 'g07', 'g08']);
  assert.deepEqual(BAKE_FRAMES.map(row => row.file), ORIGINAL_GOLDEN_FILES);
  assert.equal(BAKE_FRAMES[3].night, true);
  assert.equal(dominantBake(worldBakeFixture().frames[0].layers), 'background');
});

test('bake-attribution evidence yields five protocol passes', () => {
  assert.deepEqual(worldBakeResults(worldBakeFixture()).map(row => row.status), Array(5).fill('passed'));
});
...
]) test(`world-bake evidence fails closed: ${name}`, () => {
  const r = worldBakeFixture();
  mutate(r);
  assert.throws(() => worldBakeResults(r));
});

test('baseline PNG hash drift fails closed', () => {
  const r = worldBakeFixture();
  r.baselineHashes['ayalon-day-g01.png'] = '0'.repeat(64);
  assert.throws(() => worldBakeResults(r), /baseline hash drift/);
});

test('zero full-frame present mismatch remains failed', () => {
  const r = worldBakeFixture();
  r.frames[0] = { ...r.frames[0], presentPct: 0, presentMismatched: 0 };
  const row = worldBakeResults(r).find(item => item.case.includes('still mismatches'));
  assert.equal(row.status, 'failed');
  assert.equal(row.failures, 1);
});

test('a rest-camera miss remains failed during bake sampling', () => {
  const r = worldBakeFixture();
  r.frames[1] = { ...r.frames[1], follow: 9.2, height: 2.28 };
  assert.throws(() => worldBakeResults(r));
});

test('invalid world-bake report is retained before validation throws', async () => {
  const out = await mkdtemp(join(tmpdir(), 'world-bake-invalid-'));
  try {
    const r = worldBakeFixture({ originalGoldenComparisons: 4 });
    await assert.rejects(retainWorldBake(r, out));
...
test('smoke keeps world-layer residual mismatch bias scene region column slice extra material factor rgb shade tone term beam ray ibl gain cube probe and bake probes', () => {
