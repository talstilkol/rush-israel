...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of autoClearColor vs autoClearDepth vs autoClearStencil. */
export const ACD_FRAMES = REGION_FRAMES;
export const ACD_LAYERS = Object.freeze([
  'color', 'depth', 'stencil', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = ACD_LAYERS;
export const COLOR_IDS = Object.freeze(['color']);
export const DEPTH_IDS = Object.freeze(['depth']);
...
export const ACD_L2_MIN = 8;
export const ACD_LUMA_MIN = 8;
export const ACD_BLUE_MIN = 8;
export const ACD_BAND_MIN = 0.02;
...
export const PRODUCT_COLOR = true;
export const PRODUCT_DEPTH = true;
export const PRODUCT_STENCIL = true;
...
export function dominantAcd(layers) {
  if (!Array.isArray(layers) || layers.length !== ACD_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotAcdMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== ACD_LAYERS.length
    || ACD_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !ACD_LAYERS.includes(layer.id)
...
export function acdBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-acd-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { color: 2083, depth: 2083, stencil: 2083, env: 2083, both: 2084, pack: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'color' ? 18 : id === 'both' ? 20 : id === 'pack' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'color' ? -0.10 : id === 'both' ? -0.08 : id === 'pack' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = ACD_FRAMES.find(row => row.id === id);
  ...
  const layers = ACD_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantAcd: 'color',
    contributingAcd: ['color', 'pack'],
    colorAxis: ['color'],
    envAxis: [],
    bothAxis: ['both'],
    depthAxis: [],
...
export function worldAcdFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, colorCount: 2083, depthCount: 2083, stencilCount: 2083, envCount: 2083, bothCount: 2086, packCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    colorClears: true, colorHidesLeftover: true, colorKeepsEnv: true, colorKeepsHemi: true, colorKeepsDepth: true, colorKeepsStencil: true,
    depthClears: true, depthHidesLeftover: true, depthKeepsEnv: true, depthKeepsHemi: true, depthKeepsColor: true, depthKeepsStencil: true,
    stencilClears: true, stencilHidesLeftover: true, stencilKeepsEnv: true, stencilKeepsHemi: true, stencilKeepsColor: true, stencilKeepsDepth: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsColor: true, envKeepsDepth: true, envKeepsStencil: true, envClearsHue: false,
    packClearsColor: true, packClearsDepth: true, packClearsStencil: true, packHidesLeftover: true, packKeepsEnv: true, packKeepsHemi: true,
    bothSetsGray: true, bothClearsColor: true, bothClearsDepth: true, bothClearsStencil: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    acdDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-acd-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: ACD_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldAcdResults(r) {
  assert.ok(r && typeof r === 'object', 'world-acd evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-acd frames missing');
  assert.equal(leftoverIsNotAcdMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of autoClearColor vs autoClearDepth vs autoClearStencil mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'acd isolation still combined with hemi');
  assert.equal(r.colorClears, true, 'color isolation did not force autoClearColor off');
  assert.equal(r.colorHidesLeftover, true, 'color isolation did not hide leftover empty-scene meshes');
  assert.equal(r.colorKeepsEnv, true, 'color isolation clears scene.environment');
  assert.equal(r.colorKeepsHemi, true, 'hemi.color left product during color isolation');
  assert.equal(r.colorKeepsDepth, true, 'autoClearDepth left product during color isolation');
  assert.equal(r.colorKeepsStencil, true, 'autoClearStencil left product during color isolation');
  assert.equal(r.depthClears, true, 'depth isolation did not force autoClearDepth off');
  assert.equal(r.depthHidesLeftover, true, 'depth isolation did not hide leftover empty-scene meshes');
  assert.equal(r.depthKeepsEnv, true, 'depth isolation clears scene.environment');
  assert.equal(r.depthKeepsHemi, true, 'hemi.color left product during depth isolation');
  assert.equal(r.depthKeepsColor, true, 'autoClearColor left product during depth isolation');
  assert.equal(r.depthKeepsStencil, true, 'autoClearStencil left product during depth isolation');
  assert.equal(r.stencilClears, true, 'stencil isolation did not force autoClearStencil off');
  ...
  assert.equal(r.stencilKeepsColor, true, 'autoClearColor left product during stencil isolation');
  assert.equal(r.stencilKeepsDepth, true, 'autoClearDepth left product during stencil isolation');
  ...
  assert.equal(r.envKeepsColor, true, 'autoClearColor left product during env isolation');
  assert.equal(r.envKeepsDepth, true, 'autoClearDepth left product during env isolation');
  assert.equal(r.envKeepsStencil, true, 'autoClearStencil left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsColor, true, 'pack isolation did not force autoClearColor off');
  assert.equal(r.packClearsDepth, true, 'pack isolation did not force autoClearDepth off');
  assert.equal(r.packClearsStencil, true, 'pack isolation did not force autoClearStencil off');
  ...
  assert.equal(r.bothClearsColor, true, 'both isolation did not force autoClearColor off');
  assert.equal(r.bothClearsDepth, true, 'both isolation did not force autoClearDepth off');
  assert.equal(r.bothClearsStencil, true, 'both isolation did not force autoClearStencil off');
  ...
  assert.equal(r.intensityClearsIbl, false, 'acd isolation clears scene.environment via sun');
  assert.equal(r.acdDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after acd restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during acd probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during acd probe');
  ...
  assert.ok(r.colorCount >= 151, 'color isolation missing');
  assert.ok(r.depthCount >= 151, 'depth isolation missing');
  assert.ok(r.stencilCount >= 151, 'stencil isolation missing');
  assert.ok(r.packCount >= 151, 'combined autoClearColor-autoClearDepth-autoClearStencil isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both autoClearColor-autoClearDepth-autoClearStencil+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during acd probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during acd probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during acd probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during acd probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during acd probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during acd probe');
  ...
  assert.equal(acdBufferIsNotOriginalGolden(r), false, 'acd buffer is not original-golden');
  ...
  for (const spec of ACD_FRAMES) {
    ...
    if (!row.dominantAcd || !row.dominant || row.layers.length !== ACD_LAYERS.length
      || ACD_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.colorAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.depthAxis) || !Array.isArray(row.stencilAxis) || !Array.isArray(row.packAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || ACD_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of autoClearColor vs autoClearDepth vs autoClearStencil attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present acd axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'autoClearColor vs autoClearDepth vs autoClearStencil independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'acd-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldAcd(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldAcdResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldAcd(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let colorClears = false;
        let colorHidesLeftover = false;
        let colorKeepsEnv = true;
        let colorKeepsHemi = true;
        let colorKeepsDepth = true;
        let colorKeepsStencil = true;
        let depthClears = false;
        let depthHidesLeftover = false;
        let depthKeepsEnv = true;
        let depthKeepsHemi = true;
        let depthKeepsColor = true;
        let depthKeepsStencil = true;
        let stencilClears = false;
        let stencilHidesLeftover = false;
        let stencilKeepsEnv = true;
        let stencilKeepsHemi = true;
        let stencilKeepsColor = true;
        let stencilKeepsDepth = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsColor = true;
        let envKeepsDepth = true;
        let envKeepsStencil = true;
        let packClearsColor = false;
        let packClearsDepth = false;
        let packClearsStencil = false;
        let packHidesLeftover = false;
        let packKeepsEnv = true;
        let packKeepsHemi = true;
        let bothSetsGray = false;
        let bothClearsColor = false;
        let bothClearsDepth = false;
        ...
        const setColor = () => {
          const prev = engine.renderer.autoClearColor;
          engine.renderer.autoClearColor = false;
          return [() => { engine.renderer.autoClearColor = prev; }];
        };
        const setDepth = () => {
          const prev = engine.renderer.autoClearDepth;
          engine.renderer.autoClearDepth = false;
          return [() => { engine.renderer.autoClearDepth = prev; }];
        };
        const setStencil = () => {
          const prev = engine.renderer.autoClearStencil;
          engine.renderer.autoClearStencil = false;
          return [() => { engine.renderer.autoClearStencil = prev; }];
        };
        const isolate = {
          color() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevDepth = engine.renderer.autoClearDepth;
            const prevStencil = engine.renderer.autoClearStencil;
            const restorers = [...hideMeshes(leftoverOf()), ...setColor()];
            colorClears = true;
            if (leftoverHidden()) colorHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) colorKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) colorKeepsHemi = false;
            if (engine.renderer.autoClearDepth !== prevDepth) colorKeepsDepth = false;
            if (engine.renderer.autoClearStencil !== prevStencil) colorKeepsStencil = false;
            return restorers;
          },
          depth() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevColor = engine.renderer.autoClearColor;
            const prevStencil = engine.renderer.autoClearStencil;
            const restorers = [...hideMeshes(leftoverOf()), ...setDepth()];
            depthClears = true;
            if (leftoverHidden()) depthHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) depthKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) depthKeepsHemi = false;
            if (engine.renderer.autoClearColor !== prevColor) depthKeepsColor = false;
            if (engine.renderer.autoClearStencil !== prevStencil) depthKeepsStencil = false;
            return restorers;
          },
          stencil() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.autoClearColor !== prevColor) stencilKeepsColor = false;
            if (engine.renderer.autoClearDepth !== prevDepth) stencilKeepsDepth = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.autoClearColor !== prevColor) envKeepsColor = false;
            if (engine.renderer.autoClearDepth !== prevDepth) envKeepsDepth = false;
            if (engine.renderer.autoClearStencil !== prevStencil) envKeepsStencil = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
            ...
            packClearsColor = true;
            packClearsDepth = true;
          ...
          both() {
            ...
            bothClearsColor = true;
            bothClearsDepth = true;
        ...
        const bothIsCombined = !colorKeepsHemi || !depthKeepsHemi || !stencilKeepsHemi || !envKeepsHemi || !packKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.color();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.depth();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.stencil();
      ...
      frames: ACD_FRAMES,
      layers: ['color', 'depth', 'stencil', 'env', 'both', 'pack'],
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = ACD_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantAcd: dominantAcd(layers),
        contributingAcd: layers.filter(layer => layer.bandDelta <= -ACD_BAND_MIN).map(layer => layer.id),
        colorAxis: layers.filter(layer => COLOR_IDS.includes(layer.id) && layer.bandDelta <= -ACD_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -ACD_BAND_MIN || layer.blueDelta <= -ACD_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -ACD_BAND_MIN).map(layer => layer.id),
        depthAxis: layers.filter(layer => DEPTH_IDS.includes(layer.id) && layer.bandDelta <= -ACD_BAND_MIN).map(layer => layer.id),
        stencilAxis: layers.filter(layer => STENCIL_IDS.includes(layer.id) && layer.bandDelta <= -ACD_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -ACD_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      colorClears: report.colorClears,
      colorHidesLeftover: report.colorHidesLeftover,
      colorKeepsEnv: report.colorKeepsEnv,
      colorKeepsHemi: report.colorKeepsHemi,
      colorKeepsDepth: report.colorKeepsDepth,
      colorKeepsStencil: report.colorKeepsStencil,
      depthClears: report.depthClears,
      depthHidesLeftover: report.depthHidesLeftover,
      depthKeepsEnv: report.depthKeepsEnv,
      depthKeepsHemi: report.depthKeepsHemi,
      depthKeepsColor: report.depthKeepsColor,
      depthKeepsStencil: report.depthKeepsStencil,
      ...
      stencilKeepsColor: report.stencilKeepsColor,
      stencilKeepsDepth: report.stencilKeepsDepth,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsColor: report.envKeepsColor,
      envKeepsDepth: report.envKeepsDepth,
      envKeepsStencil: report.envKeepsStencil,
      envClearsHue: report.envClearsHue,
      packClearsColor: report.packClearsColor,
      packClearsDepth: report.packClearsDepth,
      ...
      bothClearsColor: report.bothClearsColor,
      bothClearsDepth: report.bothClearsDepth,
      ...
      acdDeltasUseEmptyBaseline: report.acdDeltasUseEmptyBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-acd-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldAcd(browser, url) {
  const report = await measureWorldAcd(browser, url);
  const out = process.env.WORLD_ACD_OUTPUT ?? fromRoot('artifacts', 'world-acd');
  const results = await retainWorldAcd(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldAcd(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
