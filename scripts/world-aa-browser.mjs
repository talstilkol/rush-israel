...
/** Leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of antialias vs samples vs alpha. */
export const AA_FRAMES = REGION_FRAMES;
export const AA_LAYERS = Object.freeze([
  'aa', 'samples', 'alpha', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = AA_LAYERS;
export const AA_IDS = Object.freeze(['aa']);
export const SAMPLES_IDS = Object.freeze(['samples']);
export const ALPHA_IDS = Object.freeze(['alpha']);
...
export const AA_L2_MIN = 8;
export const AA_LUMA_MIN = 8;
export const AA_BLUE_MIN = 8;
export const AA_BAND_MIN = 0.02;
...
export const WIDE_SAMPLES = 4;
export const PRODUCT_AA = false;
export const PRODUCT_ALPHA = false;
...
export function dominantAa(layers) {
  if (!Array.isArray(layers) || layers.length !== AA_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotAaMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== AA_LAYERS.length
    || AA_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !AA_LAYERS.includes(layer.id)
    ...
    || !frame.dominantAa
    || !frame.dominant
    || !Array.isArray(frame.aaAxis)
    || !Array.isArray(frame.envAxis)
    || !Array.isArray(frame.bothAxis)
    || !Array.isArray(frame.samplesAxis)
    || !Array.isArray(frame.alphaAxis)
...
export function aaBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-aa-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { aa: 2083, samples: 2083, alpha: 2083, env: 2083, both: 2084, pack: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'aa' ? 18 : id === 'both' ? 20 : id === 'pack' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'aa' ? -0.10 : id === 'both' ? -0.08 : id === 'pack' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = AA_FRAMES.find(row => row.id === id);
  ...
  const layers = AA_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantAa: 'aa',
    contributingAa: ['aa', 'pack'],
    aaAxis: ['aa'],
    envAxis: [],
    bothAxis: ['both'],
    samplesAxis: [],
...
export function worldAaFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, aaCount: 2083, samplesCount: 2083, alphaCount: 2083, envCount: 2083, bothCount: 2086, packCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    aaClears: true, aaHidesLeftover: true, aaKeepsEnv: true, aaKeepsHemi: true, aaKeepsSamples: true, aaKeepsAlpha: true,
    samplesSetsWide: true, samplesHidesLeftover: true, samplesKeepsEnv: true, samplesKeepsHemi: true, samplesKeepsAa: true, samplesKeepsAlpha: true,
    alphaClears: true, alphaHidesLeftover: true, alphaKeepsEnv: true, alphaKeepsHemi: true, alphaKeepsAa: true, alphaKeepsSamples: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsAa: true, envKeepsSamples: true, envKeepsAlpha: true, envClearsHue: false,
    packClearsAa: true, packSetsSamples: true, packClearsAlpha: true, packHidesLeftover: true, packKeepsEnv: true, packKeepsHemi: true,
    bothSetsGray: true, bothClearsAa: true, bothSetsSamples: true, bothClearsAlpha: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    aaDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-aa-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: AA_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldAaResults(r) {
  assert.ok(r && typeof r === 'object', 'world-aa evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-aa frames missing');
  assert.equal(leftoverIsNotAaMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of antialias vs samples vs alpha mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'aa isolation still combined with hemi');
  assert.equal(r.aaClears, true, 'aa isolation did not force drawing-buffer path');
  assert.equal(r.aaHidesLeftover, true, 'aa isolation did not hide leftover empty-scene meshes');
  assert.equal(r.aaKeepsEnv, true, 'aa isolation clears scene.environment');
  assert.equal(r.aaKeepsHemi, true, 'hemi.color left product during aa isolation');
  assert.equal(r.aaKeepsSamples, true, 'samples left product during aa isolation');
  assert.equal(r.aaKeepsAlpha, true, 'alpha left product during aa isolation');
  assert.equal(r.samplesSetsWide, true, 'samples isolation did not set RT samples 4');
  assert.equal(r.samplesHidesLeftover, true, 'samples isolation did not hide leftover empty-scene meshes');
  assert.equal(r.samplesKeepsEnv, true, 'samples isolation clears scene.environment');
  assert.equal(r.samplesKeepsHemi, true, 'hemi.color left product during samples isolation');
  assert.equal(r.samplesKeepsAa, true, 'aa left product during samples isolation');
  assert.equal(r.samplesKeepsAlpha, true, 'alpha left product during samples isolation');
  assert.equal(r.alphaClears, true, 'alpha isolation did not set clear alpha 0');
  assert.equal(r.alphaHidesLeftover, true, 'alpha isolation did not hide leftover empty-scene meshes');
  assert.equal(r.alphaKeepsEnv, true, 'alpha isolation clears scene.environment');
  assert.equal(r.alphaKeepsHemi, true, 'hemi.color left product during alpha isolation');
  assert.equal(r.alphaKeepsAa, true, 'aa left product during alpha isolation');
  assert.equal(r.alphaKeepsSamples, true, 'samples left product during alpha isolation');
  ...
  assert.equal(r.envKeepsAa, true, 'aa left product during env isolation');
  assert.equal(r.envKeepsSamples, true, 'samples left product during env isolation');
  assert.equal(r.envKeepsAlpha, true, 'alpha left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsAa, true, 'pack isolation did not force drawing-buffer path');
  assert.equal(r.packSetsSamples, true, 'pack isolation did not set RT samples 4');
  assert.equal(r.packClearsAlpha, true, 'pack isolation did not set clear alpha 0');
  ...
  assert.equal(r.bothClearsAa, true, 'both isolation did not force drawing-buffer path');
  assert.equal(r.bothSetsSamples, true, 'both isolation did not set RT samples 4');
  assert.equal(r.bothClearsAlpha, true, 'both isolation did not set clear alpha 0');
  ...
  assert.equal(r.intensityClearsIbl, false, 'aa isolation clears scene.environment via sun');
  assert.equal(r.aaDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after aa restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during aa probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during aa probe');
  ...
  assert.ok(r.aaCount >= 151, 'aa isolation missing');
  assert.ok(r.samplesCount >= 151, 'samples isolation missing');
  assert.ok(r.alphaCount >= 151, 'alpha isolation missing');
  assert.ok(r.packCount >= 151, 'combined antialias-samples-alpha isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both antialias-samples-alpha+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during aa probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during aa probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during aa probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during aa probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during aa probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during aa probe');
  ...
  assert.equal(aaBufferIsNotOriginalGolden(r), false, 'aa buffer is not original-golden');
  ...
  for (const spec of AA_FRAMES) {
    ...
    if (!row.dominantAa || !row.dominant || row.layers.length !== AA_LAYERS.length
      || AA_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.aaAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.samplesAxis) || !Array.isArray(row.alphaAxis) || !Array.isArray(row.packAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || AA_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of antialias vs samples vs alpha attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present aa axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'antialias vs samples vs alpha independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'aa-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldAa(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldAaResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldAa(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let aaClears = false;
        let aaHidesLeftover = false;
        let aaKeepsEnv = true;
        let aaKeepsHemi = true;
        let aaKeepsSamples = true;
        let aaKeepsAlpha = true;
        let samplesSetsWide = false;
        let samplesHidesLeftover = false;
        let samplesKeepsEnv = true;
        let samplesKeepsHemi = true;
        let samplesKeepsAa = true;
        let samplesKeepsAlpha = true;
        let alphaClears = false;
        let alphaHidesLeftover = false;
        let alphaKeepsEnv = true;
        let alphaKeepsHemi = true;
        let alphaKeepsAa = true;
        let alphaKeepsSamples = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsAa = true;
        let envKeepsSamples = true;
        let envKeepsAlpha = true;
        let packClearsAa = false;
        let packSetsSamples = false;
        let packClearsAlpha = false;
        let packHidesLeftover = false;
        let packKeepsEnv = true;
        let packKeepsHemi = true;
        let bothSetsGray = false;
        let bothClearsAa = false;
        let bothSetsSamples = false;
        let bothClearsAlpha = false;
        ...
        const setAa = () => {
          engine.post.setTier('low');
          return [() => engine.post.setTier('high')];
        };
        const setSamples = () => {
          const composer = engine.post.composer;
          const rt1 = composer.renderTarget1;
          const rt2 = composer.renderTarget2;
          const prev1 = rt1.samples ?? 0;
          const prev2 = rt2.samples ?? 0;
          rt1.dispose();
          rt2.dispose();
          rt1.samples = 4;
          rt2.samples = 4;
        ...
        const setAlpha = () => {
          const prev = engine.renderer.getClearAlpha();
          engine.renderer.setClearAlpha(0);
          return [() => engine.renderer.setClearAlpha(prev)];
        };
        const isolate = {
          aa() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevSamples = engine.post.composer.renderTarget1.samples ?? 0;
            const prevAlpha = engine.renderer.getClearAlpha();
            const restorers = [...hideMeshes(leftoverOf()), ...setAa()];
            aaClears = true;
            if (leftoverHidden()) aaHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) aaKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) aaKeepsHemi = false;
            if ((engine.post.composer.renderTarget1.samples ?? 0) !== prevSamples) aaKeepsSamples = false;
            if (engine.renderer.getClearAlpha() !== prevAlpha) aaKeepsAlpha = false;
            return restorers;
          },
          samples() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevAlpha = engine.renderer.getClearAlpha();
            const restorers = [...hideMeshes(leftoverOf()), ...setSamples()];
            samplesSetsWide = true;
            if (leftoverHidden()) samplesHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) samplesKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) samplesKeepsHemi = false;
            samplesKeepsAa = true;
            if (engine.renderer.getClearAlpha() !== prevAlpha) samplesKeepsAlpha = false;
            return restorers;
          },
          alpha() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevSamples = engine.post.composer.renderTarget1.samples ?? 0;
            const restorers = [...hideMeshes(leftoverOf()), ...setAlpha()];
            alphaClears = true;
            if (leftoverHidden()) alphaHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) alphaKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) alphaKeepsHemi = false;
            alphaKeepsAa = true;
            if ((engine.post.composer.renderTarget1.samples ?? 0) !== prevSamples) alphaKeepsSamples = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            envKeepsAa = true;
            if ((engine.post.composer.renderTarget1.samples ?? 0) !== prevSamples) envKeepsSamples = false;
            if (engine.renderer.getClearAlpha() !== prevAlpha) envKeepsAlpha = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
            ...
            packClearsAa = true;
            packSetsSamples = true;
            packClearsAlpha = true;
          ...
          both() {
            ...
            bothClearsAa = true;
            bothSetsSamples = true;
            bothClearsAlpha = true;
        ...
        const bothIsCombined = !aaKeepsHemi || !samplesKeepsHemi || !alphaKeepsHemi || !envKeepsHemi || !packKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.aa();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.samples();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.alpha();
      ...
      frames: AA_FRAMES,
      layers: ['aa', 'samples', 'alpha', 'env', 'both', 'pack'],
    ...
    const frames = report.frames.map(row => {
      ...
      return {
        ...
        layers,
        dominantAa: dominantAa(layers),
        contributingAa: layers.filter(layer => layer.bandDelta <= -AA_BAND_MIN).map(layer => layer.id),
        aaAxis: layers.filter(layer => AA_IDS.includes(layer.id) && layer.bandDelta <= -AA_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -AA_BAND_MIN || layer.blueDelta <= -AA_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -AA_BAND_MIN).map(layer => layer.id),
        samplesAxis: layers.filter(layer => SAMPLES_IDS.includes(layer.id) && layer.bandDelta <= -AA_BAND_MIN).map(layer => layer.id),
        alphaAxis: layers.filter(layer => ALPHA_IDS.includes(layer.id) && layer.bandDelta <= -AA_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -AA_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      aaClears: report.aaClears,
      aaHidesLeftover: report.aaHidesLeftover,
      aaKeepsEnv: report.aaKeepsEnv,
      aaKeepsHemi: report.aaKeepsHemi,
      aaKeepsSamples: report.aaKeepsSamples,
      aaKeepsAlpha: report.aaKeepsAlpha,
      samplesSetsWide: report.samplesSetsWide,
      samplesHidesLeftover: report.samplesHidesLeftover,
      samplesKeepsEnv: report.samplesKeepsEnv,
      samplesKeepsHemi: report.samplesKeepsHemi,
      samplesKeepsAa: report.samplesKeepsAa,
      samplesKeepsAlpha: report.samplesKeepsAlpha,
      alphaClears: report.alphaClears,
      alphaHidesLeftover: report.alphaHidesLeftover,
      alphaKeepsEnv: report.alphaKeepsEnv,
      alphaKeepsHemi: report.alphaKeepsHemi,
      alphaKeepsAa: report.alphaKeepsAa,
      alphaKeepsSamples: report.alphaKeepsSamples,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsAa: report.envKeepsAa,
      envKeepsSamples: report.envKeepsSamples,
      envKeepsAlpha: report.envKeepsAlpha,
      envClearsHue: report.envClearsHue,
      packClearsAa: report.packClearsAa,
      packSetsSamples: report.packSetsSamples,
      packClearsAlpha: report.packClearsAlpha,
      ...
      bothClearsAa: report.bothClearsAa,
      bothSetsSamples: report.bothSetsSamples,
      bothClearsAlpha: report.bothClearsAlpha,
      bothHidesLeftover: report.bothHidesLeftover,
      bothKeepsHemi: report.bothKeepsHemi,
      intensityClearsIbl: report.intensityClearsIbl,
      aaDeltasUseEmptyBaseline: report.aaDeltasUseEmptyBaseline,
      ...
      aaCount: report.aaCount,
      samplesCount: report.samplesCount,
      alphaCount: report.alphaCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-aa-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldAa(browser, url) {
  const report = await measureWorldAa(browser, url);
  const out = process.env.WORLD_AA_OUTPUT ?? fromRoot('artifacts', 'world-aa');
  const results = await retainWorldAa(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldAa(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
