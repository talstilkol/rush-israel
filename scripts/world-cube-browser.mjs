...
/** Combined IBL is not product cubemap content vs a neutral gray cubemap attribution. */
export const CUBE_FRAMES = REGION_FRAMES;
export const CUBE_LAYERS = Object.freeze([
  'content', 'ibl', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['ibl']);
export const CONTENT_IDS = Object.freeze(['content']);
export const NEUTRAL_CUBE_HEX = 0x808080;
export const CUBE_L2_MIN = 8;
export const CUBE_LUMA_MIN = 8;
export const CUBE_BLUE_MIN = 8;
...
export function dominantCube(layers) {
  if (!Array.isArray(layers) || layers.length !== CUBE_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function gainIsNotCubeMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== CUBE_LAYERS.length
    || CUBE_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !CUBE_LAYERS.includes(layer.id)
    ...
    || !Array.isArray(frame.contentAxis)
...
export function cubeBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-cube-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { content: 1, ibl: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'content' ? 50 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 85;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'content' ? 90 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = CUBE_FRAMES.find(row => row.id === id);
  ...
  const layers = CUBE_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantCube: 'content',
    contributingCube: ['content', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: ['ibl'],
    contentAxis: ['content'],
    ...extra,
...
export function worldCubeFixture(extra = {}) {
  return {
    ...
    contentCount: 1, iblCount: 1, intensityCount: 1, fillCount: 1, grayCount: 1,
    lumaTermsIncludeHemi: false, contentIsCombined: false, contentClearsGain: false, contentClearsIntensity: false,
    iblClearsIntensity: false, intensityClearsIbl: false,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-cube-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: CUBE_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldCubeResults(r) {
  assert.ok(r && typeof r === 'object', 'world-cube evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-cube frames missing');
  assert.equal(gainIsNotCubeMismatch(r), false, 'combined IBL is not product cubemap content vs a neutral gray cubemap mismatch');
  ...
  assert.equal(r.contentIsCombined, false, 'content isolation still combined with ibl/gain');
  assert.equal(r.contentClearsGain, false, 'content isolation clears environmentIntensity');
  assert.equal(r.contentClearsIntensity, false, 'content isolation clears intensity');
  assert.equal(r.iblClearsIntensity, false, 'ibl isolation clears intensity');
  ...
  assert.ok(r.contentCount >= 1, 'product cubemap missing');
  assert.ok(r.iblCount >= 1, 'ibl isolation missing');
  assert.ok(r.grayCount >= 1, 'neutral gray cubemap missing');
  ...
  assert.equal(cubeBufferIsNotOriginalGolden(r), false, 'cube buffer is not original-golden');
  ...
  let cameraFail = 0, mismatchFail = 0, cubeFail = 0;
  for (const spec of CUBE_FRAMES) {
    ...
    if (!row.dominantCube || !row.dominant || row.layers.length !== CUBE_LAYERS.length
      || CUBE_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.contentAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || CUBE_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) cubeFail += 1;
  }
  return [
    {
      case: 'combined IBL is not product cubemap content vs a neutral gray cubemap attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present cube axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'content/ibl and intensity/fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
      status: cubeFail ? 'failed' : 'passed',
      probes: 4,
      failures: cubeFail,
    },
    {
    ...
    {
      case: 'cube-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldCube(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldCubeResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldCube(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const grayScene = new THREE.Scene();
        grayScene.background = new THREE.Color(0x808080);
        const pmrem = new THREE.PMREMGenerator(engine.renderer);
        gray = pmrem.fromScene(grayScene, 0);
        pmrem.dispose();
        const isolate = {
          content() {
            const environment = engine.scene.environment;
            engine.scene.environment = gray.texture;
        ...
        const contentRestorers = isolate.content();
        const contentIsCombined = contentRestorers.length !== 1;
        const contentClearsGain = engine.scene.environmentIntensity !== gainBefore;
        const contentClearsIntensity = sun.intensity !== intensityBefore;
        contentRestorers.forEach(restore => restore());
        const iblRestorers = isolate.ibl();
        const iblClearsIntensity = sun.intensity !== intensityBefore;
        iblRestorers.forEach(restore => restore());
        ...
        return {
          ...
          contentIsCombined,
          contentClearsGain,
          contentClearsIntensity,
          iblClearsIntensity,
          intensityClearsIbl,
          grayCount: gray?.texture ? 1 : 0,
          ...counts,
          glError: gl.getError(),
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = CUBE_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantCube: dominantCube(layers),
        contributingCube: layers.filter(layer => layer.l2Delta <= -CUBE_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -CUBE_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -CUBE_BLUE_MIN).map(layer => layer.id),
        contentAxis: layers.filter(layer => CONTENT_IDS.includes(layer.id) && (layer.l2Delta <= -CUBE_L2_MIN || layer.blueDelta <= -CUBE_BLUE_MIN)).map(layer => layer.id),
      };
    });
    return {
      ...
      contentIsCombined: report.contentIsCombined,
      contentClearsGain: report.contentClearsGain,
      contentClearsIntensity: report.contentClearsIntensity,
      iblClearsIntensity: report.iblClearsIntensity,
      intensityClearsIbl: report.intensityClearsIbl,
      contentCount: report.contentCount,
      iblCount: report.iblCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-cube-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldCube(browser, url) {
  const report = await measureWorldCube(browser, url);
  const out = process.env.WORLD_CUBE_OUTPUT ?? fromRoot('artifacts', 'world-cube');
  const results = await retainWorldCube(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldCube(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
