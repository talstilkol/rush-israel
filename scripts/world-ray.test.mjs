...
import {
  BAND_PIXELS, COLOR_IDS, RAY_BLUE_MIN, RAY_FRAMES, RAY_L2_MIN, RAY_LAYERS, RAY_LUMA_MIN,
  FAILURE_LIMIT, IBL_IDS, LUMA_IDS, PIXEL_THRESHOLD, PRODUCT_EXPOSURE,
  rayBufferIsNotOriginalGolden, dominantRay, luma, retainWorldRay,
  beamIsNotRayMismatch, worldRayFixture, worldRayResults,
} from './world-ray-browser.mjs';
import { worldBeamFixture } from './world-beam-browser.mjs';
...
test('dominant 200px beam report without color vs intensity is not ray mismatch', () => {
  assert.equal(beamIsNotRayMismatch({ frames: [] }), true);
  assert.equal(beamIsNotRayMismatch(worldBeamFixture()), true);
  assert.equal(beamIsNotRayMismatch(worldRayFixture()), false);
  assert.throws(() => worldRayResults(worldBeamFixture()), /combined sun luma is not color vs intensity mismatch/);
});

test('ray buffer without original-golden protocol identity is not original-golden', () => {
  assert.equal(rayBufferIsNotOriginalGolden(worldRayFixture()), false);
  assert.equal(rayBufferIsNotOriginalGolden({
    ...worldRayFixture(), originalGoldenComparisons: 4, protocol: 'original-golden',
  }), true);
  assert.equal(PIXEL_THRESHOLD, 0.12);
  assert.equal(FAILURE_LIMIT, 0.08);
  assert.equal(RAY_L2_MIN, 8);
  assert.equal(RAY_LUMA_MIN, 8);
  assert.equal(RAY_BLUE_MIN, 8);
  assert.equal(PRODUCT_EXPOSURE, 0.56);
  assert.equal(BAND_PIXELS, 1280 * 200);
  assert.deepEqual(RAY_LAYERS, ['intensity', 'color', 'fill', 'environment']);
  ...
  assert.throws(() => worldRayResults({ ...worldRayFixture(), productGroundHex: 0 }), /product ground color retuned/);
  assert.throws(() => worldRayResults({ ...worldRayFixture(), productExposure: 1 }), /product exposure retuned/);
  assert.throws(() => worldRayResults({ ...worldRayFixture(), lumaTermsIncludeHemi: true }), /luma terms include hemi/);
  assert.throws(() => worldRayResults({ ...worldRayFixture(), sunIsCombined: true }), /sun intensity and color still isolated together/);
  assert.throws(() => worldRayResults({ ...worldRayFixture(), colorClearsIntensity: true }), /color isolation clears intensity/);
  assert.throws(() => worldRayResults({ ...worldRayFixture(), intensityClearsColor: true }), /intensity isolation clears color/);
});
...
test('locked ray poses match original-golden cameras and PNG hashes', () => {
  assert.deepEqual(RAY_FRAMES.map(row => row.id), ['g01', 'g05', 'g07', 'g08']);
  assert.deepEqual(RAY_FRAMES.map(row => row.file), ORIGINAL_GOLDEN_FILES);
  assert.equal(RAY_FRAMES[3].night, true);
  assert.equal(dominantRay(worldRayFixture().frames[0].layers), 'intensity');
});

test('ray-attribution evidence yields five protocol passes', () => {
  assert.deepEqual(worldRayResults(worldRayFixture()).map(row => row.status), Array(5).fill('passed'));
});
...
]) test(`world-ray evidence fails closed: ${name}`, () => {
  const r = worldRayFixture();
  mutate(r);
  assert.throws(() => worldRayResults(r));
});

test('baseline PNG hash drift fails closed', () => {
  const r = worldRayFixture();
  r.baselineHashes['ayalon-day-g01.png'] = '0'.repeat(64);
  assert.throws(() => worldRayResults(r), /baseline hash drift/);
});

test('zero full-frame present mismatch remains failed', () => {
  const r = worldRayFixture();
  r.frames[0] = { ...r.frames[0], presentPct: 0, presentMismatched: 0 };
  const row = worldRayResults(r).find(item => item.case.includes('still mismatches'));
  assert.equal(row.status, 'failed');
  assert.equal(row.failures, 1);
});

test('a rest-camera miss remains failed during ray sampling', () => {
  const r = worldRayFixture();
  r.frames[1] = { ...r.frames[1], follow: 9.2, height: 2.28 };
  assert.throws(() => worldRayResults(r));
});

test('invalid world-ray report is retained before validation throws', async () => {
  const out = await mkdtemp(join(tmpdir(), 'world-ray-invalid-'));
  try {
    const r = worldRayFixture({ originalGoldenComparisons: 4 });
    await assert.rejects(retainWorldRay(r, out));
...
test('smoke keeps world-layer residual mismatch bias scene region column slice extra material factor rgb shade tone term beam and ray probes', () => {
