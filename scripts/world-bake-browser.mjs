...
/** Combined bakeEnv IBL is not background vs hemi vs sun disc attribution. */
export const BAKE_FRAMES = REGION_FRAMES;
export const BAKE_LAYERS = Object.freeze([
  'background', 'hemi', 'disc', 'intensity', 'fill',
]);
...
export const BAKE_L2_MIN = 8;
export const BAKE_LUMA_MIN = 8;
export const BAKE_BLUE_MIN = 8;
...
export function dominantBake(layers) {
  if (!Array.isArray(layers) || layers.length !== BAKE_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function probeIsNotBakeMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== BAKE_LAYERS.length
    || BAKE_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !BAKE_LAYERS.includes(layer.id)
...
export function bakeBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-bake-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { background: 1, hemi: 1, disc: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'background' ? 50 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'background' ? 90 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = BAKE_FRAMES.find(row => row.id === id);
  ...
  const layers = BAKE_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantBake: 'background',
    contributingBake: ['background', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: ['background'],
    hemiAxis: [],
    discAxis: [],
...
export function worldBakeFixture(extra = {}) {
  return {
    ...
    backgroundCount: 1, hemiCount: 1, discCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, backgroundIsCombined: false, backgroundClearsGain: false, backgroundClearsIntensity: false,
    hemiClearsIntensity: false, intensityClearsIbl: false,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-bake-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: BAKE_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldBakeResults(r) {
  assert.ok(r && typeof r === 'object', 'world-bake evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-bake frames missing');
  assert.equal(probeIsNotBakeMismatch(r), false, 'combined bakeEnv IBL is not background vs hemi vs sun disc mismatch');
  ...
  assert.equal(r.backgroundIsCombined, false, 'background isolation still combined with hemi/disc/gain');
  assert.equal(r.backgroundClearsGain, false, 'background isolation clears environmentIntensity');
  assert.equal(r.backgroundClearsIntensity, false, 'background isolation clears intensity');
  assert.equal(r.hemiClearsIntensity, false, 'hemi isolation clears intensity');
  ...
  assert.ok(r.backgroundCount >= 1, 'bakeEnv background missing');
  ...
  assert.equal(bakeBufferIsNotOriginalGolden(r), false, 'bake buffer is not original-golden');
  ...
  for (const spec of BAKE_FRAMES) {
    ...
    if (!row.dominantBake || !row.dominant || row.layers.length !== BAKE_LAYERS.length
      || BAKE_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.hemiAxis) || !Array.isArray(row.discAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || BAKE_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined bakeEnv IBL is not background vs hemi vs sun disc attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present bake axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'bakeEnv background/hemi/disc and intensity/fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'bake-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldBake(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldBakeResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldBake(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const bakeVariant = (omit) => {
          ...
          if (!omit.background) tmp.background = new THREE.Color(night ? 0x182436 : 0x3a9ae0);
        ...
        const isolate = {
          background() {
            return swapEnv(bakeVariant({ background: true }));
          },
          hemi() {
        ...
        const backgroundIsCombined = backgroundRestorers.length !== 1;
        const backgroundClearsGain = engine.scene.environmentIntensity !== gainBefore;
        const backgroundClearsIntensity = sun.intensity !== intensityBefore;
        backgroundRestorers.forEach(restore => restore());
        const hemiRestorers = isolate.hemi();
        const hemiClearsIntensity = sun.intensity !== intensityBefore;
        ...
        return {
          ...
          backgroundClearsGain,
          backgroundClearsIntensity,
          hemiClearsIntensity,
          intensityClearsIbl,
          ...counts,
          glError: gl.getError(),
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = BAKE_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantBake: dominantBake(layers),
        contributingBake: layers.filter(layer => layer.l2Delta <= -BAKE_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -BAKE_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -BAKE_BLUE_MIN).map(layer => layer.id),
        hemiAxis: layers.filter(layer => HEMI_IDS.includes(layer.id) && (layer.l2Delta <= -BAKE_L2_MIN || layer.blueDelta <= -BAKE_BLUE_MIN)).map(layer => layer.id),
        discAxis: layers.filter(layer => DISC_IDS.includes(layer.id) && (layer.l2Delta <= -BAKE_L2_MIN || layer.blueDelta <= -BAKE_BLUE_MIN)).map(layer => layer.id),
      };
    });
    return {
      ...
      backgroundClearsGain: report.backgroundClearsGain,
      backgroundClearsIntensity: report.backgroundClearsIntensity,
      hemiClearsIntensity: report.hemiClearsIntensity,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-bake-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldBake(browser, url) {
  const report = await measureWorldBake(browser, url);
  const out = process.env.WORLD_BAKE_OUTPUT ?? fromRoot('artifacts', 'world-bake');
  const results = await retainWorldBake(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldBake(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
