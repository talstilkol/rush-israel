...
/** Combined dir luma is not sun vs near attribution. */
export const BEAM_FRAMES = REGION_FRAMES;
export const BEAM_LAYERS = Object.freeze([
  'sun', 'near', 'fill', 'environment',
]);
export const LUMA_IDS = Object.freeze(['sun', 'near', 'fill']);
export const IBL_IDS = Object.freeze(['environment']);
export const BEAM_L2_MIN = 8;
export const BEAM_LUMA_MIN = 8;
export const BEAM_BLUE_MIN = 8;
...
export function dominantBeam(layers) {
  if (!Array.isArray(layers) || layers.length !== BEAM_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function termIsNotBeamMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== BEAM_LAYERS.length
    || BEAM_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !BEAM_LAYERS.includes(layer.id)
...
export function beamBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-beam-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { sun: 1, near: 1, fill: 1, environment: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'sun' ? 50 : id === 'near' ? 100 : id === 'fill' ? 90 : 85;
...
function frame(id, extra = {}) {
  const spec = BEAM_FRAMES.find(row => row.id === id);
  ...
  const layers = BEAM_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantBeam: 'sun',
    contributingBeam: ['sun', 'environment'],
    lumaAxis: ['sun'],
    iblAxis: ['environment'],
    ...extra,
  };
...
export function worldBeamFixture(extra = {}) {
  return {
    ...
    sunCount: 1, nearCount: 1, fillCount: 1, environmentCount: 1,
    lumaTermsIncludeHemi: false, dirIsCombined: false, sunClearsNear: false, nearClearsSun: false,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-beam-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: BEAM_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldBeamResults(r) {
  assert.ok(r && typeof r === 'object', 'world-beam evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-beam frames missing');
  assert.equal(termIsNotBeamMismatch(r), false, 'combined dir luma is not sun vs near mismatch');
  ...
  assert.equal(r.dirIsCombined, false, 'sun and near still isolated together');
  assert.equal(r.sunClearsNear, false, 'sun isolation clears near');
  assert.equal(r.nearClearsSun, false, 'near isolation clears sun');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.sunCount >= 1, 'sun light missing');
  assert.ok(r.nearCount >= 1, 'near light missing');
  ...
  assert.equal(beamBufferIsNotOriginalGolden(r), false, 'beam buffer is not original-golden');
  ...
  let cameraFail = 0, mismatchFail = 0, beamFail = 0;
  for (const spec of BEAM_FRAMES) {
    ...
    if (!row.dominantBeam || !row.dominant || row.layers.length !== BEAM_LAYERS.length
      || BEAM_LAYERS.some(id => !byId[id])
      ...
      || BEAM_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) beamFail += 1;
  }
  return [
    {
      case: 'combined dir luma is not sun vs near attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present beam axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'sun/near/fill and environment of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
      status: beamFail ? 'failed' : 'passed',
      probes: 4,
      failures: beamFail,
    },
    {
    ...
    {
      case: 'beam-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldBeam(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldBeamResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldBeam(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex }) => {
      ...
      try {
        ...
        const isolate = {
          ...
          near() {
            return lightsOf(object => object === sunNear);
          },
          fill() {
        ...
        engine.world.group.traverse(object => {
          ...
          const intensity = object.intensity;
          const restorers = [...isolate.sun(), ...isolate.near(), ...isolate.fill()];
          if (object.intensity !== intensity) lumaTermsIncludeHemi = true;
          restorers.forEach(restore => restore());
        });
        const nearSaved = sunNear.intensity;
        const sunSaved = sun.intensity;
        const sunRestorers = isolate.sun();
        const dirIsCombined = sunRestorers.length !== 1;
        const sunClearsNear = sunNear.intensity !== nearSaved;
        sunRestorers.forEach(restore => restore());
        const nearRestorers = isolate.near();
        const nearClearsSun = sun.intensity !== sunSaved;
        nearRestorers.forEach(restore => restore());
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = BEAM_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantBeam: dominantBeam(layers),
        contributingBeam: layers.filter(layer => layer.l2Delta <= -BEAM_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -BEAM_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -BEAM_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      dirIsCombined: report.dirIsCombined,
      sunClearsNear: report.sunClearsNear,
      nearClearsSun: report.nearClearsSun,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-beam-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldBeam(browser, url) {
  const report = await measureWorldBeam(browser, url);
  const out = process.env.WORLD_BEAM_OUTPUT ?? fromRoot('artifacts', 'world-beam');
  const results = await retainWorldBeam(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldBeam(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
