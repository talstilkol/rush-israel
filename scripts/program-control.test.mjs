...
test("current state and queue agree on every post-merge program count", () => {
  ...
  const queue = readJson("QUEUE.json");
  assert.equal(current.state_semantics.effective_event, "merge_of_RSH_046_pull_request");
  assert.equal(queue.state_effective_on, "merge_of_RSH_046_pull_request");
...
test("the historical batch and all completed one-unit authorizations are closed", () => {
  ...
  assert.equal(current.batch_authorization["RSH-030_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-030_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-031_authorized"], false);
  assert.equal(current.batch_authorization["RSH-031_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-031_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-032_authorized"], false);
  assert.equal(current.batch_authorization["RSH-032_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-032_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-033_authorized"], false);
  assert.equal(current.batch_authorization["RSH-033_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-033_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-034_authorized"], false);
  assert.equal(current.batch_authorization["RSH-034_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-034_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-035_authorized"], false);
  assert.equal(current.batch_authorization["RSH-035_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-035_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-036_authorized"], false);
  assert.equal(current.batch_authorization["RSH-036_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-036_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-037_authorized"], false);
  assert.equal(current.batch_authorization["RSH-037_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-037_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-038_authorized"], false);
  assert.equal(current.batch_authorization["RSH-038_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-038_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-039_authorized"], false);
  assert.equal(current.batch_authorization["RSH-039_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-039_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-040_authorized"], false);
  assert.equal(current.batch_authorization["RSH-040_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-040_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-041_authorized"], false);
  assert.equal(current.batch_authorization["RSH-041_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-041_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-042_authorized"], false);
  assert.equal(current.batch_authorization["RSH-042_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-042_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-043_authorized"], false);
  assert.equal(current.batch_authorization["RSH-043_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-043_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-044_authorized"], false);
  assert.equal(current.batch_authorization["RSH-044_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-044_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-045_authorized"], false);
  assert.equal(current.batch_authorization["RSH-045_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-045_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-046_authorized"], false);
  assert.equal(current.batch_authorization["RSH-046_was_separately_authorized"], true);
  assert.equal(current.batch_authorization["RSH-046_authorization_consumed"], true);
  assert.equal(current.batch_authorization["RSH-047_authorized"], false);
  assert.deepEqual(current.prior_single_unit_authorization.authorized_units, ["RSH-045"]);
  assert.equal(current.prior_single_unit_authorization.state, "consumed_on_RSH-045_merge");
  assert.deepEqual(current.single_unit_authorization.authorized_units, ["RSH-046"]);
  assert.equal(current.single_unit_authorization.completed_units, 1);
  assert.equal(current.single_unit_authorization.state, "consumed_on_RSH-046_merge");
  ...
  assert.equal(queue.state_rules["RSH-001–RSH-046"], "accepted");
  assert.deepEqual(queue.state_rules.eligible, []);
  assert.equal(queue.next_instruction_contract.current_action, "No unit is authorized; RSH-047 remains deferred until a new explicit owner instruction.");
  assert.equal(queue.next_instruction_contract.RSH_046_completed, true);
  assert.equal(queue.next_instruction_contract.RSH_046_authorization_consumed, true);
  assert.equal(queue.next_instruction_contract.RSH_047_authorized, false);
  assert.deepEqual(queue.policy.latest_single_unit_authorization.authorized_units, ["RSH-046"]);
  assert.equal(queue.policy.latest_single_unit_authorization.state, "consumed_on_RSH-046_merge");
});
...
test("RSH-029 exact evidence is reconciled before RSH-030 acceptance", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const accepted = current.accepted_units["RSH-029"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 32);
  assert.equal(accepted.validated_head_sha, "09b00d9f143ea6d43c8c72e679cc5c02bd38de97");
  assert.equal(accepted.merge_sha, "a42a99bed98c4a55b343e744cf374e68d3b51899");
  assert.equal(accepted.tree_sha, "6337499af1a77cdc6f5516758b8f2c845aad3c43");
  assert.equal(accepted.workflow_run, 33707956418);
  assert.equal(accepted.workflow_job, 100501088757);
  assert.equal(accepted.post_merge_workflow_run, 33708235679);
  assert.equal(accepted.post_merge_workflow_job, 100501942400);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-029"].merge_sha, accepted.merge_sha);
  const entry = baseline.baselines.find((item) => item.id === "B029-rsh-029-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 32);
});

test("RSH-030 exact evidence is reconciled before RSH-031 acceptance", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const accepted = current.accepted_units["RSH-030"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 33);
  assert.equal(accepted.validated_head_sha, "a89bd428a2db94695c7729da4908d0bd2f4d6bb0");
  assert.equal(accepted.merge_sha, "7a4451f4c55fe8aa44ad7999fc6de6e1c78c868c");
  assert.equal(accepted.tree_sha, "17cf5e70eac866aee7422f8f0e94279b7088683e");
  assert.equal(accepted.workflow_run, 33710365905);
  assert.equal(accepted.workflow_job, 100508326545);
  assert.equal(accepted.post_merge_workflow_run, 33710717424);
  assert.equal(accepted.post_merge_workflow_job, 100509391733);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-030"].merge_sha, accepted.merge_sha);
  const entry = baseline.baselines.find((item) => item.id === "B030-rsh-030-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 33);
});

test("RSH-031 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  ...
  const hero = readJson("HERO-CAR-MANIFEST.json");
  const daylight = readJson("DAYLIGHT-SKY-MANIFEST.json");
  assert.equal(current.accepted_units["RSH-031"].state, "accepted");
  assert.equal(current.accepted_units["RSH-031"].merge_sha, "04d922adc712a73cb0ccc7bd444a64b7b9157afd");
  assert.equal(current.accepted_units["RSH-031"].validated_head_sha, "9f0eaeca265d44505e6dbc34e9e8b08a2dfd35fd");
  assert.equal(current.accepted_units["RSH-031"].pull_request, 34);
  assert.equal(queue.accepted["RSH-031"].merge_sha, "04d922adc712a73cb0ccc7bd444a64b7b9157afd");
  ...
  assert.equal(records.deferred_boundary.rsh_030_started, true);
  assert.equal(records.deferred_boundary.rsh_031_started, true);
  assert.equal(records.deferred_boundary.rsh_032_started, true);
  assert.equal(records.deferred_boundary.rsh_030_authorized, true);
  assert.equal(records.deferred_boundary.rsh_031_authorized, true);
  assert.equal(records.deferred_boundary.rsh_032_authorized, true);
  ...
  assert.equal(hero.hero.count, 5);
  assert.equal(hero.hero.fictional, true);
  assert.equal(hero.hero.scanned, false);
  assert.equal(hero.hero.hero_gltf_present, false);
  assert.equal(hero.hero.lod0_max_triangles, 40000);
  assert.equal(hero.hero.gis_claim, false);
  assert.equal(hero.hero.owner_freeze, false);
  assert.equal(daylight.daylight.look, "summer14");
  assert.equal(daylight.daylight.hdri, false);
  assert.equal(daylight.ibl.kind, "tiny_pmrem");
  assert.equal(daylight.ibl.real_sky_ibl, false);
  assert.equal(daylight.sky.width, 1024);
  assert.equal(daylight.daylight.gis_claim, false);
  assert.equal(daylight.daylight.owner_freeze, false);
  const entry = baseline.baselines.find((item) => item.id === "B031-rsh-031-accepted");
  assert.equal(entry.commit_sha, "04d922adc712a73cb0ccc7bd444a64b7b9157afd");
  assert.equal(entry.validated_head_sha, "9f0eaeca265d44505e6dbc34e9e8b08a2dfd35fd");
  assert.equal(entry.pull_request, 34);
});
...
test("findings close RSH-024 secret scanning without overstating GitHub enforcement", () => {
  ...
  assert.match(findings, /\| P2-10 \| P2 \| \*\*CLOSED\*\*/);
  assert.match(findings, /\| P2-12 \| P2 \| \*\*CLOSED\*\*/);
});
...
test("product, architecture, save and recovery tests are in the complete suite", () => {
  ...
  assert.equal(readJson("PHYSICS-CALIBRATION-MANIFEST.json").unit, "RSH-033");
  assert.equal(readJson("AUDIO-HUD-INPUT-MANIFEST.json").unit, "RSH-034");
...
test("RSH-032 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const night = readJson("NIGHT-WEATHER-MANIFEST.json");
  const accepted = current.accepted_units["RSH-032"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 35);
  assert.equal(accepted.validated_head_sha, "2a826fd20cb2c45e13a322443c4d9c1186ee53a9");
  assert.equal(accepted.merge_sha, "f8d055c86b80ba1a72555e41668e071f74b32536");
  assert.equal(accepted.tree_sha, "9fd7f5b578f5be33f835527883f8b55342f279f5");
  assert.equal(accepted.workflow_run, 33713967588);
  assert.equal(accepted.post_merge_workflow_run, 33714396567);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-032"].merge_sha, accepted.merge_sha);
  assert.equal(night.night.look, "night");
  assert.equal(night.night.hdri, false);
  assert.equal(night.night.default_boot, false);
  assert.equal(night.weather.default, "clear");
  assert.equal(night.ibl.kind, "tiny_pmrem");
  assert.equal(night.ibl.real_sky_ibl, false);
  assert.equal(night.sky.width, 1024);
  assert.equal(night.night.gis_claim, false);
  assert.equal(night.night.owner_freeze, false);
  const entry = baseline.baselines.find((item) => item.id === "B032-rsh-032-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 35);
});

test("RSH-033 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const physics = readJson("PHYSICS-CALIBRATION-MANIFEST.json");
  const accepted = current.accepted_units["RSH-033"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 36);
  assert.equal(accepted.validated_head_sha, "ea95c3197ac547960c7096561034d3471fe5bd4d");
  assert.equal(accepted.merge_sha, "550c681445b2473cbd377131b60715ade0e58774");
  assert.equal(accepted.tree_sha, "4a2e0d3a02cb721a87b0a9485937c0a4dc5b1788");
  assert.equal(accepted.workflow_run, 33725688108);
  assert.equal(accepted.post_merge_workflow_run, 33726086520);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-033"].merge_sha, accepted.merge_sha);
  assert.equal(physics.calibration.physics_version, 7);
  assert.equal(physics.calibration.claim_tolerance, 0.15);
  assert.deepEqual(physics.calibration.claims, [8.4, 6.6, 4.9, 5.8, 3.5]);
  assert.equal(physics.calibration.cars_rewritten, false);
  assert.equal(physics.calibration.gear_dump_below_v100, false);
  assert.equal(physics.calibration.launch_drag_compensate, true);
  const entry = baseline.baselines.find((item) => item.id === "B033-rsh-033-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 36);
});

test("RSH-034 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const audio = readJson("AUDIO-HUD-INPUT-MANIFEST.json");
  const accepted = current.accepted_units["RSH-034"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 37);
  assert.equal(accepted.validated_head_sha, "285d871ee9a22b02ace7f732ac096eb7405c43e8");
  assert.equal(accepted.merge_sha, "0f53ae1e1451c3eff30a15829c0c0f43762feeb4");
  assert.equal(accepted.tree_sha, "3700c72ab79aefb96a0ba65adf92102a8058522f");
  assert.equal(accepted.workflow_run, 33728755421);
  assert.equal(accepted.post_merge_workflow_run, 33729193130);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-034"].merge_sha, accepted.merge_sha);
  assert.equal(audio.lock.backend, "oscillator");
  assert.equal(audio.lock.radio_count, 4);
  assert.equal(audio.lock.hud_speed_scale, 3.6);
  assert.equal(audio.lock.ffb, false);
  const entry = baseline.baselines.find((item) => item.id === "B034-rsh-034-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 37);
});

test("RSH-036 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const freeze = readJson("AYALON-FREEZE-MANIFEST.json");
  const accepted = current.accepted_units["RSH-036"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 40);
  assert.equal(accepted.validated_head_sha, "8ea8b31364096488860c1fe62779a38bd5d66c82");
  assert.equal(accepted.merge_sha, "4904161cc786061acc11942f76ed5666eb5ed61f");
  assert.equal(accepted.tree_sha, "2aa37f2b3508e9df3fd52a3bcf2af5533b90dab8");
  assert.equal(accepted.workflow_run, 34704099167);
  assert.equal(accepted.post_merge_workflow_run, 34704422852);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-036"].merge_sha, accepted.merge_sha);
  assert.equal(freeze.lock.freeze_granted, true);
  assert.equal(freeze.lock.source_count, 36);
  const entry = baseline.baselines.find((item) => item.id === "B036-rsh-036-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 40);
});

test("RSH-037 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const perf = readJson("PERF-INSTRUMENT-MANIFEST.json");
  const accepted = current.accepted_units["RSH-037"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 41);
  assert.equal(accepted.validated_head_sha, "8ca8e017a000a01251cb6c8481cd1b53a1d7bb7f");
  assert.equal(accepted.merge_sha, "91754100fc6a5d7bfc852e6aafa28cd17c1762ba");
  assert.equal(accepted.tree_sha, "2e09ed141e6658168889e8030ea19f97f526b746");
  assert.equal(accepted.workflow_run, 34705668679);
  assert.equal(accepted.post_merge_workflow_run, 34706006889);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-037"].merge_sha, accepted.merge_sha);
  assert.equal(perf.lock.sample_cap, 120);
  assert.equal(perf.lock.quality_profiles, false);
  const entry = baseline.baselines.find((item) => item.id === "B037-rsh-037-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 41);
});

test("RSH-038 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const quality = readJson("QUALITY-PROFILES-MANIFEST.json");
  const accepted = current.accepted_units["RSH-038"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 42);
  assert.equal(accepted.validated_head_sha, "6e3fc4e73346e27b5e11057ae9001076021280a4");
  assert.equal(accepted.merge_sha, "817ab7ed71d396b200188b16c27d627a1f2d7814");
  assert.equal(accepted.tree_sha, "dae1cddef51270ccdcf085df12db52164fff383b");
  assert.equal(accepted.workflow_run, 34706548905);
  assert.equal(accepted.post_merge_workflow_run, 34706788427);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-038"].merge_sha, accepted.merge_sha);
  assert.deepEqual(quality.lock.profiles, ["compat", "balanced", "high", "ultra", "photo"]);
  const entry = baseline.baselines.find((item) => item.id === "B038-rsh-038-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 42);
});

test("RSH-039 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const budgets = readJson("PERF-BUDGETS-MANIFEST.json");
  const accepted = current.accepted_units["RSH-039"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 43);
  assert.equal(accepted.validated_head_sha, "11d75d4e13316bacccbef5a2fdc3458cedebebff");
  assert.equal(accepted.merge_sha, "da690504d360a5b06f53783e9f86e0d7bf73cc01");
  assert.equal(accepted.tree_sha, "5555ead41e9eb263accaac1bb51bdcb02d71900b");
  assert.equal(accepted.workflow_run, 34707731150);
  assert.equal(accepted.post_merge_workflow_run, 34707925345);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-039"].merge_sha, accepted.merge_sha);
  assert.equal(budgets.lock.cache_max_age_s, 31536000);
  assert.equal(budgets.lock.draw_call_target, 80);
  const entry = baseline.baselines.find((item) => item.id === "B039-rsh-039-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 43);
});

test("RSH-040 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const leaks = readJson("LEAK-CYCLES-MANIFEST.json");
  const accepted = current.accepted_units["RSH-040"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 44);
  assert.equal(accepted.validated_head_sha, "c48cd3c4aef0551b20167a67be85da548cb04c31");
  assert.equal(accepted.merge_sha, "b24dea10f4a6ff186ee0c73e44a4fc693ad384f9");
  assert.equal(accepted.tree_sha, "f404cbf8e584346d707c6f25b68aa187bf738245");
  assert.equal(accepted.workflow_run, 34708531367);
  assert.equal(accepted.post_merge_workflow_run, 34709086445);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-040"].merge_sha, accepted.merge_sha);
  assert.equal(leaks.lock.enter_exit_cycles, 20);
  assert.equal(leaks.lock.context_loss_enforced, false);
  const entry = baseline.baselines.find((item) => item.id === "B040-rsh-040-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 44);
});

test("RSH-041 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const context = readJson("CONTEXT-LOSS-MANIFEST.json");
  const accepted = current.accepted_units["RSH-041"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 45);
  assert.equal(accepted.validated_head_sha, "33737ed5a6339f48ca4dcc2e0259302f5d4e3441");
  assert.equal(accepted.merge_sha, "e8129df3e1fa5b62720d11b794395fdc531eaddf");
  assert.equal(accepted.tree_sha, "fef0d3f728fae5c7e18b4c5ba8363eaec53ea463");
  assert.equal(accepted.workflow_run, 34741228839);
  assert.equal(accepted.post_merge_workflow_run, 34741441092);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-041"].merge_sha, accepted.merge_sha);
  assert.equal(context.lock.context_loss_cycles, 8);
  assert.equal(context.lock.context_loss_enforced, true);
  assert.equal(context.lock.soak_enforced, false);
  const entry = baseline.baselines.find((item) => item.id === "B041-rsh-041-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 45);
});

test("RSH-042 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const accepted = current.accepted_units["RSH-042"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 46);
  assert.equal(accepted.validated_head_sha, "040ff0031fecf81817eb5cf0babd9b3086d2c76b");
  assert.equal(accepted.merge_sha, "b7fb239daed34b5e14bf4760bde25ec86c0b656b");
  assert.equal(accepted.tree_sha, "7b606b14f43a29fa40a06d472deaa46724152c63");
  assert.equal(accepted.workflow_run, 34742472787);
  assert.equal(accepted.post_merge_workflow_run, 34743990139);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-042"].merge_sha, accepted.merge_sha);
  assert.equal(current.accepted_units["RSH-042"].soak_duration_s, 1800);
  assert.equal(current.accepted_units["RSH-042"].soak_enforced, true);
  assert.equal(current.accepted_units["RSH-042"].smoke_substitutes, false);
  const entry = baseline.baselines.find((item) => item.id === "B042-rsh-042-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 46);
});

test("RSH-043 becomes accepted on merge and consumes exactly one authorization", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const night = readJson("NIGHT-WEATHER-MANIFEST.json");
  ...
  const physics = readJson("PHYSICS-CALIBRATION-MANIFEST.json");
  const audio = readJson("AUDIO-HUD-INPUT-MANIFEST.json");
  const golden = readJson("AYALON-GOLDEN-MANIFEST.json");
  const freeze = readJson("AYALON-FREEZE-MANIFEST.json");
  const perf = readJson("PERF-INSTRUMENT-MANIFEST.json");
  const quality = readJson("QUALITY-PROFILES-MANIFEST.json");
  const budgets = readJson("PERF-BUDGETS-MANIFEST.json");
  ...
  const schema = readJson("SAVE-SCHEMA-MANIFEST.json");
  const device_matri = readJson("DEVICE-MATRIX-MANIFEST.json");
  assert.equal(queue.counts.accepted, 46);
  assert.equal(queue.counts.in_review, 0);
  assert.equal(queue.counts.eligible, 0);
  assert.equal(queue.counts.deferred, 21);
  assert.equal(queue.counts.remaining, 21);
  assert.equal(queue.queue_head.id, "RSH-047");
  ...
  assert.equal(current.last_transition.unit, "RSH-046");
  assert.equal(current.accepted_units["RSH-042"].state, "accepted");
  assert.equal(current.accepted_units["RSH-043"].state, "accepted");
  assert.equal(current.accepted_units["RSH-043"].device_matrix_enforced, true);
  assert.equal(current.accepted_units["RSH-043"].real_device_baseline_accepted, false);
  assert.equal(current.accepted_units["RSH-043"].webgl2_required, true);
  assert.equal(current.accepted_units["RSH-043"].webgpu_default, false);
  assert.equal(current.accepted_units["RSH-043"].target_browser_count, 6);
  assert.equal(current.accepted_units["RSH-043"].input_maps_unified, false);
  assert.equal(schema.current_schema.version, 3);
  assert.equal(schema.migration_graph.length, 3);
  assert.equal(baseline.working_state.unit, "RSH-046");
  assert.equal(baseline.working_state.state, "accepted_on_merge");
  assert.equal(queue.next_instruction_contract.authorization_remaining, 0);
  assert.equal(queue.next_instruction_contract.RSH_047_authorized, false);
  assert.equal(queue.next_after_acceptance.id, "RSH-047");
  assert.equal(daylight.daylight.look, "summer14");
  assert.equal(daylight.deferred_boundary.queue_head, "RSH-047");
  assert.equal(night.deferred_boundary.queue_head, "RSH-047");
  assert.equal(physics.deferred_boundary.queue_head, "RSH-047");
  assert.equal(audio.deferred_boundary.queue_head, "RSH-047");
  assert.equal(golden.lock.unique_count, 20);
  assert.equal(golden.lock.freeze_granted, false);
  assert.equal(golden.deferred_boundary.queue_head, "RSH-047");
  assert.equal(freeze.lock.freeze_granted, true);
  assert.equal(freeze.lock.source_count, 36);
  assert.equal(freeze.deferred_boundary.queue_head, "RSH-047");
  assert.equal(perf.lock.sample_cap, 120);
  assert.equal(perf.deferred_boundary.queue_head, "RSH-047");
  assert.equal(quality.lock.drop_p95_ms, 20);
  assert.equal(quality.deferred_boundary.queue_head, "RSH-047");
  assert.equal(budgets.lock.cache_max_age_s, 31536000);
  assert.equal(budgets.lock.draw_call_target, 80);
  assert.equal(budgets.deferred_boundary.queue_head, "RSH-047");
  assert.equal(leaks.lock.enter_exit_cycles, 20);
  assert.equal(leaks.deferred_boundary.queue_head, "RSH-047");
  assert.equal(context.lock.context_loss_cycles, 8);
  assert.equal(context.deferred_boundary.queue_head, "RSH-047");
  const soak = readJson("THIRTY-SOAK-MANIFEST.json");
  assert.equal(soak.lock.soak_duration_s, 1800);
  assert.equal(soak.lock.smoke_substitutes, false);
  assert.equal(soak.lock.soak_enforced, true);
  assert.equal(soak.deferred_boundary.queue_head, "RSH-047");
  assert.equal(device_matri.deferred_boundary.queue_head, "RSH-047");
  assert.equal(owner.unique_pack_approved, true);
  assert.equal(owner.freeze_granted, false);
});

test("RSH-044 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const maps = readJson("INPUT-MAPS-MANIFEST.json");
  const device = readJson("DEVICE-MATRIX-MANIFEST.json");
  const accepted = current.accepted_units["RSH-044"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 49);
  assert.equal(accepted.validated_head_sha, "fb75c1983e61aa0cdb10ee7d6dd3d02939826b62");
  assert.equal(accepted.merge_sha, "7ebe241496e9206efe1ad7a5068924cbc299ced2");
  assert.equal(accepted.tree_sha, "55be9d8c8c88e74e35a4d73ecabd725e293f0dce");
  assert.equal(accepted.workflow_run, 34746307729);
  assert.equal(accepted.post_merge_workflow_run, 34746455661);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-044"].merge_sha, accepted.merge_sha);
  assert.equal(accepted.input_maps_unified, true);
  assert.equal(accepted.rtl_scope_complete, false);
  assert.equal(maps.lock.input_maps_unified, true);
  assert.equal(maps.lock.pause_keyboard_only, true);
  assert.equal(maps.lock.rtl_scope_complete, false);
  assert.equal(maps.deferred_boundary.queue_head, "RSH-047");
  assert.equal(device.lock.input_maps_unified, false);
  const entry = baseline.baselines.find((item) => item.id === "B044-rsh-044-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 49);
});

test("RSH-045 is reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const scope = readJson("RTL-SCOPE-MANIFEST.json");
  const maps = readJson("INPUT-MAPS-MANIFEST.json");
  const accepted = current.accepted_units["RSH-045"];
  assert.equal(accepted.state, "accepted");
  assert.equal(accepted.pull_request, 50);
  assert.equal(accepted.validated_head_sha, "12056161771c7820213d0a5657d20ebce746e257");
  assert.equal(accepted.merge_sha, "97a3190306c59cc14aaffc659c7c499a8a95b7c9");
  assert.equal(accepted.tree_sha, "6f7d5c9d3e9fa89776d4e208f19002a18f4dd44c");
  assert.equal(accepted.workflow_run, 34747615853);
  assert.equal(accepted.post_merge_workflow_run, 34747812420);
  assert.equal(accepted.unresolved_review_threads, 0);
  assert.equal(queue.accepted["RSH-045"].merge_sha, accepted.merge_sha);
  assert.equal(accepted.rtl_scope_complete, true);
  assert.equal(accepted.arabic_copy_complete, false);
  assert.equal(accepted.onboarding_complete, false);
  assert.equal(accepted.html_lang_static_he, true);
  assert.equal(accepted.document_lang_synced, false);
  ...
  assert.equal(scope.deferred_boundary.queue_head, "RSH-047");
  assert.equal(maps.lock.rtl_scope_complete, false);
  const entry = baseline.baselines.find((item) => item.id === "B045-rsh-045-accepted");
  assert.equal(entry.commit_sha, accepted.merge_sha);
  assert.equal(entry.validated_head_sha, accepted.validated_head_sha);
  assert.equal(entry.pull_request, 50);
});

test("RSH-046 becomes accepted on merge and consumes exactly one authorization", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const onboarding = readJson("ONBOARDING-MANIFEST.json");
  const scope = readJson("RTL-SCOPE-MANIFEST.json");
  assert.equal(queue.counts.accepted, 46);
  assert.equal(queue.counts.deferred, 21);
  assert.equal(queue.counts.remaining, 21);
  ...
  assert.equal(queue.queue_head.state, "deferred_not_authorized");
  assert.equal(current.last_transition.unit, "RSH-046");
  assert.equal(current.accepted_units["RSH-045"].state, "accepted");
  assert.equal(current.accepted_units["RSH-046"].state, "accepted_on_merge");
  assert.equal(current.accepted_units["RSH-046"].onboarding_complete, true);
  assert.equal(current.accepted_units["RSH-046"].title_boot, true);
  assert.equal(current.accepted_units["RSH-046"].settings_panel, true);
  assert.equal(current.accepted_units["RSH-046"].save_recovery_ux, true);
  assert.equal(current.accepted_units["RSH-046"].first_run_wizard, false);
  assert.equal(current.accepted_units["RSH-046"].pwa_complete, false);
  assert.equal(current.accepted_units["RSH-046"].a11y_complete, false);
  assert.equal(onboarding.lock.onboarding_complete, true);
  assert.equal(onboarding.lock.first_run_wizard, false);
  assert.equal(onboarding.lock.boot_screen, "title");
  assert.equal(onboarding.lock.pwa_complete, false);
  assert.equal(onboarding.deferred_boundary.queue_head, "RSH-047");
  assert.equal(scope.lock.onboarding_complete, false);
  assert.equal(baseline.working_state.unit, "RSH-046");
  assert.equal(queue.next_instruction_contract.RSH_047_authorized, false);
});
