...
test("current state and queue agree on every post-merge program count", () => {
  ...
  const queue = readJson("QUEUE.json");
  assert.equal(current.state_semantics.effective_event, "merge_of_RSH_045_pull_request");
  assert.equal(queue.state_effective_on, "merge_of_RSH_045_pull_request");
...
test("the historical batch and all completed one-unit authorizations are closed", () => {
  ...
  assert.deepEqual(current.prior_single_unit_authorization.authorized_units, ["RSH-044"]);
  assert.equal(current.prior_single_unit_authorization.state, "consumed_on_RSH-044_merge");
  assert.deepEqual(current.single_unit_authorization.authorized_units, ["RSH-045"]);
  assert.equal(current.single_unit_authorization.completed_units, 1);
  assert.equal(current.single_unit_authorization.state, "consumed_on_RSH-045_merge");
  ...
  assert.equal(queue.state_rules["RSH-001–RSH-045"], "accepted");
  assert.deepEqual(queue.state_rules.eligible, []);
  assert.equal(queue.next_instruction_contract.current_action, "No unit is authorized; RSH-046 remains deferred until a new explicit owner instruction.");
  assert.equal(queue.next_instruction_contract.RSH_045_completed, true);
  assert.equal(queue.next_instruction_contract.RSH_045_authorization_consumed, true);
  assert.equal(queue.next_instruction_contract.RSH_046_authorized, false);
  assert.deepEqual(queue.policy.latest_single_unit_authorization.authorized_units, ["RSH-045"]);
  assert.equal(queue.policy.latest_single_unit_authorization.state, "consumed_on_RSH-045_merge");
});
...
test("RSH-043 becomes accepted on merge and consumes exactly one authorization", () => {
  ...
  assert.equal(queue.counts.accepted, 45);
  assert.equal(queue.counts.in_review, 0);
  assert.equal(queue.counts.eligible, 0);
  assert.equal(queue.counts.deferred, 22);
  assert.equal(queue.counts.remaining, 22);
  assert.equal(queue.queue_head.id, "RSH-046");
  ...
  assert.equal(current.last_transition.unit, "RSH-045");
  ...
  assert.equal(queue.next_instruction_contract.authorization_remaining, 0);
  assert.equal(queue.next_instruction_contract.RSH_046_authorized, false);
  assert.equal(queue.next_after_acceptance.id, "RSH-046");
  assert.equal(daylight.daylight.look, "summer14");
  assert.equal(daylight.deferred_boundary.queue_head, "RSH-046");
  assert.equal(night.deferred_boundary.queue_head, "RSH-046");
  assert.equal(physics.deferred_boundary.queue_head, "RSH-046");
  assert.equal(audio.deferred_boundary.queue_head, "RSH-046");
  assert.equal(golden.lock.unique_count, 20);
  assert.equal(golden.lock.freeze_granted, false);
  assert.equal(golden.deferred_boundary.queue_head, "RSH-046");
  assert.equal(freeze.lock.freeze_granted, true);
  assert.equal(freeze.lock.source_count, 36);
  assert.equal(freeze.deferred_boundary.queue_head, "RSH-046");
  assert.equal(perf.lock.sample_cap, 120);
  assert.equal(perf.deferred_boundary.queue_head, "RSH-046");
  assert.equal(quality.lock.drop_p95_ms, 20);
  assert.equal(quality.deferred_boundary.queue_head, "RSH-046");
  assert.equal(budgets.lock.cache_max_age_s, 31536000);
  assert.equal(budgets.lock.draw_call_target, 80);
  assert.equal(budgets.deferred_boundary.queue_head, "RSH-046");
  assert.equal(leaks.lock.enter_exit_cycles, 20);
  assert.equal(leaks.deferred_boundary.queue_head, "RSH-046");
  assert.equal(context.lock.context_loss_cycles, 8);
  assert.equal(context.deferred_boundary.queue_head, "RSH-046");
  ...
  assert.equal(soak.deferred_boundary.queue_head, "RSH-046");
  assert.equal(device_matri.deferred_boundary.queue_head, "RSH-046");
...
test("RSH-044 is reconciled to exact accepted evidence", () => {
  ...
  assert.equal(maps.deferred_boundary.queue_head, "RSH-046");
...
test("RSH-045 becomes accepted on merge and consumes exactly one authorization", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  ...
  assert.equal(queue.queue_head.state, "deferred_not_authorized");
  assert.equal(current.last_transition.unit, "RSH-045");
  assert.equal(current.accepted_units["RSH-044"].state, "accepted");
  assert.equal(current.accepted_units["RSH-045"].state, "accepted_on_merge");
