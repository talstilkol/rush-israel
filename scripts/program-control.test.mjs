...
test("current state and queue agree on every post-merge program count", () => {
  ...
  const queue = readJson("QUEUE.json");
  assert.equal(current.state_semantics.effective_event, "merge_of_RSH_036_pull_request");
  assert.equal(queue.state_effective_on, "merge_of_RSH_036_pull_request");
...
test("the historical batch and all completed one-unit authorizations are closed", () => {
  ...
  assert.deepEqual(current.prior_single_unit_authorization.authorized_units, ["RSH-035"]);
  assert.equal(current.prior_single_unit_authorization.state, "consumed_on_RSH-035_merge");
  assert.deepEqual(current.single_unit_authorization.authorized_units, ["RSH-036"]);
  assert.equal(current.single_unit_authorization.completed_units, 1);
  assert.equal(current.single_unit_authorization.state, "consumed_on_RSH-036_merge");
  ...
  assert.equal(queue.state_rules["RSH-001–RSH-036"], "accepted");
  assert.deepEqual(queue.state_rules.eligible, []);
  assert.equal(queue.next_instruction_contract.current_action, "No unit is authorized; RSH-037 remains deferred until a new explicit owner instruction.");
  assert.equal(queue.next_instruction_contract.RSH_036_completed, true);
  assert.equal(queue.next_instruction_contract.RSH_036_authorization_consumed, true);
  assert.equal(queue.next_instruction_contract.RSH_037_authorized, false);
  assert.deepEqual(queue.policy.latest_single_unit_authorization.authorized_units, ["RSH-036"]);
  assert.equal(queue.policy.latest_single_unit_authorization.state, "consumed_on_RSH-036_merge");
});
...
test("RSH-036 becomes accepted on merge and consumes exactly one authorization", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const night = readJson("NIGHT-WEATHER-MANIFEST.json");
  ...
  const schema = readJson("SAVE-SCHEMA-MANIFEST.json");
  assert.equal(queue.counts.accepted, 36);
  assert.equal(queue.counts.in_review, 0);
  assert.equal(queue.counts.eligible, 0);
  assert.equal(queue.counts.deferred, 31);
  assert.equal(queue.counts.remaining, 31);
  assert.equal(queue.queue_head.id, "RSH-037");
  ...
  assert.equal(current.last_transition.unit, "RSH-036");
  assert.equal(current.accepted_units["RSH-035"].state, "accepted_on_merge");
  assert.equal(current.accepted_units["RSH-036"].state, "accepted_on_merge");
  assert.equal(current.accepted_units["RSH-036"].source_count, 36);
  assert.equal(current.accepted_units["RSH-036"].freeze_granted, true);
  assert.equal(schema.current_schema.version, 3);
  assert.equal(schema.migration_graph.length, 3);
  ...
  assert.equal(physics.deferred_boundary.rsh_036_authorized, true);
  assert.equal(audio.deferred_boundary.queue_head, "RSH-037");
  assert.equal(audio.deferred_boundary.rsh_037_authorized, false);
  ...
  assert.equal(golden.lock.placeholders_are_unique_evidence, false);
  ...
  assert.equal(freeze.lock.gis_claim, false);
  assert.equal(freeze.lock.public_distribution, false);
  assert.equal(freeze.deferred_boundary.queue_head, "RSH-037");
  assert.equal(owner.unique_pack_approved, true);
  assert.equal(owner.freeze_granted, false);
  const accepted = current.accepted_units["RSH-035"];
  assert.equal(accepted.unique_count, 20);
  assert.equal(accepted.freeze_granted, false);
});
