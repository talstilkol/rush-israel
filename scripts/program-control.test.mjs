...
test("current state and queue agree on every post-merge program count", () => {
  ...
  const queue = readJson("QUEUE.json");
  assert.equal(current.state_semantics.effective_event, "merge_of_pull_request_15");
  assert.equal(queue.state_effective_on, "merge_of_pull_request_15");
...
test("the owner-bounded next-five batch has exactly three accepted units on merge", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  ...
  assert.equal(current.batch_authorization.completed_units, 3);
  assert.equal(queue.policy.active_bounded_batch.completed, 3);
  ...
  assert.equal(queue.next_instruction_contract.batch_authority_remaining, 2);
});

test("RSH-007 through RSH-011 are reconciled to exact accepted evidence", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
...
test("RSH-012 becomes accepted on merge and RSH-013 is eligible but not pre-created", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  assert.equal(queue.counts.accepted, 12);
  assert.equal(queue.counts.in_review, 0);
  assert.equal(queue.counts.eligible, 1);
  assert.equal(queue.counts.remaining, 55);
  assert.equal(queue.queue_head.id, "RSH-013");
  assert.equal(queue.queue_head.state, "eligible");
  ...
  assert.equal(current.last_transition.unit, "RSH-012");
  assert.equal(current.last_transition.pull_request, 15);
  assert.equal(current.last_transition.state, "accepted_on_merge");
  assert.equal(current.accepted_units["RSH-012"].state, "accepted_on_merge");
  assert.equal(baseline.working_state.unit, "RSH-012");
  assert.equal(baseline.working_state.state, "accepted_on_merge_of_PR_15");
  assert.equal(current.validation["RSH-013_precreated"], false);
  assert.equal(queue.next_instruction_contract["RSH-013_precreated"], false);
  assert.equal(queue.next_after_acceptance.id, "RSH-014");
});
...
test("RSH-012 metadata is explicit while public release remains blocked", () => {
  ...
  assert.equal(current.rsh_012_metadata.state, "accepted_on_merge");
});
