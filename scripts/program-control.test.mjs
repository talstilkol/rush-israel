...
test("current state and queue agree on every post-merge program count", () => {
  ...
  const queue = readJson("QUEUE.json");
  assert.equal(current.state_semantics.effective_event, "merge_of_RSH_041_pull_request");
  assert.equal(queue.state_effective_on, "merge_of_RSH_041_pull_request");
...
test("the historical batch and all completed one-unit authorizations are closed", () => {
  ...
  assert.deepEqual(current.prior_single_unit_authorization.authorized_units, ["RSH-040"]);
  assert.equal(current.prior_single_unit_authorization.state, "consumed_on_RSH-040_merge");
  assert.deepEqual(current.single_unit_authorization.authorized_units, ["RSH-041"]);
  assert.equal(current.single_unit_authorization.completed_units, 1);
  assert.equal(current.single_unit_authorization.state, "consumed_on_RSH-041_merge");
  ...
  assert.equal(queue.state_rules["RSH-001–RSH-041"], "accepted");
  assert.deepEqual(queue.state_rules.eligible, []);
  assert.equal(queue.next_instruction_contract.current_action, "No unit is authorized; RSH-042 remains deferred until a new explicit owner instruction.");
  assert.equal(queue.next_instruction_contract.RSH_041_completed, true);
  assert.equal(queue.next_instruction_contract.RSH_041_authorization_consumed, true);
  assert.equal(queue.next_instruction_contract.RSH_042_authorized, false);
  assert.deepEqual(queue.policy.latest_single_unit_authorization.authorized_units, ["RSH-041"]);
  assert.equal(queue.policy.latest_single_unit_authorization.state, "consumed_on_RSH-041_merge");
});
...
test("RSH-041 becomes accepted on merge and consumes exactly one authorization", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const night = readJson("NIGHT-WEATHER-MANIFEST.json");
  ...
  const schema = readJson("SAVE-SCHEMA-MANIFEST.json");
  assert.equal(queue.counts.accepted, 41);
  assert.equal(queue.counts.in_review, 0);
  assert.equal(queue.counts.eligible, 0);
  assert.equal(queue.counts.deferred, 26);
  assert.equal(queue.counts.remaining, 26);
  assert.equal(queue.queue_head.id, "RSH-042");
  ...
  assert.equal(current.last_transition.unit, "RSH-041");
  assert.equal(current.accepted_units["RSH-040"].state, "accepted");
  assert.equal(current.accepted_units["RSH-041"].state, "accepted_on_merge");
  assert.equal(current.accepted_units["RSH-041"].context_loss_cycles, 8);
  assert.equal(current.accepted_units["RSH-041"].context_loss_enforced, true);
  assert.equal(current.accepted_units["RSH-041"].soak_enforced, false);
  assert.equal(schema.current_schema.version, 3);
  assert.equal(schema.migration_graph.length, 3);
  assert.equal(baseline.working_state.unit, "RSH-041");
  assert.equal(baseline.working_state.state, "accepted_on_merge");
  assert.equal(queue.next_instruction_contract.authorization_remaining, 0);
  assert.equal(queue.next_instruction_contract.RSH_042_authorized, false);
  assert.equal(queue.next_after_acceptance.id, "RSH-042");
  assert.equal(daylight.daylight.look, "summer14");
  assert.equal(daylight.deferred_boundary.queue_head, "RSH-042");
  assert.equal(night.deferred_boundary.queue_head, "RSH-042");
  assert.equal(physics.deferred_boundary.queue_head, "RSH-042");
  assert.equal(physics.deferred_boundary.rsh_041_authorized, true);
  assert.equal(audio.deferred_boundary.queue_head, "RSH-042");
  assert.equal(audio.deferred_boundary.rsh_042_authorized, false);
  assert.equal(golden.lock.unique_count, 20);
  assert.equal(golden.lock.freeze_granted, false);
  assert.equal(golden.deferred_boundary.queue_head, "RSH-042");
  assert.equal(freeze.lock.freeze_granted, true);
  assert.equal(freeze.lock.source_count, 36);
  assert.equal(freeze.deferred_boundary.queue_head, "RSH-042");
  assert.equal(perf.lock.sample_cap, 120);
  assert.equal(perf.deferred_boundary.queue_head, "RSH-042");
  assert.equal(quality.lock.drop_p95_ms, 20);
  assert.equal(quality.deferred_boundary.queue_head, "RSH-042");
  assert.equal(budgets.lock.cache_max_age_s, 31536000);
  assert.equal(budgets.lock.draw_call_target, 80);
  assert.equal(budgets.deferred_boundary.queue_head, "RSH-042");
  assert.equal(leaks.lock.enter_exit_cycles, 20);
  assert.equal(leaks.lock.context_loss_enforced, false);
  assert.equal(leaks.deferred_boundary.queue_head, "RSH-042");
  ...
  assert.equal(context.deferred_boundary.queue_head, "RSH-042");
  assert.equal(owner.unique_pack_approved, true);
  assert.equal(owner.freeze_granted, false);
  const accepted = current.accepted_units["RSH-040"];
  assert.equal(accepted.enter_exit_cycles, 20);
  assert.equal(accepted.context_loss_enforced, false);
});
