...
test("current state and queue agree on every post-merge program count", () => {
  ...
  const queue = readJson("QUEUE.json");
  assert.equal(current.state_semantics.effective_event, "merge_of_RSH_033_pull_request");
  assert.equal(queue.state_effective_on, "merge_of_RSH_033_pull_request");
...
test("the historical batch and all completed one-unit authorizations are closed", () => {
  ...
  assert.deepEqual(current.prior_single_unit_authorization.authorized_units, ["RSH-032"]);
  assert.equal(current.prior_single_unit_authorization.state, "consumed_on_RSH-032_merge");
  assert.deepEqual(current.single_unit_authorization.authorized_units, ["RSH-033"]);
  assert.equal(current.single_unit_authorization.completed_units, 1);
  assert.equal(current.single_unit_authorization.state, "consumed_on_RSH-033_merge");
  ...
  assert.equal(queue.state_rules["RSH-001–RSH-033"], "accepted");
  assert.deepEqual(queue.state_rules.eligible, []);
  assert.equal(queue.next_instruction_contract.current_action, "No unit is authorized; RSH-034 remains deferred until a new explicit owner instruction.");
  assert.equal(queue.next_instruction_contract.RSH_033_completed, true);
  assert.equal(queue.next_instruction_contract.RSH_033_authorization_consumed, true);
  assert.equal(queue.next_instruction_contract.RSH_034_authorized, false);
  assert.deepEqual(queue.policy.latest_single_unit_authorization.authorized_units, ["RSH-033"]);
  assert.equal(queue.policy.latest_single_unit_authorization.state, "consumed_on_RSH-033_merge");
});
...
test("RSH-033 becomes accepted on merge and consumes exactly one authorization", () => {
  const current = readJson("CURRENT-STATE.json");
  const queue = readJson("QUEUE.json");
  const baseline = readJson("BASELINE-REGISTER.json");
  const night = readJson("NIGHT-WEATHER-MANIFEST.json");
  ...
  const schema = readJson("SAVE-SCHEMA-MANIFEST.json");
  assert.equal(queue.counts.accepted, 33);
  assert.equal(queue.counts.in_review, 0);
  assert.equal(queue.counts.eligible, 0);
  assert.equal(queue.counts.deferred, 34);
  assert.equal(queue.counts.remaining, 34);
  assert.equal(queue.queue_head.id, "RSH-034");
  ...
  assert.equal(current.last_transition.unit, "RSH-033");
  assert.equal(current.accepted_units["RSH-032"].state, "accepted");
  assert.equal(current.accepted_units["RSH-032"].merge_sha, "f8d055c86b80ba1a72555e41668e071f74b32536");
  assert.equal(current.accepted_units["RSH-032"].validated_head_sha, "2a826fd20cb2c45e13a322443c4d9c1186ee53a9");
  assert.equal(current.accepted_units["RSH-033"].state, "accepted_on_merge");
  assert.equal(schema.current_schema.version, 3);
  assert.equal(schema.migration_graph.length, 3);
  assert.equal(baseline.working_state.unit, "RSH-033");
  assert.equal(baseline.working_state.state, "accepted_on_merge");
  assert.equal(queue.next_instruction_contract.authorization_remaining, 0);
  assert.equal(queue.next_instruction_contract.RSH_034_authorized, false);
  assert.equal(queue.next_after_acceptance.id, "RSH-034");
  assert.equal(daylight.daylight.look, "summer14");
  assert.equal(daylight.deferred_boundary.queue_head, "RSH-034");
  assert.equal(night.deferred_boundary.queue_head, "RSH-034");
  ...
  assert.equal(physics.deferred_boundary.queue_head, "RSH-034");
  assert.equal(physics.deferred_boundary.rsh_034_authorized, false);
  const accepted = current.accepted_units["RSH-032"];
  assert.equal(accepted.pull_request, 35);
  assert.equal(accepted.workflow_run, 33713967588);
  assert.equal(accepted.post_merge_workflow_run, 33714396567);
