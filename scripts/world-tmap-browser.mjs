...
/** Leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of toneMapping vs outputColorSpace vs ColorManagement. */
export const TMAP_FRAMES = REGION_FRAMES;
export const TMAP_LAYERS = Object.freeze([
  'tone', 'space', 'mgmt', 'env', 'both', 'enc',
]);
export const CAPTURE_LAYERS = TMAP_LAYERS;
export const TONE_IDS = Object.freeze(['tone']);
export const SPACE_IDS = Object.freeze(['space']);
export const MGMT_IDS = Object.freeze(['mgmt']);
export const IBL_IDS = Object.freeze(['env']);
export const BOTH_IDS = Object.freeze(['both']);
export const ENC_IDS = Object.freeze(['enc']);
export const PRODUCT_HEMI_SKY_HEX = 0xa8c8e8;
export const PRODUCT_HEMI_NIGHT_HEX = 0x6a88b0;
export const TMAP_L2_MIN = 8;
export const TMAP_LUMA_MIN = 8;
export const TMAP_BLUE_MIN = 8;
export const TMAP_BAND_MIN = 0.02;
...
export function dominantTmap(layers) {
  if (!Array.isArray(layers) || layers.length !== TMAP_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotTmapMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== TMAP_LAYERS.length
    || TMAP_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !TMAP_LAYERS.includes(layer.id)
    ...
    || !frame.dominantTmap
    || !frame.dominant
    || !Array.isArray(frame.toneAxis)
    || !Array.isArray(frame.envAxis)
    || !Array.isArray(frame.bothAxis)
    || !Array.isArray(frame.spaceAxis)
    || !Array.isArray(frame.mgmtAxis)
    || !Array.isArray(frame.encAxis)
...
export function tmapBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-tmap-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { tone: 2083, space: 2083, mgmt: 2083, env: 2083, both: 2084, enc: 2083 }[id] ?? 1;
  const baselineL2 = 40;
  const baselineLuma = 40;
  const baselineBlue = 90;
  const l2 = id === 'tone' ? 18 : id === 'both' ? 20 : id === 'enc' ? 22 : id === 'env' ? 40 : 40;
  const lumaVal = 40;
  const blueVal = id === 'env' ? 70 : 90;
  const bandDelta = id === 'tone' ? -0.10 : id === 'both' ? -0.08 : id === 'enc' ? -0.05 : id === 'env' ? 0.0076 : 0;
  return {
...
function frame(id, extra = {}) {
  const spec = TMAP_FRAMES.find(row => row.id === id);
  ...
  const layers = TMAP_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantTmap: 'tone',
    contributingTmap: ['tone', 'enc'],
    toneAxis: ['tone'],
    envAxis: [],
    bothAxis: ['both'],
    spaceAxis: [],
...
export function worldTmapFixture(extra = {}) {
  return {
    ...
    rampsCount: 150, leftoverCount: 2082, toneCount: 2083, spaceCount: 2083, mgmtCount: 2083, envCount: 2083, bothCount: 2086, encCount: 2085, buildingsCount: 696, instancedCount: 47, groundCount: 1, remainingCount: 260,
    lumaTermsIncludeHemi: false, envIsCombined: false, bothIsCombined: false,
    toneSetsNoToneMapping: true, toneHidesLeftover: true, toneKeepsEnv: true, toneKeepsHemi: true, toneKeepsSpace: true, toneKeepsMgmt: true,
    spaceSetsLinear: true, spaceHidesLeftover: true, spaceKeepsEnv: true, spaceKeepsHemi: true, spaceKeepsTone: true, spaceKeepsMgmt: true,
    mgmtDisablesColorManagement: true, mgmtHidesLeftover: true, mgmtKeepsEnv: true, mgmtKeepsHemi: true, mgmtKeepsTone: true, mgmtKeepsSpace: true,
    envSetsGray: true, envHidesLeftover: true, envKeepsHemi: true, envKeepsTone: true, envKeepsSpace: true, envKeepsMgmt: true, envClearsHue: false,
    encSetsNoToneMapping: true, encSetsLinear: true, encDisablesMgmt: true, encHidesLeftover: true, encKeepsEnv: true, encKeepsHemi: true,
    bothSetsGray: true, bothSetsNoToneMapping: true, bothSetsLinear: true, bothDisablesMgmt: true, bothHidesLeftover: true, bothKeepsHemi: true,
    intensityClearsIbl: false,
    tmapDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-tmap-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: TMAP_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldTmapResults(r) {
  assert.ok(r && typeof r === 'object', 'world-tmap evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-tmap frames missing');
  assert.equal(leftoverIsNotTmapMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of toneMapping vs outputColorSpace vs ColorManagement mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'tmap isolation still combined with hemi');
  assert.equal(r.toneSetsNoToneMapping, true, 'tone isolation did not set NoToneMapping');
  assert.equal(r.toneHidesLeftover, true, 'tone isolation did not hide leftover empty-scene meshes');
  assert.equal(r.toneKeepsEnv, true, 'tone isolation clears scene.environment');
  assert.equal(r.toneKeepsHemi, true, 'hemi.color left product during tone isolation');
  assert.equal(r.toneKeepsSpace, true, 'outputColorSpace left product sRGB during tone isolation');
  assert.equal(r.toneKeepsMgmt, true, 'ColorManagement left enabled during tone isolation');
  assert.equal(r.spaceSetsLinear, true, 'space isolation did not set LinearSRGBColorSpace');
  assert.equal(r.spaceHidesLeftover, true, 'space isolation did not hide leftover empty-scene meshes');
  assert.equal(r.spaceKeepsEnv, true, 'space isolation clears scene.environment');
  assert.equal(r.spaceKeepsHemi, true, 'hemi.color left product during space isolation');
  assert.equal(r.spaceKeepsTone, true, 'toneMapping left product ACES during space isolation');
  assert.equal(r.spaceKeepsMgmt, true, 'ColorManagement left enabled during space isolation');
  assert.equal(r.mgmtDisablesColorManagement, true, 'mgmt isolation did not disable ColorManagement');
  assert.equal(r.mgmtHidesLeftover, true, 'mgmt isolation did not hide leftover empty-scene meshes');
  assert.equal(r.mgmtKeepsEnv, true, 'mgmt isolation clears scene.environment');
  assert.equal(r.mgmtKeepsHemi, true, 'hemi.color left product during mgmt isolation');
  assert.equal(r.mgmtKeepsTone, true, 'toneMapping left product ACES during mgmt isolation');
  assert.equal(r.mgmtKeepsSpace, true, 'outputColorSpace left product sRGB during mgmt isolation');
  ...
  assert.equal(r.envKeepsTone, true, 'toneMapping left product ACES during env isolation');
  assert.equal(r.envKeepsSpace, true, 'outputColorSpace left product sRGB during env isolation');
  assert.equal(r.envKeepsMgmt, true, 'ColorManagement left enabled during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.encSetsNoToneMapping, true, 'enc isolation did not set NoToneMapping');
  assert.equal(r.encSetsLinear, true, 'enc isolation did not set LinearSRGBColorSpace');
  assert.equal(r.encDisablesMgmt, true, 'enc isolation did not disable ColorManagement');
  assert.equal(r.encHidesLeftover, true, 'enc isolation did not hide leftover empty-scene meshes');
  assert.equal(r.encKeepsEnv, true, 'enc isolation clears scene.environment');
  assert.equal(r.encKeepsHemi, true, 'hemi.color left product during enc isolation');
  assert.equal(r.bothSetsGray, true, 'both isolation did not swap gray cubemap');
  assert.equal(r.bothSetsNoToneMapping, true, 'both isolation did not set NoToneMapping');
  assert.equal(r.bothSetsLinear, true, 'both isolation did not set LinearSRGBColorSpace');
  assert.equal(r.bothDisablesMgmt, true, 'both isolation did not disable ColorManagement');
  ...
  assert.equal(r.intensityClearsIbl, false, 'tmap isolation clears scene.environment via sun');
  assert.equal(r.tmapDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after tmap restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during tmap probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during tmap probe');
  ...
  assert.ok(r.toneCount >= 151, 'toneMapping isolation missing');
  assert.ok(r.spaceCount >= 151, 'outputColorSpace isolation missing');
  assert.ok(r.mgmtCount >= 151, 'ColorManagement isolation missing');
  assert.ok(r.encCount >= 151, 'combined encode isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both encode+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during tmap probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during tmap probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during tmap probe');
  ...
  assert.equal(tmapBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of TMAP_FRAMES) {
    ...
    if (!row.dominantTmap || !row.dominant || row.layers.length !== TMAP_LAYERS.length
      || TMAP_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.toneAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.bothAxis)
      || !Array.isArray(row.spaceAxis) || !Array.isArray(row.mgmtAxis) || !Array.isArray(row.encAxis)
      || !Number.isFinite(row.leftoverBaselinePct)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || TMAP_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of toneMapping vs outputColorSpace vs ColorManagement attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present tmap axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'toneMapping vs outputColorSpace vs ColorManagement independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'tmap-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldTmap(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldTmapResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldTmap(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let toneSetsNoToneMapping = false;
        let toneHidesLeftover = false;
        let toneKeepsEnv = true;
        let toneKeepsHemi = true;
        let toneKeepsSpace = true;
        let toneKeepsMgmt = true;
        let spaceSetsLinear = false;
        let spaceHidesLeftover = false;
        let spaceKeepsEnv = true;
        let spaceKeepsHemi = true;
        let spaceKeepsTone = true;
        let spaceKeepsMgmt = true;
        let mgmtDisablesColorManagement = false;
        let mgmtHidesLeftover = false;
        let mgmtKeepsEnv = true;
        let mgmtKeepsHemi = true;
        let mgmtKeepsTone = true;
        let mgmtKeepsSpace = true;
        let envSetsGray = false;
        let envHidesLeftover = false;
        let envKeepsHemi = true;
        let envKeepsTone = true;
        let envKeepsSpace = true;
        let envKeepsMgmt = true;
        let encSetsNoToneMapping = false;
        let encSetsLinear = false;
        let encDisablesMgmt = false;
        let encHidesLeftover = false;
        let encKeepsEnv = true;
        let encKeepsHemi = true;
        let bothSetsGray = false;
        let bothSetsNoToneMapping = false;
        let bothSetsLinear = false;
        let bothDisablesMgmt = false;
        ...
        const setTone = (value) => {
          const prev = engine.renderer.toneMapping;
          engine.renderer.toneMapping = value;
          return [() => { engine.renderer.toneMapping = prev; }];
        };
        const setSpace = (value) => {
          const prev = engine.renderer.outputColorSpace;
          engine.renderer.outputColorSpace = value;
          return [() => { engine.renderer.outputColorSpace = prev; }];
        };
        const setMgmt = (value) => {
          const prev = THREE.ColorManagement.enabled;
          THREE.ColorManagement.enabled = value;
          return [() => { THREE.ColorManagement.enabled = prev; }];
        };
        const isolate = {
          tone() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevSpace = engine.renderer.outputColorSpace;
            const prevMgmt = THREE.ColorManagement.enabled;
            const restorers = [...hideMeshes(leftoverOf()), ...setTone(THREE.NoToneMapping)];
            if (engine.renderer.toneMapping === THREE.NoToneMapping) toneSetsNoToneMapping = true;
            if (leftoverHidden()) toneHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) toneKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) toneKeepsHemi = false;
            if (engine.renderer.outputColorSpace !== prevSpace) toneKeepsSpace = false;
            if (THREE.ColorManagement.enabled !== prevMgmt) toneKeepsMgmt = false;
            return restorers;
          },
          space() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevTone = engine.renderer.toneMapping;
            const prevMgmt = THREE.ColorManagement.enabled;
            const restorers = [...hideMeshes(leftoverOf()), ...setSpace(THREE.LinearSRGBColorSpace)];
            if (engine.renderer.outputColorSpace === THREE.LinearSRGBColorSpace) spaceSetsLinear = true;
            if (leftoverHidden()) spaceHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) spaceKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) spaceKeepsHemi = false;
            if (engine.renderer.toneMapping !== prevTone) spaceKeepsTone = false;
            if (THREE.ColorManagement.enabled !== prevMgmt) spaceKeepsMgmt = false;
            return restorers;
          },
          mgmt() {
            const prevHemi = hemiRgb();
            ...
            const prevTone = engine.renderer.toneMapping;
            const prevSpace = engine.renderer.outputColorSpace;
            const restorers = [...hideMeshes(leftoverOf()), ...setMgmt(false)];
            if (THREE.ColorManagement.enabled === false) mgmtDisablesColorManagement = true;
            if (leftoverHidden()) mgmtHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) mgmtKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) mgmtKeepsHemi = false;
            if (engine.renderer.toneMapping !== prevTone) mgmtKeepsTone = false;
            if (engine.renderer.outputColorSpace !== prevSpace) mgmtKeepsSpace = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.toneMapping !== prevTone) envKeepsTone = false;
            if (engine.renderer.outputColorSpace !== prevSpace) envKeepsSpace = false;
            if (THREE.ColorManagement.enabled !== prevMgmt) envKeepsMgmt = false;
            return restorers;
          },
          enc() {
            const prevHemi = hemiRgb();
            ...
            const restorers = [
              ...
              ...setTone(THREE.NoToneMapping),
              ...setSpace(THREE.LinearSRGBColorSpace),
              ...setMgmt(false),
            ];
            if (engine.renderer.toneMapping === THREE.NoToneMapping) encSetsNoToneMapping = true;
            if (engine.renderer.outputColorSpace === THREE.LinearSRGBColorSpace) encSetsLinear = true;
            if (THREE.ColorManagement.enabled === false) encDisablesMgmt = true;
            if (leftoverHidden()) encHidesLeftover = true;
            if (engine.scene.environment !== prevEnv) encKeepsEnv = false;
            if (hemiRgb().some((rgb, i) => rgb !== prevHemi[i])) encKeepsHemi = false;
            return restorers;
          },
          both() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.toneMapping === THREE.NoToneMapping) bothSetsNoToneMapping = true;
            if (engine.renderer.outputColorSpace === THREE.LinearSRGBColorSpace) bothSetsLinear = true;
            if (THREE.ColorManagement.enabled === false) bothDisablesMgmt = true;
        ...
        const bothIsCombined = !toneKeepsHemi || !spaceKeepsHemi || !mgmtKeepsHemi || !envKeepsHemi || !encKeepsHemi || !bothKeepsHemi;
        const toneRestorers = isolate.tone();
        toneRestorers.forEach(restore => restore());
        const spaceRestorers = isolate.space();
        spaceRestorers.forEach(restore => restore());
        const mgmtRestorers = isolate.mgmt();
        mgmtRestorers.forEach(restore => restore());
        const encRestorers = isolate.enc();
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = TMAP_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantTmap: dominantTmap(layers),
        contributingTmap: layers.filter(layer => layer.bandDelta <= -TMAP_BAND_MIN).map(layer => layer.id),
        toneAxis: layers.filter(layer => TONE_IDS.includes(layer.id) && layer.bandDelta <= -TMAP_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -TMAP_BAND_MIN || layer.blueDelta <= -TMAP_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -TMAP_BAND_MIN).map(layer => layer.id),
        spaceAxis: layers.filter(layer => SPACE_IDS.includes(layer.id) && layer.bandDelta <= -TMAP_BAND_MIN).map(layer => layer.id),
        mgmtAxis: layers.filter(layer => MGMT_IDS.includes(layer.id) && layer.bandDelta <= -TMAP_BAND_MIN).map(layer => layer.id),
        encAxis: layers.filter(layer => ENC_IDS.includes(layer.id) && layer.bandDelta <= -TMAP_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      toneSetsNoToneMapping: report.toneSetsNoToneMapping,
      toneHidesLeftover: report.toneHidesLeftover,
      toneKeepsEnv: report.toneKeepsEnv,
      toneKeepsHemi: report.toneKeepsHemi,
      toneKeepsSpace: report.toneKeepsSpace,
      toneKeepsMgmt: report.toneKeepsMgmt,
      spaceSetsLinear: report.spaceSetsLinear,
      spaceHidesLeftover: report.spaceHidesLeftover,
      spaceKeepsEnv: report.spaceKeepsEnv,
      spaceKeepsHemi: report.spaceKeepsHemi,
      spaceKeepsTone: report.spaceKeepsTone,
      spaceKeepsMgmt: report.spaceKeepsMgmt,
      mgmtDisablesColorManagement: report.mgmtDisablesColorManagement,
      mgmtHidesLeftover: report.mgmtHidesLeftover,
      mgmtKeepsEnv: report.mgmtKeepsEnv,
      mgmtKeepsHemi: report.mgmtKeepsHemi,
      mgmtKeepsTone: report.mgmtKeepsTone,
      mgmtKeepsSpace: report.mgmtKeepsSpace,
      envSetsGray: report.envSetsGray,
      envHidesLeftover: report.envHidesLeftover,
      envKeepsHemi: report.envKeepsHemi,
      envKeepsTone: report.envKeepsTone,
      envKeepsSpace: report.envKeepsSpace,
      envKeepsMgmt: report.envKeepsMgmt,
      envClearsHue: report.envClearsHue,
      encSetsNoToneMapping: report.encSetsNoToneMapping,
      encSetsLinear: report.encSetsLinear,
      encDisablesMgmt: report.encDisablesMgmt,
      encHidesLeftover: report.encHidesLeftover,
      encKeepsEnv: report.encKeepsEnv,
      encKeepsHemi: report.encKeepsHemi,
      bothSetsGray: report.bothSetsGray,
      bothSetsNoToneMapping: report.bothSetsNoToneMapping,
      bothSetsLinear: report.bothSetsLinear,
      bothDisablesMgmt: report.bothDisablesMgmt,
      bothHidesLeftover: report.bothHidesLeftover,
      bothKeepsHemi: report.bothKeepsHemi,
      intensityClearsIbl: report.intensityClearsIbl,
      tmapDeltasUseEmptyBaseline: report.tmapDeltasUseEmptyBaseline,
      ...
      toneCount: report.toneCount,
      spaceCount: report.spaceCount,
      mgmtCount: report.mgmtCount,
      encCount: report.encCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-tmap-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldTmap(browser, url) {
  const report = await measureWorldTmap(browser, url);
  const out = process.env.WORLD_TMAP_OUTPUT ?? fromRoot('artifacts', 'world-tmap');
  const results = await retainWorldTmap(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldTmap(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
