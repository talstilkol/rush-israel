...
/** Combined leftover gray cubemap vs IBL-off is not AmbientLight vs HemisphereLight vs RectAreaLight attribution. */
export const AMB_FRAMES = REGION_FRAMES;
export const AMB_LAYERS = Object.freeze([
  'amb', 'hemi', 'rect', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['amb']);
export const HEMI_IDS = Object.freeze(['hemi']);
export const RECT_IDS = Object.freeze(['rect']);
export const HUE_IDS = Object.freeze(['hue']);
export const GRAY_BASELINE_IDS = Object.freeze(['amb', 'hemi', 'rect']);
export const AMB_L2_MIN = 8;
export const AMB_LUMA_MIN = 8;
export const AMB_BLUE_MIN = 8;
...
export function dominantAmb(layers) {
  if (!Array.isArray(layers) || layers.length !== AMB_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function recvIsNotAmbMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== AMB_LAYERS.length
    || AMB_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !AMB_LAYERS.includes(layer.id)
    ...
    || !Array.isArray(frame.ambAxis)
    || !Array.isArray(frame.hemiAxis)
    || !Array.isArray(frame.rectAxis)
...
export function ambBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-amb-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { amb: 1, hemi: 1, rect: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'rect' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'rect' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = AMB_FRAMES.find(row => row.id === id);
  ...
  const layers = AMB_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantAmb: 'hue',
    contributingAmb: ['rect', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    ambAxis: [],
    hemiAxis: [],
    rectAxis: ['rect'],
...
export function worldAmbFixture(extra = {}) {
  return {
    ...
    ambCount: 1, hemiCount: 1, rectCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, ambIsCombined: false, ambClearsOff: false,
    ambClearsHue: false, hemiClearsAmb: false, hueClearsOff: false, intensityClearsIbl: false,
    ambDeltasUseGrayBaseline: true,
    ambZerosAmbient: true, hemiZerosHemi: true, rectIsolationRan: true,
    probeKeepsLightProbe: true, basicKeepsBasicEnvMap: true, shaderKeepsRoadCompile: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-amb-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: AMB_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldAmbResults(r) {
  assert.ok(r && typeof r === 'object', 'world-amb evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-amb frames missing');
  assert.equal(recvIsNotAmbMismatch(r), false, 'combined leftover gray cubemap vs IBL-off is not AmbientLight vs HemisphereLight vs RectAreaLight mismatch');
  ...
  assert.equal(r.ambIsCombined, false, 'amb isolation still combined with hemi or disc');
  assert.equal(r.ambClearsOff, false, 'amb isolation clears IBL-off');
  assert.equal(r.ambClearsHue, false, 'amb isolation clears gray cubemap hue');
  assert.equal(r.hemiClearsAmb, false, 'hemi isolation uses AmbientLight intensity');
  ...
  assert.equal(r.ambDeltasUseGrayBaseline, true, 'amb vs hemi vs rect still uses product present as baseline');
  assert.equal(r.ambZerosAmbient, true, 'AmbientLight intensity stays product during amb isolation');
  assert.equal(r.hemiZerosHemi, true, 'HemisphereLight intensity stays product during hemi isolation');
  assert.equal(r.rectIsolationRan, true, 'RectAreaLight traverse did not run');
  assert.equal(r.probeKeepsLightProbe, true, 'LightProbe intensity left product during amb probe');
  assert.equal(r.basicKeepsBasicEnvMap, true, 'MeshBasic envMap left product during amb probe');
  assert.equal(r.shaderKeepsRoadCompile, true, 'road-shader onBeforeCompile left bound during amb probe');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.ambCount >= 1, 'AmbientLight isolation missing');
  assert.ok(r.hemiCount >= 1, 'HemisphereLight isolation missing');
  assert.ok(r.rectCount >= 1, 'RectAreaLight isolation missing');
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during amb probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during amb probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during amb probe');
  ...
  assert.equal(ambBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of AMB_FRAMES) {
    ...
    if (!row.dominantAmb || !row.dominant || row.layers.length !== AMB_LAYERS.length
      || AMB_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.ambAxis) || !Array.isArray(row.hemiAxis)
      || !Array.isArray(row.rectAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || AMB_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover gray cubemap vs IBL-off is not AmbientLight vs HemisphereLight vs RectAreaLight attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present amb axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'AmbientLight vs HemisphereLight vs RectAreaLight independently of LightProbe vs MeshBasic envMap vs custom road-shader env vs 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'amb-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldAmb(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldAmbResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldAmb(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let ambZerosAmbient = false;
        let hemiZerosHemi = false;
        let rectIsolationRan = false;
        ...
        const zeroLights = (pred, flag) => {
          const restorers = [];
          engine.scene.traverse(object => {
            if (!pred(object)) return;
            const intensity = object.intensity;
            object.intensity = 0;
            if (object.intensity === 0) flag();
            restorers.push(() => { object.intensity = intensity; });
          });
          return restorers;
        };
        const isolate = {
          amb() {
            ...
            const restorers = zeroLights(object => object.isAmbientLight, () => { ambZerosAmbient = true; });
            return [() => { restorers.forEach(restore => restore()); restoreEnv(); }];
          },
          hemi() {
            const [restoreEnv] = swapEnv(bakeGray());
            const restorers = zeroLights(object => object.isHemisphereLight, () => { hemiZerosHemi = true; });
            return [() => { restorers.forEach(restore => restore()); restoreEnv(); }];
          },
          rect() {
            const [restoreEnv] = swapEnv(bakeGray());
            const restorers = zeroLights(object => object.isRectAreaLight, () => {});
            rectIsolationRan = true;
            return [() => { restorers.forEach(restore => restore()); restoreEnv(); }];
          },
          hue() {
        ...
        const basicMats = materialsMatching((_, mat) => !!mat?.isMeshBasicMaterial);
        const basicBefore = basicMats.map(mat => mat.envMap);
        const shaderMats = materialsMatching((_, mat) => isRoadMaterial(mat));
        const shaderBefore = shaderMats.map(mat => mat.onBeforeCompile);
        const ambRestorers = isolate.amb();
        const ambTex = engine.scene.environment;
        const ambIsCombined = ambRestorers.length !== 1;
        const ambClearsOff = ambTex == null;
        const ambClearsHue = ambTex === hueProbe.texture;
        ambRestorers.forEach(restore => restore());
        const hemiRestorers = isolate.hemi();
        const hemiClearsAmb = engine.scene.environment == null
          || engine.scene.environment === ambTex;
        ...
        const basicKeepsBasicEnvMap = basicMats.every((mat, i) => mat.envMap === basicBefore[i]);
        const shaderKeepsRoadCompile = shaderMats.every((mat, i) => mat.onBeforeCompile === shaderBefore[i]);
        ...
        return {
          ...
          basicKeepsBasicEnvMap,
          shaderKeepsRoadCompile,
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = AMB_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantAmb: dominantAmb(layers),
        contributingAmb: layers.filter(layer => layer.l2Delta <= -AMB_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -AMB_LUMA_MIN).map(layer => layer.id),
        ambAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -AMB_BLUE_MIN).map(layer => layer.id),
        hemiAxis: layers.filter(layer => HEMI_IDS.includes(layer.id) && (layer.l2Delta <= -AMB_L2_MIN || layer.blueDelta <= -AMB_BLUE_MIN)).map(layer => layer.id),
        rectAxis: layers.filter(layer => RECT_IDS.includes(layer.id) && layer.blueDelta <= -AMB_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -AMB_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      ambIsCombined: report.ambIsCombined,
      ambClearsOff: report.ambClearsOff,
      ambClearsHue: report.ambClearsHue,
      hemiClearsAmb: report.hemiClearsAmb,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      ambDeltasUseGrayBaseline: report.ambDeltasUseGrayBaseline,
      ambZerosAmbient: report.ambZerosAmbient,
      hemiZerosHemi: report.hemiZerosHemi,
      rectIsolationRan: report.rectIsolationRan,
      probeKeepsLightProbe: report.probeKeepsLightProbe,
      basicKeepsBasicEnvMap: report.basicKeepsBasicEnvMap,
      shaderKeepsRoadCompile: report.shaderKeepsRoadCompile,
      ...
      ambCount: report.ambCount,
      hemiCount: report.hemiCount,
      rectCount: report.rectCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-amb-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldAmb(browser, url) {
  const report = await measureWorldAmb(browser, url);
  const out = process.env.WORLD_AMB_OUTPUT ?? fromRoot('artifacts', 'world-amb');
  const results = await retainWorldAmb(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldAmb(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
