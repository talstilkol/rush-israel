...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of shadowMap.enabled vs xr.cameraAutoUpdate vs debug.onShaderError. */
export const SXO_FRAMES = REGION_FRAMES;
export const SXO_LAYERS = Object.freeze([
  'lights', 'debug', 'xr', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = SXO_LAYERS;
...
export const SXO_L2_MIN = 8;
export const SXO_LUMA_MIN = 8;
export const SXO_BLUE_MIN = 8;
export const SXO_BAND_MIN = 0.02;
...
export function dominantSxo(layers) {
  if (!Array.isArray(layers) || layers.length !== SXO_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotSxoMismatch(report) {
  if (!report?.frames?.length) return true;
  if (report.protocol !== 'world-sxo-buffer-not-original-golden') return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== SXO_LAYERS.length
    || SXO_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !SXO_LAYERS.includes(layer.id)
...
export function sxoBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-sxo-buffer-not-original-golden'
...
function frame(id, extra = {}) {
  const spec = SXO_FRAMES.find(row => row.id === id);
  ...
  const layers = SXO_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantSxo: 'lights',
    contributingSxo: ['lights', 'pack'],
    lightsAxis: ['lights'],
    envAxis: [],
    bothAxis: ['both'],
    debugAxis: [],
...
export function worldSxoFixture(extra = {}) {
  return {
    ...
    sxoDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-sxo-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: SXO_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldSxoResults(r) {
  assert.ok(r && typeof r === 'object', 'world-sxo evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-sxo frames missing');
  assert.equal(leftoverIsNotSxoMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of shadowMap.enabled vs xr.cameraAutoUpdate vs debug.onShaderError mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'sxo isolation still combined with hemi');
  assert.equal(r.lightsClears, true, 'type isolation did not force shadowMap.enabled off');
  ...
  assert.equal(r.lightsKeepsDebug, true, 'xr.cameraAutoUpdate left product during type isolation');
  assert.equal(r.lightsKeepsXr, true, 'debug.onShaderError left product during type isolation');
  assert.equal(r.debugClears, true, 'needs isolation did not force xr.cameraAutoUpdate off');
  ...
  assert.equal(r.debugKeepsLights, true, 'shadowMap.enabled left product during needs isolation');
  assert.equal(r.debugKeepsXr, true, 'debug.onShaderError left product during needs isolation');
  assert.equal(r.xrClears, true, 'clip isolation did not force debug.onShaderError');
  ...
  assert.equal(r.xrKeepsLights, true, 'shadowMap.enabled left product during clip isolation');
  assert.equal(r.xrKeepsDebug, true, 'xr.cameraAutoUpdate left product during clip isolation');
  ...
  assert.equal(r.envKeepsLights, true, 'shadowMap.enabled left product during env isolation');
  assert.equal(r.envKeepsDebug, true, 'xr.cameraAutoUpdate left product during env isolation');
  assert.equal(r.envKeepsXr, true, 'debug.onShaderError left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsLights, true, 'pack isolation did not force shadowMap.enabled off');
  assert.equal(r.packClearsDebug, true, 'pack isolation did not force xr.cameraAutoUpdate off');
  assert.equal(r.packClearsXr, true, 'pack isolation did not force debug.onShaderError');
  ...
  assert.equal(r.bothClearsLights, true, 'both isolation did not force shadowMap.enabled off');
  assert.equal(r.bothClearsDebug, true, 'both isolation did not force xr.cameraAutoUpdate off');
  assert.equal(r.bothClearsXr, true, 'both isolation did not force debug.onShaderError');
  ...
  assert.equal(r.intensityClearsIbl, false, 'sxo isolation clears scene.environment via shadowMap.enabled');
  assert.equal(r.sxoDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after tnc restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during sxo probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during sxo probe');
  ...
  assert.ok(r.packCount >= 151, 'combined shadowMap.enabled-xr.cameraAutoUpdate-debug.onShaderError isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both shadowMap.enabled-xr.cameraAutoUpdate-debug.onShaderError+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during sxo probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during sxo probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during sxo probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during sxo probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during sxo probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during sxo probe');
  ...
  assert.equal(sxoBufferIsNotOriginalGolden(r), false, 'ldx buffer is not original-golden');
  ...
  for (const spec of SXO_FRAMES) {
    ...
    if (!row.dominantSxo || !row.dominant || row.layers.length !== SXO_LAYERS.length
      || SXO_LAYERS.some(id => !byId[id])
      ...
      || SXO_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of shadowMap.enabled vs xr.cameraAutoUpdate vs debug.onShaderError attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present sxo axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'shadowMap.enabled vs xr.cameraAutoUpdate vs debug.onShaderError independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'sxo-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldSxo(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldSxoResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldSxo(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const setDebug = () => {
          const prev = engine.renderer.xr.cameraAutoUpdate;
          engine.renderer.xr.cameraAutoUpdate = false;
          return [() => { engine.renderer.xr.cameraAutoUpdate = prev; }];
        };
        const setXr = () => {
          const prev = engine.renderer.debug.onShaderError;
          engine.renderer.debug.onShaderError = () => {};
          return [() => { engine.renderer.debug.onShaderError = prev; }];
        };
        const isolate = {
          lights() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevNeeds = engine.renderer.xr.cameraAutoUpdate;
            const prevClip = engine.renderer.debug.onShaderError;
            ...
            if (engine.renderer.xr.cameraAutoUpdate !== prevNeeds) lightsKeepsDebug = false;
            if (engine.renderer.debug.onShaderError !== prevClip) lightsKeepsXr = false;
            return restorers;
          },
          debug() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.debug.onShaderError !== prevClip) debugKeepsXr = false;
            return restorers;
          },
          xr() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.xr.cameraAutoUpdate !== prevNeeds) xrKeepsDebug = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.xr.cameraAutoUpdate !== prevNeeds) envKeepsDebug = false;
            if (engine.renderer.debug.onShaderError !== prevClip) envKeepsXr = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = SXO_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantSxo: dominantSxo(layers),
        contributingSxo: layers.filter(layer => layer.bandDelta <= -SXO_BAND_MIN).map(layer => layer.id),
        lightsAxis: layers.filter(layer => LIGHTS_IDS.includes(layer.id) && layer.bandDelta <= -SXO_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -SXO_BAND_MIN || layer.blueDelta <= -SXO_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -SXO_BAND_MIN).map(layer => layer.id),
        debugAxis: layers.filter(layer => DEBUG_IDS.includes(layer.id) && layer.bandDelta <= -SXO_BAND_MIN).map(layer => layer.id),
        xrAxis: layers.filter(layer => XR_IDS.includes(layer.id) && layer.bandDelta <= -SXO_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -SXO_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      sxoDeltasUseEmptyBaseline: report.sxoDeltasUseEmptyBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-sxo-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldSxo(browser, url) {
  const report = await measureWorldSxo(browser, url);
  const out = process.env.WORLD_SXO_OUTPUT ?? fromRoot('artifacts', 'world-sxo');
  const results = await retainWorldSxo(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldSxo(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
