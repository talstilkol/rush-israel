...
/** Combined scene.environment blue is not texture vs environmentIntensity attribution. */
export const GAIN_FRAMES = REGION_FRAMES;
export const GAIN_LAYERS = Object.freeze([
  'texture', 'gain', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['texture']);
export const GAIN_IDS = Object.freeze(['gain']);
export const GAIN_L2_MIN = 8;
export const GAIN_LUMA_MIN = 8;
export const GAIN_BLUE_MIN = 8;
...
export function dominantGain(layers) {
  if (!Array.isArray(layers) || layers.length !== GAIN_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function iblIsNotGainMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== GAIN_LAYERS.length
    || GAIN_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !GAIN_LAYERS.includes(layer.id)
...
export function gainBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-gain-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { texture: 1, gain: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'texture' ? 50 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'texture' ? 90 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = GAIN_FRAMES.find(row => row.id === id);
  ...
  const layers = GAIN_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantGain: 'texture',
    contributingGain: ['texture', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: ['texture'],
    gainAxis: [],
    ...extra,
...
export function worldGainFixture(extra = {}) {
  return {
    ...
    textureCount: 1, gainCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, textureIsCombined: false, gainClearsTexture: false, textureClearsGain: false,
    intensityClearsTexture: false, textureClearsIntensity: false,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-gain-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: GAIN_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldGainResults(r) {
  assert.ok(r && typeof r === 'object', 'world-gain evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-gain frames missing');
  assert.equal(iblIsNotGainMismatch(r), false, 'combined scene.environment blue is not texture vs environmentIntensity mismatch');
  ...
  assert.equal(r.textureIsCombined, false, 'texture isolation still combined with environmentIntensity');
  assert.equal(r.gainClearsTexture, false, 'gain isolation clears scene.environment');
  assert.equal(r.textureClearsGain, false, 'texture isolation clears environmentIntensity');
  assert.equal(r.intensityClearsTexture, false, 'intensity isolation clears scene.environment');
  assert.equal(r.textureClearsIntensity, false, 'texture isolation clears intensity');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.textureCount >= 1, 'scene environment texture missing');
  assert.ok(r.gainCount >= 1, 'scene environmentIntensity missing');
  ...
  assert.equal(gainBufferIsNotOriginalGolden(r), false, 'gain buffer is not original-golden');
  ...
  let cameraFail = 0, mismatchFail = 0, gainFail = 0;
  for (const spec of GAIN_FRAMES) {
    ...
    if (!row.dominantGain || !row.dominant || row.layers.length !== GAIN_LAYERS.length
      || GAIN_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.gainAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || GAIN_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) gainFail += 1;
  }
  return [
    {
      case: 'combined scene.environment blue is not texture vs environmentIntensity attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present gain axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'texture/gain and intensity/fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
      status: gainFail ? 'failed' : 'passed',
      probes: 4,
      failures: gainFail,
    },
    {
    ...
    {
      case: 'gain-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldGain(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldGainResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldGain(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex }) => {
      ...
      try {
        ...
        const isolate = {
          ...
          gain() {
            const prev = engine.scene.environmentIntensity;
            engine.scene.environmentIntensity = 0;
            return [() => { engine.scene.environmentIntensity = prev; }];
          },
          intensity() {
        ...
        const textureRestorers = isolate.texture();
        const textureIsCombined = textureRestorers.length !== 1;
        const textureClearsGain = engine.scene.environmentIntensity !== gainBefore;
        const textureClearsIntensity = sun.intensity !== intensityBefore;
        textureRestorers.forEach(restore => restore());
        const gainRestorers = isolate.gain();
        const gainClearsTexture = engine.scene.environment !== environmentBefore;
        gainRestorers.forEach(restore => restore());
        const intensityRestorers = isolate.intensity();
        const intensityClearsTexture = engine.scene.environment !== environmentBefore;
        ...
        return {
          ...
          gainClearsTexture,
          textureClearsGain,
          intensityClearsTexture,
          textureClearsIntensity,
          ...counts,
          glError: gl.getError(),
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = GAIN_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantGain: dominantGain(layers),
        contributingGain: layers.filter(layer => layer.l2Delta <= -GAIN_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -GAIN_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -GAIN_BLUE_MIN).map(layer => layer.id),
        gainAxis: layers.filter(layer => GAIN_IDS.includes(layer.id) && (layer.l2Delta <= -GAIN_L2_MIN || layer.blueDelta <= -GAIN_BLUE_MIN)).map(layer => layer.id),
      };
    });
    return {
      ...
      textureIsCombined: report.textureIsCombined,
      gainClearsTexture: report.gainClearsTexture,
      textureClearsGain: report.textureClearsGain,
      intensityClearsTexture: report.intensityClearsTexture,
      textureClearsIntensity: report.textureClearsIntensity,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-gain-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldGain(browser, url) {
  const report = await measureWorldGain(browser, url);
  const out = process.env.WORLD_GAIN_OUTPUT ?? fromRoot('artifacts', 'world-gain');
  const results = await retainWorldGain(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldGain(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
