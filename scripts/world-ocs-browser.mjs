...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of setOpaqueSort vs setTransparentSort vs setClearAlpha. */
export const OCS_FRAMES = REGION_FRAMES;
export const OCS_LAYERS = Object.freeze([
  'lights', 'debug', 'xr', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = OCS_LAYERS;
...
export const OCS_L2_MIN = 8;
export const OCS_LUMA_MIN = 8;
export const OCS_BLUE_MIN = 8;
export const OCS_BAND_MIN = 0.02;
...
export function dominantOcs(layers) {
  if (!Array.isArray(layers) || layers.length !== OCS_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotOcsMismatch(report) {
  if (!report?.frames?.length) return true;
  if (report.protocol !== 'world-ocs-buffer-not-original-golden') return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== OCS_LAYERS.length
    || OCS_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !OCS_LAYERS.includes(layer.id)
...
export function ocsBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-ocs-buffer-not-original-golden'
...
function frame(id, extra = {}) {
  const spec = OCS_FRAMES.find(row => row.id === id);
  ...
  const layers = OCS_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantOcs: 'lights',
    contributingFrs: ['lights', 'pack'],
    lightsAxis: ['lights'],
    envAxis: [],
    bothAxis: ['both'],
    debugAxis: [],
...
export function worldOcsFixture(extra = {}) {
  return {
    ...
    ocsDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-ocs-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: OCS_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldOcsResults(r) {
  assert.ok(r && typeof r === 'object', 'world-ocs evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-ocs frames missing');
  assert.equal(leftoverIsNotOcsMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of setOpaqueSort vs setTransparentSort vs setClearAlpha mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'ocs isolation still combined with hemi');
  assert.equal(r.lightsClears, true, 'type isolation did not force setOpaqueSort reverse');
  ...
  assert.equal(r.lightsKeepsDebug, true, 'transparentSort left product during type isolation');
  assert.equal(r.lightsKeepsXr, true, 'clearAlpha left product during type isolation');
  assert.equal(r.debugClears, true, "needs isolation did not force setTransparentSort reverse");
  ...
  assert.equal(r.debugKeepsLights, true, 'opaqueSort left product during needs isolation');
  assert.equal(r.debugKeepsXr, true, 'clearAlpha left product during needs isolation');
  assert.equal(r.xrClears, true, 'clip isolation did not force setClearAlpha 1');
  ...
  assert.equal(r.xrKeepsLights, true, 'opaqueSort left product during clip isolation');
  assert.equal(r.xrKeepsDebug, true, 'transparentSort left product during clip isolation');
  ...
  assert.equal(r.envKeepsLights, true, 'opaqueSort left product during env isolation');
  assert.equal(r.envKeepsDebug, true, 'transparentSort left product during env isolation');
  assert.equal(r.envKeepsXr, true, 'clearAlpha left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsLights, true, 'pack isolation did not force setOpaqueSort reverse');
  assert.equal(r.packClearsDebug, true, "pack isolation did not force setTransparentSort reverse");
  assert.equal(r.packClearsXr, true, 'pack isolation did not force setClearAlpha 1');
  ...
  assert.equal(r.bothClearsLights, true, 'both isolation did not force setOpaqueSort reverse');
  assert.equal(r.bothClearsDebug, true, "both isolation did not force setTransparentSort reverse");
  assert.equal(r.bothClearsXr, true, 'both isolation did not force setClearAlpha 1');
  ...
  assert.equal(r.intensityClearsIbl, false, 'ocs isolation clears scene.environment via setOpaqueSort');
  assert.equal(r.ocsDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after tnc restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during ocs probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during ocs probe');
  ...
  assert.ok(r.packCount >= 151, 'combined setOpaqueSort-setTransparentSort-setClearAlpha isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both setOpaqueSort-setTransparentSort-setClearAlpha+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during ocs probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during ocs probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during ocs probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during ocs probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during ocs probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during ocs probe');
  ...
  assert.equal(ocsBufferIsNotOriginalGolden(r), false, 'ldx buffer is not original-golden');
  ...
  for (const spec of OCS_FRAMES) {
    ...
    if (!row.dominantOcs || !row.dominant || row.layers.length !== OCS_LAYERS.length
      || OCS_LAYERS.some(id => !byId[id])
      ...
      || OCS_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of setOpaqueSort vs setTransparentSort vs setClearAlpha attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present ocs axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'setOpaqueSort vs setTransparentSort vs setClearAlpha independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'ocs-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldOcs(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldOcsResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldFrs(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const setLights = () => {
          engine.renderer.setOpaqueSort((a, b) => (b.z - a.z) || (b.id - a.id));
          return [() => { engine.renderer.setOpaqueSort(null); }];
        };
        const setDebug = () => {
          engine.renderer.setTransparentSort((a, b) => (b.z - a.z) || (b.id - a.id));
          return [() => { engine.renderer.setTransparentSort(null); }];
        };
        const setXr = () => {
        ...
        const isolate = {
          lights() {
            ...
            const prevNeeds = engine.renderer.xr.getReferenceSpace();
            const prevClip = engine.renderer.getClearAlpha();
            ...
            if (engine.renderer.xr.getReferenceSpace() !== prevNeeds) lightsKeepsDebug = false;
            if (engine.renderer.getClearAlpha() !== prevClip) lightsKeepsXr = false;
            return restorers;
          },
          debug() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.getClearAlpha() !== prevClip) debugKeepsXr = false;
            return restorers;
          },
          xr() {
            const prevHemi = hemiRgb();
            ...
            xrClears = engine.renderer.getClearAlpha() === 1;
            ...
            if (engine.renderer.xr.getReferenceSpace() !== prevNeeds) xrKeepsDebug = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.xr.getReferenceSpace() !== prevNeeds) envKeepsDebug = false;
            if (engine.renderer.getClearAlpha() !== prevClip) envKeepsXr = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
            ...
            packClearsXr = engine.renderer.getClearAlpha() === 1;
          ...
          both() {
            ...
            bothClearsXr = engine.renderer.getClearAlpha() === 1;
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = OCS_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantOcs: dominantOcs(layers),
        contributingFrs: layers.filter(layer => layer.bandDelta <= -OCS_BAND_MIN).map(layer => layer.id),
        lightsAxis: layers.filter(layer => LIGHTS_IDS.includes(layer.id) && layer.bandDelta <= -OCS_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -OCS_BAND_MIN || layer.blueDelta <= -OCS_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -OCS_BAND_MIN).map(layer => layer.id),
        debugAxis: layers.filter(layer => DEBUG_IDS.includes(layer.id) && layer.bandDelta <= -OCS_BAND_MIN).map(layer => layer.id),
        xrAxis: layers.filter(layer => XR_IDS.includes(layer.id) && layer.bandDelta <= -OCS_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -OCS_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      ocsDeltasUseEmptyBaseline: report.ocsDeltasUseEmptyBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-ocs-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldOcs(browser, url) {
  const report = await measureWorldFrs(browser, url);
  const out = process.env.WORLD_OCS_OUTPUT ?? fromRoot('artifacts', 'world-ocs');
  const results = await retainWorldOcs(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldOcs(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
