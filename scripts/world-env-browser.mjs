...
/** Combined remaining cubemap vs IBL-off with gray hemi is not scene.environment vs material.envMap vs both attribution. */
export const ENV_FRAMES = REGION_FRAMES;
export const ENV_LAYERS = Object.freeze([
  'env', 'mat', 'both', 'hue', 'intensity', 'fill',
]);
export const CAPTURE_LAYERS = Object.freeze([
  'env', 'mat', 'both', 'hgray', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['env']);
export const MAT_IDS = Object.freeze(['mat']);
export const BOTH_IDS = Object.freeze(['both']);
export const HUE_IDS = Object.freeze(['hue']);
export const HGRAY_BASELINE_IDS = Object.freeze(['env', 'mat', 'both']);
export const PRODUCT_HEMI_SKY_HEX = 0xa8c8e8;
export const PRODUCT_HEMI_NIGHT_HEX = 0x6a88b0;
export const ENV_L2_MIN = 8;
export const ENV_LUMA_MIN = 8;
export const ENV_BLUE_MIN = 8;
...
export function dominantEnv(layers) {
  if (!Array.isArray(layers) || layers.length !== ENV_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function offIsNotEnvMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== ENV_LAYERS.length
    || ENV_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !ENV_LAYERS.includes(layer.id)
...
export function envBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-env-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { env: 1, mat: 1, both: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'env' ? 70 : id === 'both' ? 65 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'env' ? 110 : id === 'both' ? 105 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = ENV_FRAMES.find(row => row.id === id);
  ...
  const layers = ENV_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantEnv: 'hue',
    contributingEnv: ['env', 'both', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    envAxis: ['env'],
    matAxis: [],
    bothAxis: ['both'],
...
export function worldEnvFixture(extra = {}) {
  return {
    ...
    envCount: 1, matCount: 1, bothCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, envIsCombined: false, envClearsEnv: true,
    envSetsGray: true, envKeepsMaps: true, envClearsHue: false,
    matClearsMaps: true, matSetsGray: true, matKeepsEnv: true,
    bothClearsEnv: true, bothClearsMaps: true, bothSetsGray: true,
    hueClearsOff: false, intensityClearsIbl: false,
    envDeltasUseHgrayBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-env-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: ENV_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldEnvResults(r) {
  assert.ok(r && typeof r === 'object', 'world-env evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-env frames missing');
  assert.equal(offIsNotEnvMismatch(r), false, 'combined remaining cubemap vs IBL-off with gray hemi is not scene.environment vs material.envMap vs both mismatch');
  ...
  assert.equal(r.matClearsMaps, true, 'material.envMap isolation did not clear material.envMap');
  assert.equal(r.matSetsGray, true, 'hemi.color stays product during material.envMap isolation');
  assert.equal(r.matKeepsEnv, true, 'material.envMap isolation clears scene.environment');
  assert.equal(r.bothClearsEnv, true, 'both isolation did not clear scene.environment');
  assert.equal(r.bothClearsMaps, true, 'both isolation did not clear material.envMap');
  ...
  assert.equal(r.envDeltasUseHgrayBaseline, true, 'env vs mat vs both still uses product present as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product during env probe');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during env probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during env probe');
  ...
  assert.ok(r.matCount >= 1, 'material.envMap isolation missing');
  assert.ok(r.bothCount >= 1, 'scene.environment plus material.envMap isolation missing');
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during env probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during env probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during env probe');
  ...
  assert.equal(envBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of ENV_FRAMES) {
    ...
    if (!row.dominantEnv || !row.dominant || row.layers.length !== ENV_LAYERS.length
      || ENV_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.envAxis) || !Array.isArray(row.matAxis)
      || !Array.isArray(row.bothAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || ENV_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined remaining cubemap vs IBL-off with gray hemi is not scene.environment vs material.envMap vs both attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present env axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'scene.environment vs material.envMap vs both independently of 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'env-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldEnv(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldEnvResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldEnv(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let matClearsMaps = false;
        let matSetsGray = false;
        let matKeepsEnv = true;
        let bothClearsEnv = false;
        let bothClearsMaps = false;
        ...
        const nullEnvMaps = (flag) => {
          const restorers = [];
          for (const mat of envMapMaterials()) {
            const prev = mat.envMap;
            mat.envMap = null;
            if (mat.envMap == null) flag();
            restorers.push(() => { mat.envMap = prev; });
          }
          return restorers;
        };
        const isolate = {
          ...
          mat() {
            ...
            const hemiRestorers = grayHemi(() => { matSetsGray = true; });
            const mapRestorers = nullEnvMaps(() => { matClearsMaps = true; });
            if (engine.scene.environment == null) matKeepsEnv = false;
            return [() => {
              mapRestorers.forEach(restore => restore());
          ...
          both() {
            ...
            const mapRestorers = nullEnvMaps(() => { bothClearsMaps = true; });
        ...
        const matRestorers = isolate.mat();
        matRestorers.forEach(restore => restore());
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = ENV_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantEnv: dominantEnv(layers),
        contributingEnv: layers.filter(layer => layer.l2Delta <= -ENV_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -ENV_LUMA_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -ENV_BLUE_MIN).map(layer => layer.id),
        matAxis: layers.filter(layer => MAT_IDS.includes(layer.id) && layer.blueDelta <= -ENV_BLUE_MIN).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && (layer.l2Delta <= -ENV_L2_MIN || layer.blueDelta <= -ENV_BLUE_MIN)).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -ENV_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      matClearsMaps: report.matClearsMaps,
      matSetsGray: report.matSetsGray,
      matKeepsEnv: report.matKeepsEnv,
      bothClearsEnv: report.bothClearsEnv,
      bothClearsMaps: report.bothClearsMaps,
      bothSetsGray: report.bothSetsGray,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      envDeltasUseHgrayBaseline: report.envDeltasUseHgrayBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-env-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldEnv(browser, url) {
  const report = await measureWorldEnv(browser, url);
  const out = process.env.WORLD_ENV_OUTPUT ?? fromRoot('artifacts', 'world-env');
  const results = await retainWorldEnv(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldEnv(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
