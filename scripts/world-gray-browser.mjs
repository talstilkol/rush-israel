...
/** Combined leftover IBL is not gray cubemap vs IBL-off attribution. */
export const GRAY_FRAMES = REGION_FRAMES;
export const GRAY_LAYERS = Object.freeze([
  'gray', 'hue', 'hemi', 'disc', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['gray']);
export const HUE_IDS = Object.freeze(['hue']);
export const DISC_IDS = Object.freeze(['disc']);
export const HEMI_IDS = Object.freeze(['hemi']);
export const GRAY_L2_MIN = 8;
export const GRAY_LUMA_MIN = 8;
export const GRAY_BLUE_MIN = 8;
...
export function dominantGray(layers) {
  if (!Array.isArray(layers) || layers.length !== GRAY_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function skyIsNotGrayMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== GRAY_LAYERS.length
    || GRAY_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !GRAY_LAYERS.includes(layer.id)
...
export function grayBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-gray-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { gray: 1, hue: 1, hemi: 1, disc: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'gray' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'gray' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = GRAY_FRAMES.find(row => row.id === id);
  ...
  const layers = GRAY_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantGray: 'hue',
    contributingGray: ['gray', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: ['gray'],
    hueAxis: ['hue'],
    hemiAxis: [],
...
export function worldGrayFixture(extra = {}) {
  return {
    ...
    grayCount: 1, hueCount: 1, hemiCount: 1, discCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, grayIsCombined: false, grayClearsHue: false, grayClearsIntensity: false,
    hueClearsGray: false, intensityClearsIbl: false, grayDeltasUseHueBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-gray-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: GRAY_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldGrayResults(r) {
  assert.ok(r && typeof r === 'object', 'world-gray evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-gray frames missing');
  assert.equal(skyIsNotGrayMismatch(r), false, 'combined leftover IBL is not gray cubemap vs IBL-off mismatch');
  ...
  assert.equal(r.grayIsCombined, false, 'gray isolation still combined with hue');
  assert.equal(r.grayClearsHue, false, 'gray isolation clears hue cubemap');
  assert.equal(r.grayClearsIntensity, false, 'gray isolation clears intensity');
  ...
  assert.equal(r.grayDeltasUseHueBaseline, true, 'gray vs off still uses product present as baseline');
  ...
  assert.ok(r.hemiCount >= 1, 'bakeEnv hemi missing');
  assert.ok(r.discCount >= 1, 'bakeEnv disc missing');
  ...
  assert.equal(grayBufferIsNotOriginalGolden(r), false, 'gray buffer is not original-golden');
  ...
  for (const spec of GRAY_FRAMES) {
    ...
    if (!row.dominantGray || !row.dominant || row.layers.length !== GRAY_LAYERS.length
      || GRAY_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.hueAxis)
      || !Array.isArray(row.hemiAxis) || !Array.isArray(row.discAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || GRAY_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover IBL is not gray cubemap vs IBL-off attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present gray axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'gray IBL vs IBL-off independently of 0x3a9ae0 hue vs hemi vs disc vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'gray-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldGray(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldGrayResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldGray(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const bakeVariant = (omit) => {
          ...
          tmp.background = new THREE.Color(omit.skyHex ?? productBg);
        ...
        const isolate = {
          ...
          hemi() {
            return swapEnv(bakeVariant({ hemi: true }));
          },
          disc() {
            return swapEnv(bakeVariant({ disc: true }));
          },
          intensity() {
        ...
        const grayRestorers = isolate.gray();
        const grayIsCombined = grayRestorers.length !== 1;
        const grayClearsHue = engine.scene.environment === hueProbe.texture;
        const grayClearsIntensity = sun.intensity !== intensityBefore;
        grayRestorers.forEach(restore => restore());
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = GRAY_LAYERS.map(id => {
        ...
        const compareL2 = id === 'gray' ? hueSample.l2 : baselineSample.l2;
        const compareLuma = id === 'gray' ? hueLuma : baselineLuma;
        const compareBlue = id === 'gray' ? hueSample.live.b : baselineSample.live.b;
        const comparePct = id === 'gray' ? Number(hueBand.pct.toFixed(4)) : Number(baselineBand.pct.toFixed(4));
      ...
      return {
        ...
        layers,
        dominantGray: dominantGray(layers),
        contributingGray: layers.filter(layer => layer.l2Delta <= -GRAY_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -GRAY_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -GRAY_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -GRAY_BLUE_MIN).map(layer => layer.id),
        hemiAxis: layers.filter(layer => HEMI_IDS.includes(layer.id) && (layer.l2Delta <= -GRAY_L2_MIN || layer.blueDelta <= -GRAY_BLUE_MIN)).map(layer => layer.id),
        discAxis: layers.filter(layer => DISC_IDS.includes(layer.id) && (layer.l2Delta <= -GRAY_L2_MIN || layer.blueDelta <= -GRAY_BLUE_MIN)).map(layer => layer.id),
      };
    });
    return {
      ...
      grayIsCombined: report.grayIsCombined,
      grayClearsHue: report.grayClearsHue,
      grayClearsIntensity: report.grayClearsIntensity,
      hueClearsGray: report.hueClearsGray,
      intensityClearsIbl: report.intensityClearsIbl,
      grayDeltasUseHueBaseline: report.grayDeltasUseHueBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-gray-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldGray(browser, url) {
  const report = await measureWorldGray(browser, url);
  const out = process.env.WORLD_GRAY_OUTPUT ?? fromRoot('artifacts', 'world-gray');
  const results = await retainWorldGray(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldGray(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
