...
/** Combined leftover sRGB fromScene vs linear bake is not ColorManagement vs outputColorSpace vs toneMapping attribution. */
export const MGMT_FRAMES = REGION_FRAMES;
export const MGMT_LAYERS = Object.freeze([
  'mgmt', 'encode', 'tone', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['mgmt']);
export const ENCODE_IDS = Object.freeze(['encode']);
export const TONE_IDS = Object.freeze(['tone']);
export const HUE_IDS = Object.freeze(['hue']);
export const GRAY_BASELINE_IDS = Object.freeze(['mgmt', 'encode', 'tone']);
export const MGMT_L2_MIN = 8;
export const MGMT_LUMA_MIN = 8;
export const MGMT_BLUE_MIN = 8;
...
export function dominantMgmt(layers) {
  if (!Array.isArray(layers) || layers.length !== MGMT_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function sigmaIsNotMgmtMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== MGMT_LAYERS.length
    || MGMT_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !MGMT_LAYERS.includes(layer.id)
    ...
    || !Array.isArray(frame.encodeAxis)
...
export function mgmtBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-mgmt-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { mgmt: 1, encode: 1, tone: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'tone' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'tone' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = MGMT_FRAMES.find(row => row.id === id);
  ...
  const layers = MGMT_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantMgmt: 'hue',
    contributingMgmt: ['tone', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: [],
    encodeAxis: [],
    toneAxis: ['tone'],
...
export function worldMgmtFixture(extra = {}) {
  return {
    ...
    mgmtCount: 1, encodeCount: 1, toneCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, mgmtIsCombined: false, mgmtClearsOff: false,
    mgmtClearsHue: false, encodeClearsMgmt: false, hueClearsOff: false, intensityClearsIbl: false,
    mgmtDeltasUseGrayBaseline: true,
    mgmtDisablesColorManagement: true, encodeSetsLinearOutput: true, toneSetsNoToneMapping: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-mgmt-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: MGMT_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldMgmtResults(r) {
  assert.ok(r && typeof r === 'object', 'world-mgmt evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-mgmt frames missing');
  assert.equal(sigmaIsNotMgmtMismatch(r), false, 'combined leftover sRGB fromScene vs linear bake is not ColorManagement vs outputColorSpace vs toneMapping mismatch');
  ...
  assert.equal(r.mgmtIsCombined, false, 'mgmt isolation still combined with hemi or disc');
  assert.equal(r.mgmtClearsOff, false, 'mgmt isolation clears IBL-off');
  assert.equal(r.mgmtClearsHue, false, 'mgmt isolation clears gray cubemap hue');
  assert.equal(r.encodeClearsMgmt, false, 'encode isolation uses ColorManagement bake');
  ...
  assert.equal(r.mgmtDeltasUseGrayBaseline, true, 'mgmt vs encode vs tone still uses product present as baseline');
  assert.equal(r.mgmtDisablesColorManagement, true, 'ColorManagement stays enabled during mgmt bake');
  assert.equal(r.encodeSetsLinearOutput, true, 'outputColorSpace stays product sRGB during encode bake');
  assert.equal(r.toneSetsNoToneMapping, true, 'toneMapping stays product ACES during tone bake');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.mgmtCount >= 1, 'ColorManagement bake missing');
  assert.ok(r.encodeCount >= 1, 'outputColorSpace bake missing');
  assert.ok(r.toneCount >= 1, 'toneMapping bake missing');
  ...
  assert.equal(mgmtBufferIsNotOriginalGolden(r), false, 'mgmt buffer is not original-golden');
  ...
  for (const spec of MGMT_FRAMES) {
    ...
    if (!row.dominantMgmt || !row.dominant || row.layers.length !== MGMT_LAYERS.length
      || MGMT_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.encodeAxis)
      || !Array.isArray(row.toneAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || MGMT_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover sRGB fromScene vs linear bake is not ColorManagement vs outputColorSpace vs toneMapping attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present mgmt axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'ColorManagement vs outputColorSpace vs toneMapping during bake independently of 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'mgmt-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldMgmt(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldMgmtResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldMgmt(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let mgmtDisabledColorManagement = false;
        let encodeSetsLinearOutput = false;
        let toneSetsNoToneMapping = false;
        const bakeGray = (prepare) => {
          const restorePrep = prepare ? prepare() : () => {};
          let env;
          try {
            const tmp = new THREE.Scene();
        ...
        const isolate = {
          mgmt() {
            return bakeGray(() => {
              ...
              mgmtDisabledColorManagement = THREE.ColorManagement.enabled === false;
          ...
          encode() {
            return bakeGray(() => {
              ...
              engine.renderer.outputColorSpace = THREE.LinearSRGBColorSpace;
              encodeSetsLinearOutput = engine.renderer.outputColorSpace === THREE.LinearSRGBColorSpace;
          ...
          tone() {
            return bakeGray(() => {
              ...
              toneSetsNoToneMapping = engine.renderer.toneMapping === THREE.NoToneMapping;
        ...
        const mgmtTex = engine.scene.environment;
        const mgmtIsCombined = mgmtRestorers.length !== 1;
        const mgmtClearsOff = mgmtTex == null;
        const mgmtClearsHue = mgmtTex === hueProbe.texture;
        mgmtRestorers.forEach(restore => restore());
        const encodeRestorers = isolate.encode();
        const encodeClearsMgmt = engine.scene.environment == null
          || engine.scene.environment === mgmtTex;
        encodeRestorers.forEach(restore => restore());
        ...
        return {
          ...
          intensityClearsIbl,
          mgmtDeltasUseGrayBaseline: true,
          mgmtDisabledColorManagement,
          encodeSetsLinearOutput,
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = MGMT_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantMgmt: dominantMgmt(layers),
        contributingMgmt: layers.filter(layer => layer.l2Delta <= -MGMT_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -MGMT_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -MGMT_BLUE_MIN).map(layer => layer.id),
        encodeAxis: layers.filter(layer => ENCODE_IDS.includes(layer.id) && (layer.l2Delta <= -MGMT_L2_MIN || layer.blueDelta <= -MGMT_BLUE_MIN)).map(layer => layer.id),
        toneAxis: layers.filter(layer => TONE_IDS.includes(layer.id) && layer.blueDelta <= -MGMT_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -MGMT_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      mgmtIsCombined: report.mgmtIsCombined,
      mgmtClearsOff: report.mgmtClearsOff,
      mgmtClearsHue: report.mgmtClearsHue,
      encodeClearsMgmt: report.encodeClearsMgmt,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      mgmtDeltasUseGrayBaseline: report.mgmtDeltasUseGrayBaseline,
      mgmtDisablesColorManagement: report.mgmtDisabledColorManagement,
      encodeSetsLinearOutput: report.encodeSetsLinearOutput,
      toneSetsNoToneMapping: report.toneSetsNoToneMapping,
      mgmtCount: report.mgmtCount,
      encodeCount: report.encodeCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-mgmt-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldMgmt(browser, url) {
  const report = await measureWorldMgmt(browser, url);
  const out = process.env.WORLD_MGMT_OUTPUT ?? fromRoot('artifacts', 'world-mgmt');
  const results = await retainWorldMgmt(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldMgmt(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
