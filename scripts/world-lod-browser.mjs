...
/** Combined leftover gray cubemap vs IBL-off is not PMREM size vs CubeUV lod vs ground envMapIntensity attribution. */
export const LOD_FRAMES = REGION_FRAMES;
export const LOD_LAYERS = Object.freeze([
  'size', 'lod', 'env', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['size']);
export const LOD_IDS = Object.freeze(['lod']);
export const ENV_IDS = Object.freeze(['env']);
export const HUE_IDS = Object.freeze(['hue']);
export const GRAY_BASELINE_IDS = Object.freeze(['size', 'lod', 'env']);
export const LOD_L2_MIN = 8;
export const LOD_LUMA_MIN = 8;
export const LOD_BLUE_MIN = 8;
...
export const SMALL_PMREM_SIZE = 16;
...
export function dominantLod(layers) {
  if (!Array.isArray(layers) || layers.length !== LOD_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function mgmtIsNotLodMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== LOD_LAYERS.length
    || LOD_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !LOD_LAYERS.includes(layer.id)
...
export function lodBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-lod-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { size: 1, lod: 1, env: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'env' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'env' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = LOD_FRAMES.find(row => row.id === id);
  ...
  const layers = LOD_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantLod: 'hue',
    contributingLod: ['env', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: [],
    lodAxis: [],
    envAxis: ['env'],
...
export function worldLodFixture(extra = {}) {
  return {
    ...
    sizeCount: 1, lodCount: 1, envCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, sizeIsCombined: false, sizeClearsOff: false,
    sizeClearsHue: false, lodClearsSize: false, hueClearsOff: false, intensityClearsIbl: false,
    sizeDeltasUseGrayBaseline: true,
    sizeSetsSmallPmrem: true, lodZerosGroundRoughness: true, envZerosGroundEnvMapIntensity: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-lod-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: LOD_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldLodResults(r) {
  assert.ok(r && typeof r === 'object', 'world-lod evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-lod frames missing');
  assert.equal(mgmtIsNotLodMismatch(r), false, 'combined leftover gray cubemap vs IBL-off is not PMREM size vs CubeUV lod vs ground envMapIntensity mismatch');
  ...
  assert.equal(r.sizeIsCombined, false, 'size isolation still combined with hemi or disc');
  assert.equal(r.sizeClearsOff, false, 'size isolation clears IBL-off');
  assert.equal(r.sizeClearsHue, false, 'size isolation clears gray cubemap hue');
  assert.equal(r.lodClearsSize, false, 'lod isolation uses size-16 bake');
  ...
  assert.equal(r.sizeDeltasUseGrayBaseline, true, 'size vs lod vs env still uses product present as baseline');
  assert.equal(r.sizeSetsSmallPmrem, true, 'PMREM size stays product 256 during size bake');
  assert.equal(r.lodZerosGroundRoughness, true, 'ground roughness stays product during lod isolation');
  assert.equal(r.envZerosGroundEnvMapIntensity, true, 'ground envMapIntensity stays product during env isolation');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.sizeCount >= 1, 'PMREM size bake missing');
  assert.ok(r.lodCount >= 1, 'CubeUV lod isolation missing');
  assert.ok(r.envCount >= 1, 'ground envMapIntensity isolation missing');
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during lod probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during lod probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during lod probe');
  ...
  assert.equal(lodBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of LOD_FRAMES) {
    ...
    if (!row.dominantLod || !row.dominant || row.layers.length !== LOD_LAYERS.length
      || LOD_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.lodAxis)
      || !Array.isArray(row.envAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || LOD_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover gray cubemap vs IBL-off is not PMREM size vs CubeUV lod vs ground envMapIntensity attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present lod axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'PMREM size vs CubeUV lod vs ground envMapIntensity independently of ColorManagement vs outputColorSpace vs toneMapping vs 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'lod-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldLod(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldLodResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldLod(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath, smallSize }) => {
      ...
      try {
        ...
        let sizeSetsSmallPmrem = false;
        let lodZerosGroundRoughness = false;
        let envZerosGroundEnvMapIntensity = false;
        ...
        const bakeGray = (opts = {}) => {
          ...
          const pmrem = new THREE.PMREMGenerator(engine.renderer);
          try {
            const env = pmrem.fromScene(tmp, 0.04, 0.1, 100, { size: opts.size ?? 256 });
            owned.push(env);
            return env;
          } finally {
            pmrem.dispose();
        ...
        const isolate = {
          size() {
            const env = bakeGray({ size: smallSize });
            sizeSetsSmallPmrem = smallSize === 16;
            return swapEnv(env);
          },
          lod() {
            const [restoreEnv] = swapEnv(bakeGray());
            const restorers = [];
            for (const mat of groundMaterials()) {
              const roughness = mat.roughness;
              mat.roughness = 0;
              if (mat.roughness === 0) lodZerosGroundRoughness = true;
              restorers.push(() => { mat.roughness = roughness; });
            }
          ...
          env() {
            ...
            for (const mat of groundMaterials()) {
              ...
              if (mat.envMapIntensity === 0) envZerosGroundEnvMapIntensity = true;
        ...
        const sizeRestorers = isolate.size();
        const sizeTex = engine.scene.environment;
        const sizeIsCombined = sizeRestorers.length !== 1;
        const sizeClearsOff = sizeTex == null;
        const sizeClearsHue = sizeTex === hueProbe.texture;
        sizeRestorers.forEach(restore => restore());
        const lodRestorers = isolate.lod();
        const lodClearsSize = engine.scene.environment == null
          || engine.scene.environment === sizeTex;
        lodRestorers.forEach(restore => restore());
        ...
        return {
          ...
          sizeDeltasUseGrayBaseline: true,
          sizeSetsSmallPmrem,
          lodZerosGroundRoughness,
          envZerosGroundEnvMapIntensity,
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = LOD_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantLod: dominantLod(layers),
        contributingLod: layers.filter(layer => layer.l2Delta <= -LOD_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -LOD_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -LOD_BLUE_MIN).map(layer => layer.id),
        lodAxis: layers.filter(layer => LOD_IDS.includes(layer.id) && (layer.l2Delta <= -LOD_L2_MIN || layer.blueDelta <= -LOD_BLUE_MIN)).map(layer => layer.id),
        envAxis: layers.filter(layer => ENV_IDS.includes(layer.id) && layer.blueDelta <= -LOD_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -LOD_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      sizeIsCombined: report.sizeIsCombined,
      sizeClearsOff: report.sizeClearsOff,
      sizeClearsHue: report.sizeClearsHue,
      lodClearsSize: report.lodClearsSize,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      sizeDeltasUseGrayBaseline: report.sizeDeltasUseGrayBaseline,
      sizeSetsSmallPmrem: report.sizeSetsSmallPmrem,
      lodZerosGroundRoughness: report.lodZerosGroundRoughness,
      envZerosGroundEnvMapIntensity: report.envZerosGroundEnvMapIntensity,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-lod-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldLod(browser, url) {
  const report = await measureWorldLod(browser, url);
  const out = process.env.WORLD_LOD_OUTPUT ?? fromRoot('artifacts', 'world-lod');
  const results = await retainWorldLod(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldLod(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
