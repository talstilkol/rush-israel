...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of useLegacyLights vs debug.checkShaderErrors vs xr.enabled. */
export const LDX_FRAMES = REGION_FRAMES;
export const LDX_LAYERS = Object.freeze([
  'lights', 'debug', 'xr', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = LDX_LAYERS;
...
export const LDX_L2_MIN = 8;
export const LDX_LUMA_MIN = 8;
export const LDX_BLUE_MIN = 8;
export const LDX_BAND_MIN = 0.02;
...
export const PRODUCT_LIGHTS = false;
...
export function dominantLdx(layers) {
  if (!Array.isArray(layers) || layers.length !== LDX_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotLdxMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== LDX_LAYERS.length
    || LDX_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !LDX_LAYERS.includes(layer.id)
...
export function ldxBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-ldx-buffer-not-original-golden'
...
function frame(id, extra = {}) {
  const spec = LDX_FRAMES.find(row => row.id === id);
  ...
  const layers = LDX_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantLdx: 'lights',
    contributingLdx: ['lights', 'pack'],
    lightsAxis: ['lights'],
    envAxis: [],
    bothAxis: ['both'],
    debugAxis: [],
...
export function worldLdxFixture(extra = {}) {
  return {
    ...
    ldxDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-ldx-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: LDX_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldLdxResults(r) {
  assert.ok(r && typeof r === 'object', 'world-ldx evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-ldx frames missing');
  assert.equal(leftoverIsNotLdxMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of useLegacyLights vs debug.checkShaderErrors vs xr.enabled mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'ldx isolation still combined with hemi');
  assert.equal(r.lightsClears, true, 'type isolation did not force useLegacyLights');
  ...
  assert.equal(r.lightsKeepsDebug, true, 'checkShaderErrors left product during type isolation');
  assert.equal(r.lightsKeepsXr, true, 'xr.enabled left product during type isolation');
  assert.equal(r.debugClears, true, 'needs isolation did not force checkShaderErrors off');
  ...
  assert.equal(r.debugKeepsLights, true, 'useLegacyLights left product during needs isolation');
  assert.equal(r.debugKeepsXr, true, 'xr.enabled left product during needs isolation');
  assert.equal(r.xrClears, true, 'clip isolation did not force xr.enabled on');
  ...
  assert.equal(r.xrKeepsLights, true, 'useLegacyLights left product during clip isolation');
  assert.equal(r.xrKeepsDebug, true, 'checkShaderErrors left product during clip isolation');
  ...
  assert.equal(r.envKeepsLights, true, 'useLegacyLights left product during env isolation');
  assert.equal(r.envKeepsDebug, true, 'checkShaderErrors left product during env isolation');
  assert.equal(r.envKeepsXr, true, 'xr.enabled left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsLights, true, 'pack isolation did not force useLegacyLights');
  assert.equal(r.packClearsDebug, true, 'pack isolation did not force checkShaderErrors off');
  assert.equal(r.packClearsXr, true, 'pack isolation did not force xr.enabled on');
  ...
  assert.equal(r.bothClearsLights, true, 'both isolation did not force useLegacyLights');
  assert.equal(r.bothClearsDebug, true, 'both isolation did not force checkShaderErrors off');
  assert.equal(r.bothClearsXr, true, 'both isolation did not force xr.enabled on');
  ...
  assert.equal(r.intensityClearsIbl, false, 'ldx isolation clears scene.environment via sun');
  assert.equal(r.ldxDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after tnc restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during ldx probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during ldx probe');
  ...
  assert.ok(r.packCount >= 151, 'combined useLegacyLights-debug.checkShaderErrors-xr.enabled isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both useLegacyLights-debug.checkShaderErrors-xr.enabled+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during ldx probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during ldx probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during ldx probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during ldx probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during ldx probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during ldx probe');
  ...
  assert.equal(ldxBufferIsNotOriginalGolden(r), false, 'ldx buffer is not original-golden');
  ...
  for (const spec of LDX_FRAMES) {
    ...
    if (!row.dominantLdx || !row.dominant || row.layers.length !== LDX_LAYERS.length
      || LDX_LAYERS.some(id => !byId[id])
      ...
      || LDX_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of useLegacyLights vs debug.checkShaderErrors vs xr.enabled attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present ldx axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'useLegacyLights vs debug.checkShaderErrors vs xr.enabled independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'ldx-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldLdx(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldLdxResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldLdx(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const setLights = () => {
          const prev = engine.renderer.useLegacyLights;
          engine.renderer.useLegacyLights = true;
          return [() => { engine.renderer.useLegacyLights = prev; }];
        };
        const setDebug = () => {
          const prev = engine.renderer.debug.checkShaderErrors;
          engine.renderer.debug.checkShaderErrors = false;
          return [() => { engine.renderer.debug.checkShaderErrors = prev; }];
        };
        const setXr = () => {
          const prev = engine.renderer.xr.enabled;
          engine.renderer.xr.enabled = true;
          return [() => { engine.renderer.xr.enabled = prev; }];
        };
        const isolate = {
          lights() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevNeeds = engine.renderer.debug.checkShaderErrors;
            const prevClip = engine.renderer.xr.enabled;
            ...
            if (engine.renderer.debug.checkShaderErrors !== prevNeeds) lightsKeepsDebug = false;
            if (engine.renderer.xr.enabled !== prevClip) lightsKeepsXr = false;
            return restorers;
          },
          debug() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevType = engine.renderer.useLegacyLights;
            const prevClip = engine.renderer.xr.enabled;
            ...
            if (engine.renderer.useLegacyLights !== prevType) debugKeepsLights = false;
            if (engine.renderer.xr.enabled !== prevClip) debugKeepsXr = false;
            return restorers;
          },
          xr() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.useLegacyLights !== prevType) xrKeepsLights = false;
            if (engine.renderer.debug.checkShaderErrors !== prevNeeds) xrKeepsDebug = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.useLegacyLights !== prevType) envKeepsLights = false;
            if (engine.renderer.debug.checkShaderErrors !== prevNeeds) envKeepsDebug = false;
            if (engine.renderer.xr.enabled !== prevClip) envKeepsXr = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = LDX_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantLdx: dominantLdx(layers),
        contributingLdx: layers.filter(layer => layer.bandDelta <= -LDX_BAND_MIN).map(layer => layer.id),
        lightsAxis: layers.filter(layer => LIGHTS_IDS.includes(layer.id) && layer.bandDelta <= -LDX_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -LDX_BAND_MIN || layer.blueDelta <= -LDX_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -LDX_BAND_MIN).map(layer => layer.id),
        debugAxis: layers.filter(layer => DEBUG_IDS.includes(layer.id) && layer.bandDelta <= -LDX_BAND_MIN).map(layer => layer.id),
        xrAxis: layers.filter(layer => XR_IDS.includes(layer.id) && layer.bandDelta <= -LDX_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -LDX_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      ldxDeltasUseEmptyBaseline: report.ldxDeltasUseEmptyBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-ldx-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldLdx(browser, url) {
  const report = await measureWorldLdx(browser, url);
  const out = process.env.WORLD_LDX_OUTPUT ?? fromRoot('artifacts', 'world-ldx');
  const results = await retainWorldLdx(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldLdx(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
