...
/** Combined leftover after neutralizing hemi.color to 0x808080 is not remaining cubemap vs IBL-off attribution. */
export const OFF_FRAMES = REGION_FRAMES;
export const OFF_LAYERS = Object.freeze([
  'off', 'hgray', 'hblack', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['off']);
export const HGRAY_IDS = Object.freeze(['hgray']);
export const HBLACK_IDS = Object.freeze(['hblack']);
export const HUE_IDS = Object.freeze(['hue']);
export const GRAY_BASELINE_IDS = Object.freeze(['hgray', 'hblack']);
export const HGRAY_BASELINE_IDS = Object.freeze(['off']);
export const PRODUCT_HEMI_SKY_HEX = 0xa8c8e8;
export const PRODUCT_HEMI_NIGHT_HEX = 0x6a88b0;
export const OFF_L2_MIN = 8;
export const OFF_LUMA_MIN = 8;
export const OFF_BLUE_MIN = 8;
...
export function dominantOff(layers) {
  if (!Array.isArray(layers) || layers.length !== OFF_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function hexIsNotOffMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== OFF_LAYERS.length
    || OFF_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !OFF_LAYERS.includes(layer.id)
    ...
    || !Array.isArray(frame.hgrayAxis)
    || !Array.isArray(frame.hblackAxis)
...
export function offBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-off-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { off: 1, hgray: 1, hblack: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'hue' ? 50 : id === 'off' ? 70 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'hue' ? 90 : id === 'off' ? 110 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = OFF_FRAMES.find(row => row.id === id);
  ...
  const layers = OFF_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantOff: 'hue',
    contributingOff: ['off', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    offAxis: ['off'],
    hgrayAxis: [],
    hblackAxis: [],
...
export function worldOffFixture(extra = {}) {
  return {
    ...
    offCount: 1, hgrayCount: 1, hblackCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, offIsCombined: false, offClearsEnv: true,
    offSetsGray: true, offKeepsIntensity: true, offClearsHue: false,
    hgrayClearsOff: false, hblackClearsOff: false, hueClearsOff: false, intensityClearsIbl: false,
    offDeltasUseHgrayBaseline: true,
    hgraySetsGray: true, hblackSetsBlack: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-off-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: OFF_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldOffResults(r) {
  assert.ok(r && typeof r === 'object', 'world-off evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-off frames missing');
  assert.equal(hexIsNotOffMismatch(r), false, 'combined leftover after neutralizing hemi.color to 0x808080 is not remaining cubemap vs IBL-off mismatch');
  ...
  assert.equal(r.offIsCombined, false, 'off isolation still combined with hemi or disc');
  assert.equal(r.offClearsEnv, true, 'IBL-off isolation did not clear scene.environment');
  assert.equal(r.offSetsGray, true, 'hemi.color stays product during IBL-off isolation');
  assert.equal(r.offKeepsIntensity, true, 'HemisphereLight intensity left product during IBL-off isolation');
  assert.equal(r.offClearsHue, false, 'IBL-off isolation uses gray cubemap hue');
  assert.equal(r.hgrayClearsOff, false, 'hgray isolation clears IBL-off');
  assert.equal(r.hblackClearsOff, false, 'hblack isolation clears IBL-off');
  ...
  assert.equal(r.offDeltasUseHgrayBaseline, true, 'off vs remaining cubemap still uses product present as baseline');
  assert.equal(r.hgraySetsGray, true, 'HemisphereLight color stays product during hgray isolation');
  assert.equal(r.hblackSetsBlack, true, 'HemisphereLight color stays product during hblack isolation');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product during off probe');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during off probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during off probe');
  assert.equal(r.productExposure, PRODUCT_EXPOSURE, 'product exposure retuned');
  assert.ok(r.offCount >= 1, 'IBL-off isolation missing');
  assert.ok(r.hgrayCount >= 1, 'hemi.color 0x808080 isolation missing');
  assert.ok(r.hblackCount >= 1, 'hemi.color 0x000000 isolation missing');
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during off probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during off probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during off probe');
  ...
  assert.equal(offBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of OFF_FRAMES) {
    ...
    if (!row.dominantOff || !row.dominant || row.layers.length !== OFF_LAYERS.length
      || OFF_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.offAxis) || !Array.isArray(row.hgrayAxis)
      || !Array.isArray(row.hblackAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || OFF_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover after neutralizing hemi.color to 0x808080 is not remaining cubemap vs IBL-off attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present off axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'remaining cubemap vs IBL-off independently of hemi.color 0x808080 vs 0x000000 vs 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'off-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldOff(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldOffResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldOff(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        let offClearsEnv = false;
        let offSetsGray = false;
        let offKeepsIntensity = true;
        let hgraySetsGray = false;
        let hblackSetsBlack = false;
        const bakeGray = () => {
          const tmp = new THREE.Scene();
        ...
        const isolate = {
          off() {
            ...
            for (const light of hemiLights()) {
              ...
              if (light.color.getHex() === 0x808080) offSetsGray = true;
              if (light.intensity !== intensity) offKeepsIntensity = false;
              restorers.push(() => { light.color.setHex(hex); });
            }
            if (engine.scene.environment == null) offClearsEnv = true;
          ...
          hgray() {
            ...
            for (const light of hemiLights()) {
              const hex = light.color.getHex();
              light.color.setHex(0x808080);
              if (light.color.getHex() === 0x808080) hgraySetsGray = true;
              restorers.push(() => { light.color.setHex(hex); });
            }
            return [() => { restorers.forEach(restore => restore()); restoreEnv(); }];
          },
          hblack() {
            ...
            for (const light of hemiLights()) {
              ...
              if (light.color.getHex() === 0) hblackSetsBlack = true;
              restorers.push(() => { light.color.setHex(hex); });
            }
            return [() => { restorers.forEach(restore => restore()); restoreEnv(); }];
          },
        ...
        const intensityBefore = hemiLights().map(light => light.intensity);
        const ambBefore = [];
        const rectBefore = [];
        engine.scene.traverse(object => {
        ...
        const offRestorers = isolate.off();
        const offTex = engine.scene.environment;
        const offIsCombined = offRestorers.length !== 1;
        const offClearsHue = offTex === hueProbe.texture;
        offRestorers.forEach(restore => restore());
        const hgrayRestorers = isolate.hgray();
        const hgrayClearsOff = engine.scene.environment == null;
        hgrayRestorers.forEach(restore => restore());
        const hblackRestorers = isolate.hblack();
        const hblackClearsOff = engine.scene.environment == null;
        hblackRestorers.forEach(restore => restore());
        ...
        if (hemiLights().some((light, i) => light.intensity !== intensityBefore[i])) offKeepsIntensity = false;
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = OFF_LAYERS.map(id => {
        ...
        const compareL2 = HGRAY_BASELINE_IDS.includes(id) ? hgraySample.l2
          : GRAY_BASELINE_IDS.includes(id) ? hueSample.l2
          : baselineSample.l2;
        const compareLuma = HGRAY_BASELINE_IDS.includes(id) ? hgrayLuma
          : GRAY_BASELINE_IDS.includes(id) ? hueLuma
          : baselineLuma;
        const compareBlue = HGRAY_BASELINE_IDS.includes(id) ? hgraySample.live.b
          : GRAY_BASELINE_IDS.includes(id) ? hueSample.live.b
          : baselineSample.live.b;
        const comparePct = HGRAY_BASELINE_IDS.includes(id) ? Number(hgrayBand.pct.toFixed(4))
          : GRAY_BASELINE_IDS.includes(id) ? Number(hueBand.pct.toFixed(4))
          : Number(baselineBand.pct.toFixed(4));
      ...
      return {
        ...
        layers,
        dominantOff: dominantOff(layers),
        contributingOff: layers.filter(layer => layer.l2Delta <= -OFF_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -OFF_LUMA_MIN).map(layer => layer.id),
        offAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -OFF_BLUE_MIN).map(layer => layer.id),
        hgrayAxis: layers.filter(layer => HGRAY_IDS.includes(layer.id) && layer.blueDelta <= -OFF_BLUE_MIN).map(layer => layer.id),
        hblackAxis: layers.filter(layer => HBLACK_IDS.includes(layer.id) && (layer.l2Delta <= -OFF_L2_MIN || layer.blueDelta <= -OFF_BLUE_MIN)).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -OFF_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      offIsCombined: report.offIsCombined,
      offClearsEnv: report.offClearsEnv,
      offSetsGray: report.offSetsGray,
      offKeepsIntensity: report.offKeepsIntensity,
      offClearsHue: report.offClearsHue,
      hgrayClearsOff: report.hgrayClearsOff,
      hblackClearsOff: report.hblackClearsOff,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      offDeltasUseHgrayBaseline: report.offDeltasUseHgrayBaseline,
      hblackSetsBlack: report.hblackSetsBlack,
      hgraySetsGray: report.hgraySetsGray,
      ...
      hblackCount: report.hblackCount,
      hgrayCount: report.hgrayCount,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-off-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldOff(browser, url) {
  const report = await measureWorldOff(browser, url);
  const out = process.env.WORLD_OFF_OUTPUT ?? fromRoot('artifacts', 'world-off');
  const results = await retainWorldOff(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldOff(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
