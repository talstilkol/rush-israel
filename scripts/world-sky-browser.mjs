...
/** Combined bakeEnv background is not 0x3a9ae0 vs 0x808080 attribution. */
export const SKY_FRAMES = REGION_FRAMES;
export const SKY_LAYERS = Object.freeze([
  'sky', 'hemi', 'disc', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['sky']);
export const DISC_IDS = Object.freeze(['disc']);
export const HEMI_IDS = Object.freeze(['hemi']);
export const SKY_L2_MIN = 8;
export const SKY_LUMA_MIN = 8;
export const SKY_BLUE_MIN = 8;
...
export function dominantSky(layers) {
  if (!Array.isArray(layers) || layers.length !== SKY_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function bakeIsNotSkyMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== SKY_LAYERS.length
    || SKY_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !SKY_LAYERS.includes(layer.id)
...
export function skyBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-sky-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { sky: 1, hemi: 1, disc: 1, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'sky' ? 50 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'sky' ? 90 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = SKY_FRAMES.find(row => row.id === id);
  ...
  const layers = SKY_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantSky: 'sky',
    contributingSky: ['sky', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: ['sky'],
    hemiAxis: [],
    discAxis: [],
...
export function worldSkyFixture(extra = {}) {
  return {
    ...
    skyCount: 1, hemiCount: 1, discCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, skyIsCombined: false, skyClearsGain: false, skyClearsIntensity: false,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-sky-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: SKY_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldSkyResults(r) {
  assert.ok(r && typeof r === 'object', 'world-sky evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-sky frames missing');
  assert.equal(bakeIsNotSkyMismatch(r), false, 'combined bakeEnv background is not 0x3a9ae0 vs 0x808080 mismatch');
  ...
  assert.equal(r.skyIsCombined, false, 'sky isolation still combined with hemi/disc/gain');
  assert.equal(r.skyClearsGain, false, 'sky isolation clears environmentIntensity');
  assert.equal(r.skyClearsIntensity, false, 'sky isolation clears intensity');
  ...
  assert.ok(r.skyCount >= 1, 'bakeEnv sky missing');
  ...
  assert.equal(skyBufferIsNotOriginalGolden(r), false, 'sky buffer is not original-golden');
  ...
  for (const spec of SKY_FRAMES) {
    ...
    if (!row.dominantSky || !row.dominant || row.layers.length !== SKY_LAYERS.length
      || SKY_LAYERS.some(id => !byId[id])
      ...
      || SKY_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined bakeEnv background is not 0x3a9ae0 vs 0x808080 attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present sky axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'bakeEnv 0x3a9ae0 vs 0x808080 and hemi/disc/intensity/fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'sky-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldSky(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldSkyResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldSky(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const skyIsCombined = skyRestorers.length !== 1;
        const skyClearsGain = engine.scene.environmentIntensity !== gainBefore;
        const skyClearsIntensity = sun.intensity !== intensityBefore;
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = SKY_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantSky: dominantSky(layers),
        contributingSky: layers.filter(layer => layer.l2Delta <= -SKY_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -SKY_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -SKY_BLUE_MIN).map(layer => layer.id),
        hemiAxis: layers.filter(layer => HEMI_IDS.includes(layer.id) && (layer.l2Delta <= -SKY_L2_MIN || layer.blueDelta <= -SKY_BLUE_MIN)).map(layer => layer.id),
        discAxis: layers.filter(layer => DISC_IDS.includes(layer.id) && (layer.l2Delta <= -SKY_L2_MIN || layer.blueDelta <= -SKY_BLUE_MIN)).map(layer => layer.id),
      };
    });
    return {
      ...
      skyIsCombined: report.skyIsCombined,
      skyClearsGain: report.skyClearsGain,
      skyClearsIntensity: report.skyClearsIntensity,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-sky-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldSky(browser, url) {
  const report = await measureWorldSky(browser, url);
  const out = process.env.WORLD_SKY_OUTPUT ?? fromRoot('artifacts', 'world-sky');
  const results = await retainWorldSky(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldSky(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
