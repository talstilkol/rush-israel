...
/** Combined scene.environment blue is not background vs sky vs material envMap attribution. */
export const IBL_FRAMES = REGION_FRAMES;
export const IBL_LAYERS = Object.freeze([
  'environment', 'background', 'sky', 'envMap', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['environment', 'background', 'sky', 'envMap']);
export const PROBE_IDS = Object.freeze(['environment', 'background', 'sky', 'envMap']);
export const IBL_L2_MIN = 8;
export const IBL_LUMA_MIN = 8;
export const IBL_BLUE_MIN = 8;
...
export function dominantIbl(layers) {
  if (!Array.isArray(layers) || layers.length !== IBL_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function rayIsNotIblMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== IBL_LAYERS.length
    || IBL_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !IBL_LAYERS.includes(layer.id)
...
export function iblBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-ibl-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { environment: 1, background: 1, sky: 1, envMap: 56, intensity: 1, fill: 1 }[id] ?? 1;
  const baselineL2 = 127.4;
  const baselineLuma = 98.2;
  const baselineBlue = 132;
  const l2 = id === 'environment' ? 50 : id === 'intensity' ? 80 : id === 'fill' ? 90 : 120;
  const lumaVal = LUMA_IDS.includes(id) ? (id === 'intensity' ? 30 : 80) : 90;
  const blueVal = id === 'environment' ? 90 : 132;
  return {
...
function frame(id, extra = {}) {
  const spec = IBL_FRAMES.find(row => row.id === id);
  ...
  const layers = IBL_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantIbl: 'environment',
    contributingIbl: ['environment', 'intensity'],
    lumaAxis: ['intensity'],
    iblAxis: ['environment'],
    probeAxis: ['environment'],
    ...extra,
  };
}

export function worldIblFixture(extra = {}) {
  return {
    ...
    environmentCount: 1, backgroundCount: 1, skyCount: 1, envMapCount: 56, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, iblIsCombined: false, envMapClearsEnvironment: false, backgroundClearsEnvironment: false,
    intensityClearsEnvironment: false, environmentClearsIntensity: false,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-ibl-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: IBL_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldIblResults(r) {
  assert.ok(r && typeof r === 'object', 'world-ibl evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-ibl frames missing');
  assert.equal(rayIsNotIblMismatch(r), false, 'combined scene.environment blue is not background vs sky vs material envMap mismatch');
  ...
  assert.equal(r.iblIsCombined, false, 'environment isolation still combined with background/sky/envMap');
  assert.equal(r.envMapClearsEnvironment, false, 'envMap isolation clears scene.environment');
  assert.equal(r.backgroundClearsEnvironment, false, 'background isolation clears scene.environment');
  assert.equal(r.intensityClearsEnvironment, false, 'intensity isolation clears scene.environment');
  assert.equal(r.environmentClearsIntensity, false, 'environment isolation clears intensity');
  ...
  assert.ok(r.backgroundCount >= 1, 'scene background missing');
  assert.ok(r.skyCount >= 1, 'sky missing');
  assert.ok(r.envMapCount >= 1, 'material envMap missing');
  ...
  assert.equal(iblBufferIsNotOriginalGolden(r), false, 'ibl buffer is not original-golden');
  ...
  let cameraFail = 0, mismatchFail = 0, iblFail = 0;
  for (const spec of IBL_FRAMES) {
    ...
    if (!row.dominantIbl || !row.dominant || row.layers.length !== IBL_LAYERS.length
      || IBL_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.iblAxis) || !Array.isArray(row.probeAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || IBL_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) iblFail += 1;
  }
  return [
    {
      case: 'combined scene.environment blue is not background vs sky vs material envMap attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present ibl axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'environment/background/sky/envMap and intensity/fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
      status: iblFail ? 'failed' : 'passed',
      probes: 4,
      failures: iblFail,
    },
    {
    ...
    {
      case: 'ibl-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldIbl(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldIblResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldIbl(browser, url) {
  ...
  try {
      ...
      try {
        ...
        const isolate = {
          ...
          background() {
            ...
            return [() => { engine.scene.background = background; }];
          },
          sky() {
            const sky = engine.world.sky;
            const visible = sky.visible;
            sky.visible = false;
            return [() => { sky.visible = visible; }];
          },
          envMap() {
            const restorers = [];
            const seen = new Set();
            engine.scene.traverse(object => {
              for (const mat of materialsOf(object)) {
                if (!mat || !('envMap' in mat) || seen.has(mat)) continue;
                seen.add(mat);
        ...
        const envRestorers = isolate.environment();
        const iblIsCombined = envRestorers.length !== 1;
        const environmentClearsIntensity = sun.intensity !== intensityBefore;
        envRestorers.forEach(restore => restore());
        const backgroundRestorers = isolate.background();
        const backgroundClearsEnvironment = engine.scene.environment !== environmentBefore;
        backgroundRestorers.forEach(restore => restore());
        const envMapRestorers = isolate.envMap();
        const envMapClearsEnvironment = engine.scene.environment !== environmentBefore;
        envMapRestorers.forEach(restore => restore());
        const intensityRestorers = isolate.intensity();
        const intensityClearsEnvironment = engine.scene.environment !== environmentBefore;
        ...
        return {
          ...
          iblIsCombined,
          envMapClearsEnvironment,
          backgroundClearsEnvironment,
          intensityClearsEnvironment,
          environmentClearsIntensity,
          ...counts,
          glError: gl.getError(),
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = IBL_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantIbl: dominantIbl(layers),
        contributingIbl: layers.filter(layer => layer.l2Delta <= -IBL_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -IBL_LUMA_MIN).map(layer => layer.id),
        iblAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -IBL_BLUE_MIN).map(layer => layer.id),
        probeAxis: layers.filter(layer => PROBE_IDS.includes(layer.id) && (layer.l2Delta <= -IBL_L2_MIN || layer.blueDelta <= -IBL_BLUE_MIN)).map(layer => layer.id),
      };
    });
    return {
      ...
      iblIsCombined: report.iblIsCombined,
      envMapClearsEnvironment: report.envMapClearsEnvironment,
      backgroundClearsEnvironment: report.backgroundClearsEnvironment,
      intensityClearsEnvironment: report.intensityClearsEnvironment,
      environmentClearsIntensity: report.environmentClearsIntensity,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-ibl-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldIbl(browser, url) {
  const report = await measureWorldIbl(browser, url);
  const out = process.env.WORLD_IBL_OUTPUT ?? fromRoot('artifacts', 'world-ibl');
  const results = await retainWorldIbl(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldIbl(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
