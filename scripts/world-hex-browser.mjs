...
/** Combined leftover-occupying hemi.color is not 0xa8c8e8 vs 0x808080 vs 0x000000 attribution. */
export const HEX_FRAMES = REGION_FRAMES;
export const HEX_LAYERS = Object.freeze([
  'hgray', 'hblack', 'hint', 'hue', 'intensity', 'fill',
]);
export const LUMA_IDS = Object.freeze(['intensity', 'fill']);
export const IBL_IDS = Object.freeze(['hgray']);
...
export const GRAY_BASELINE_IDS = Object.freeze(['hgray', 'hblack', 'hint']);
export const PRODUCT_HEMI_SKY_HEX = 0xa8c8e8;
export const PRODUCT_HEMI_NIGHT_HEX = 0x6a88b0;
export const HEX_L2_MIN = 8;
export const HEX_LUMA_MIN = 8;
export const HEX_BLUE_MIN = 8;
...
export function dominantHex(layers) {
  if (!Array.isArray(layers) || layers.length !== HEX_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function hemIsNotHexMismatch(report) {
  if (!report?.frames?.length) return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== HEX_LAYERS.length
    || HEX_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !HEX_LAYERS.includes(layer.id)
...
export function hexBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-hex-buffer-not-original-golden'
...
function layerRow(id, extra = {}) {
  const hidden = { hgray: 1, hblack: 1, hint: 1, hue: 1, intensity: 1, fill: 1 }[id] ?? 1;
...
function frame(id, extra = {}) {
  const spec = HEX_FRAMES.find(row => row.id === id);
  ...
  const layers = HEX_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantHex: 'hue',
    contributingHex: ['hint', 'hue', 'intensity'],
    lumaAxis: ['intensity'],
    hgrayAxis: [],
    hblackAxis: [],
    hintAxis: ['hint'],
...
export function worldHexFixture(extra = {}) {
  return {
    ...
    hgrayCount: 1, hblackCount: 1, hintCount: 1, hueCount: 1, intensityCount: 1, fillCount: 1,
    lumaTermsIncludeHemi: false, hgrayIsCombined: false, hgrayClearsOff: false,
    hgrayClearsHue: false, hblackClearsHgray: false, hueClearsOff: false, intensityClearsIbl: false,
    hexDeltasUseGrayBaseline: true,
    hintZerosIntensity: true, hblackSetsBlack: true, hgraySetsGray: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-hex-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: HEX_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldHexResults(r) {
  assert.ok(r && typeof r === 'object', 'world-hex evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-hex frames missing');
  assert.equal(hemIsNotHexMismatch(r), false, 'combined leftover-occupying hemi.color is not 0xa8c8e8 vs 0x808080 vs 0x000000 mismatch');
  ...
  assert.equal(r.hgrayIsCombined, false, 'hgray isolation still combined with hemi or disc');
  assert.equal(r.hgrayClearsOff, false, 'hgray isolation clears IBL-off');
  assert.equal(r.hgrayClearsHue, false, 'hgray isolation clears gray cubemap hue');
  assert.equal(r.hblackClearsHgray, false, 'hblack isolation uses HemisphereLight intensity');
  ...
  assert.equal(r.hexDeltasUseGrayBaseline, true, 'hgray vs hblack vs hint still uses product present as baseline');
  ...
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product during hex probe');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during hex probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during hex probe');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during hex probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during hex probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during hex probe');
  ...
  assert.equal(hexBufferIsNotOriginalGolden(r), false, 'lod buffer is not original-golden');
  ...
  for (const spec of HEX_FRAMES) {
    ...
    if (!row.dominantHex || !row.dominant || row.layers.length !== HEX_LAYERS.length
      || HEX_LAYERS.some(id => !byId[id])
      ...
      || !Array.isArray(row.lumaAxis) || !Array.isArray(row.hgrayAxis) || !Array.isArray(row.hblackAxis)
      || !Array.isArray(row.hintAxis) || !Array.isArray(row.hueAxis)
      || row.bands.length !== REGION_BANDS.length
      || !Number.isFinite(row.bandL2)
      || HEX_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'combined leftover-occupying hemi.color is not 0xa8c8e8 vs 0x808080 vs 0x000000 attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present hex axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'hemi.color 0xa8c8e8 vs 0x808080 vs 0x000000 independently of hemi.intensity vs groundColor vs AmbientLight vs RectAreaLight vs 0x3a9ae0 hue vs intensity vs fill of the dominant 200px band ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'hex-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldHex(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldHexResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldHex(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const hgrayTex = engine.scene.environment;
        const hgrayIsCombined = hgrayRestorers.length !== 1;
        const hgrayClearsOff = hgrayTex == null;
        const hgrayClearsHue = hgrayTex === hueProbe.texture;
        hgrayRestorers.forEach(restore => restore());
        const hblackRestorers = isolate.hblack();
        const hblackClearsHgray = engine.scene.environment == null
          || engine.scene.environment === hgrayTex;
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = HEX_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantHex: dominantHex(layers),
        contributingHex: layers.filter(layer => layer.l2Delta <= -HEX_L2_MIN).map(layer => layer.id),
        lumaAxis: layers.filter(layer => LUMA_IDS.includes(layer.id) && layer.lumaDelta <= -HEX_LUMA_MIN).map(layer => layer.id),
        hgrayAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && layer.blueDelta <= -HEX_BLUE_MIN).map(layer => layer.id),
        hblackAxis: layers.filter(layer => HBLACK_IDS.includes(layer.id) && (layer.l2Delta <= -HEX_L2_MIN || layer.blueDelta <= -HEX_BLUE_MIN)).map(layer => layer.id),
        hintAxis: layers.filter(layer => HINT_IDS.includes(layer.id) && layer.blueDelta <= -HEX_BLUE_MIN).map(layer => layer.id),
        hueAxis: layers.filter(layer => HUE_IDS.includes(layer.id) && layer.blueDelta <= -HEX_BLUE_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      hgrayIsCombined: report.hgrayIsCombined,
      hgrayClearsOff: report.hgrayClearsOff,
      hgrayClearsHue: report.hgrayClearsHue,
      hblackClearsHgray: report.hblackClearsHgray,
      hueClearsOff: report.hueClearsOff,
      intensityClearsIbl: report.intensityClearsIbl,
      hexDeltasUseGrayBaseline: report.hexDeltasUseGrayBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-hex-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldHex(browser, url) {
  const report = await measureWorldHex(browser, url);
  const out = process.env.WORLD_HEX_OUTPUT ?? fromRoot('artifacts', 'world-hex');
  const results = await retainWorldHex(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldHex(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
