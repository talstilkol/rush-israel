...
/** Leftover g07 after world.group+outside is remaining empty-scene vs golden independently of xr.framebufferScaleFactor vs transmissionResolutionScale vs VSMShadowMap. */
export const FTV_FRAMES = REGION_FRAMES;
export const FTV_LAYERS = Object.freeze([
  'lights', 'debug', 'xr', 'env', 'both', 'pack',
]);
export const CAPTURE_LAYERS = FTV_LAYERS;
...
export const FTV_L2_MIN = 8;
export const FTV_LUMA_MIN = 8;
export const FTV_BLUE_MIN = 8;
export const FTV_BAND_MIN = 0.02;
...
export function dominantFtv(layers) {
  if (!Array.isArray(layers) || layers.length !== FTV_LAYERS.length) return null;
  return [...layers].sort((a, b) => a.bandDelta - b.bandDelta || a.l2Delta - b.l2Delta || a.id.localeCompare(b.id))[0]?.id ?? null;
}

export function leftoverIsNotFtvMismatch(report) {
  if (!report?.frames?.length) return true;
  if (report.protocol !== 'world-ftv-buffer-not-original-golden') return true;
  return report.frames.some(frame => !Array.isArray(frame.layers) || frame.layers.length !== FTV_LAYERS.length
    || FTV_LAYERS.some(id => !frame.layers.some(layer => layer.id === id))
    || frame.layers.some(layer => !FTV_LAYERS.includes(layer.id)
...
export function ftvBufferIsNotOriginalGolden(report) {
  return !report
    || report.protocol !== 'world-ftv-buffer-not-original-golden'
...
function frame(id, extra = {}) {
  const spec = FTV_FRAMES.find(row => row.id === id);
  ...
  const layers = FTV_LAYERS.map(layerId => layerRow(layerId));
  return {
    ...
    layers, dominantFtv: 'lights',
    contributingFtv: ['lights', 'pack'],
    lightsAxis: ['lights'],
    envAxis: [],
    bothAxis: ['both'],
    debugAxis: [],
...
export function worldFtvFixture(extra = {}) {
  return {
    ...
    ftvDeltasUseEmptyBaseline: true,
    ...
    viewport: { ...VIEWPORT },
    pageErrors: [], glError: 0, protocol: 'world-ftv-buffer-not-original-golden',
    baselineHashes: { ...RSH035_BASELINE_SHA256 },
    frames: FTV_FRAMES.map(spec => frame(spec.id)),
    ...extra,
  };
}

export function worldFtvResults(r) {
  assert.ok(r && typeof r === 'object', 'world-ftv evidence missing');
  ...
  assert.ok(Array.isArray(r.frames) && r.frames.length === 4, 'world-ftv frames missing');
  assert.equal(leftoverIsNotFtvMismatch(r), false, 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of xr.framebufferScaleFactor vs transmissionResolutionScale vs VSMShadowMap mismatch');
  ...
  assert.equal(r.bothIsCombined, false, 'ftv isolation still combined with hemi');
  assert.equal(r.lightsClears, true, 'type isolation did not force framebufferScaleFactor 0.5');
  assert.equal(r.lightsHidesLeftover, true, 'type isolation did not hide leftover empty-scene meshes');
  assert.equal(r.lightsKeepsEnv, true, 'type isolation clears scene.environment');
  assert.equal(r.lightsKeepsHemi, true, 'hemi.color left product during type isolation');
  assert.equal(r.lightsKeepsDebug, true, 'transmissionResolutionScale left product during type isolation');
  assert.equal(r.lightsKeepsXr, true, 'shadowMap.type left product during type isolation');
  assert.equal(r.debugClears, true, 'needs isolation did not force transmissionResolutionScale 0.5');
  assert.equal(r.debugHidesLeftover, true, 'needs isolation did not hide leftover empty-scene meshes');
  assert.equal(r.debugKeepsEnv, true, 'needs isolation clears scene.environment');
  assert.equal(r.debugKeepsHemi, true, 'hemi.color left product during needs isolation');
  assert.equal(r.debugKeepsLights, true, 'framebufferScaleFactor left product during needs isolation');
  assert.equal(r.debugKeepsXr, true, 'shadowMap.type left product during needs isolation');
  assert.equal(r.xrClears, true, 'clip isolation did not force VSMShadowMap');
  assert.equal(r.xrHidesLeftover, true, 'clip isolation did not hide leftover empty-scene meshes');
  assert.equal(r.xrKeepsEnv, true, 'clip isolation clears scene.environment');
  assert.equal(r.xrKeepsHemi, true, 'hemi.color left product during clip isolation');
  assert.equal(r.xrKeepsLights, true, 'framebufferScaleFactor left product during clip isolation');
  assert.equal(r.xrKeepsDebug, true, 'transmissionResolutionScale left product during clip isolation');
  ...
  assert.equal(r.envKeepsLights, true, 'framebufferScaleFactor left product during env isolation');
  assert.equal(r.envKeepsDebug, true, 'transmissionResolutionScale left product during env isolation');
  assert.equal(r.envKeepsXr, true, 'shadowMap.type left product during env isolation');
  assert.equal(r.envClearsHue, false, 'scene.environment isolation uses product sky hue bake');
  assert.equal(r.packClearsLights, true, 'pack isolation did not force framebufferScaleFactor 0.5');
  assert.equal(r.packClearsDebug, true, 'pack isolation did not force transmissionResolutionScale 0.5');
  assert.equal(r.packClearsXr, true, 'pack isolation did not force VSMShadowMap');
  ...
  assert.equal(r.bothClearsLights, true, 'both isolation did not force framebufferScaleFactor 0.5');
  assert.equal(r.bothClearsDebug, true, 'both isolation did not force transmissionResolutionScale 0.5');
  assert.equal(r.bothClearsXr, true, 'both isolation did not force VSMShadowMap');
  ...
  assert.equal(r.intensityClearsIbl, false, 'ftv isolation clears scene.environment via framebufferScaleFactor');
  assert.equal(r.ftvDeltasUseEmptyBaseline, true, 'leftover g07 still uses world.group leftover as baseline');
  assert.equal(r.hgndKeepsGround, true, 'HemisphereLight groundColor left product after tnc restorers');
  assert.equal(r.ambKeepsAmbient, true, 'AmbientLight intensity left product during ftv probe');
  assert.equal(r.rectKeepsRectArea, true, 'RectAreaLight intensity left product during ftv probe');
  ...
  assert.ok(r.debugCount >= 151, 'needs isolation missing');
  assert.ok(r.xrCount >= 151, 'clip isolation missing');
  assert.ok(r.packCount >= 151, 'combined framebufferScaleFactor-transmissionResolutionScale-VSMShadowMap isolation missing');
  assert.ok(r.envCount >= 151, 'scene.environment isolation missing');
  assert.ok(r.bothCount >= 151, 'both framebufferScaleFactor-transmissionResolutionScale-VSMShadowMap+env isolation missing');
  ...
  assert.equal(r.colorManagementStaysEnabled, true, 'ColorManagement disabled during ftv probe');
  assert.equal(r.outputColorSpaceStaysSRGB, true, 'outputColorSpace left product sRGB during ftv probe');
  assert.equal(r.toneMappingStaysACES, true, 'toneMapping left product ACES during ftv probe');
  assert.equal(r.fovStays58, true, 'product fov retuned during ftv probe');
  assert.equal(r.nearStaysProduct, true, 'product near retuned during ftv probe');
  assert.equal(r.farStaysProduct, true, 'product far retuned during ftv probe');
  ...
  assert.equal(ftvBufferIsNotOriginalGolden(r), false, 'ldx buffer is not original-golden');
  ...
  for (const spec of FTV_FRAMES) {
    ...
    if (!row.dominantFtv || !row.dominant || row.layers.length !== FTV_LAYERS.length
      || FTV_LAYERS.some(id => !byId[id])
      ...
      || FTV_LAYERS.some(id => (byId[id]?.hidden ?? 0) < 1)) bakeFail += 1;
  }
  return [
    {
      case: 'leftover g07 after world.group+outside is not remaining empty-scene vs golden independently of xr.framebufferScaleFactor vs transmissionResolutionScale vs VSMShadowMap attribution',
      status: 'passed',
      probes: 4,
      failures: 0,
    },
    {
      case: 'live rest chase stays 7.4/1.92 while present ftv axes are sampled',
      status: cameraFail ? 'failed' : 'passed',
      probes: 4,
      failures: cameraFail,
    },
    {
      case: 'xr.framebufferScaleFactor vs transmissionResolutionScale vs VSMShadowMap independently of env of leftover g07 after world.group+outside ran at 1280x800 threshold 0.12 without retuning 0xd0d4d8 or 0.56 exposure',
    ...
    {
      case: 'ftv-buffer pixelmatch is not original-golden page capture',
      status: 'passed',
      probes: 1,
      failures: 0,
    },
...
export async function retainWorldFtv(report, out) {
  await mkdir(out, { recursive: true });
  await writeFile(`${out}/results.json`, JSON.stringify(report, null, 2) + '\n');
  const results = worldFtvResults(report);
  await writeFile(`${out}/results.json`, JSON.stringify({ ...report, results }, null, 2) + '\n');
  return results;
}

export async function measureWorldFtv(browser, url) {
  ...
  try {
    ...
    const report = await page.evaluate(async ({ frames, layers, width, height, groundMin, albedoHex, threePath }) => {
      ...
      try {
        ...
        const setLights = () => {
          engine.renderer.xr.setFramebufferScaleFactor(0.5);
          return [() => { engine.renderer.xr.setFramebufferScaleFactor(1); }];
        };
        const setDebug = () => {
          const prev = engine.renderer.transmissionResolutionScale;
          engine.renderer.transmissionResolutionScale = 0.5;
          return [() => { engine.renderer.transmissionResolutionScale = prev; }];
        };
        const setXr = () => {
          const prev = engine.renderer.shadowMap.type;
          engine.renderer.shadowMap.type = THREE.VSMShadowMap;
        ...
        const isolate = {
          lights() {
            ...
            const prevNeeds = engine.renderer.transmissionResolutionScale;
            const prevClip = engine.renderer.shadowMap.type;
            ...
            if (engine.renderer.transmissionResolutionScale !== prevNeeds) lightsKeepsDebug = false;
            if (engine.renderer.shadowMap.type !== prevClip) lightsKeepsXr = false;
            return restorers;
          },
          debug() {
            const prevHemi = hemiRgb();
            const prevEnv = engine.scene.environment;
            const prevType = engine.renderer.shadowMap.enabled;
            const prevClip = engine.renderer.shadowMap.type;
            ...
            if (engine.renderer.shadowMap.enabled !== prevType) debugKeepsLights = false;
            if (engine.renderer.shadowMap.type !== prevClip) debugKeepsXr = false;
            return restorers;
          },
          xr() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.shadowMap.enabled !== prevType) xrKeepsLights = false;
            if (engine.renderer.transmissionResolutionScale !== prevNeeds) xrKeepsDebug = false;
            return restorers;
          },
          env() {
            const prevHemi = hemiRgb();
            ...
            if (engine.renderer.shadowMap.enabled !== prevType) envKeepsLights = false;
            if (engine.renderer.transmissionResolutionScale !== prevNeeds) envKeepsDebug = false;
            if (engine.renderer.shadowMap.type !== prevClip) envKeepsXr = false;
            return restorers;
          },
          pack() {
            const prevHemi = hemiRgb();
            ...
            packClearsLights = true;
            packClearsDebug = true;
            packClearsXr = true;
          ...
          both() {
            ...
            bothClearsLights = true;
            bothClearsDebug = true;
            bothClearsXr = true;
    ...
    const frames = report.frames.map(row => {
      ...
      const layers = FTV_LAYERS.map(id => {
      ...
      return {
        ...
        layers,
        dominantFtv: dominantFtv(layers),
        contributingFtv: layers.filter(layer => layer.bandDelta <= -FTV_BAND_MIN).map(layer => layer.id),
        lightsAxis: layers.filter(layer => LIGHTS_IDS.includes(layer.id) && layer.bandDelta <= -FTV_BAND_MIN).map(layer => layer.id),
        envAxis: layers.filter(layer => IBL_IDS.includes(layer.id) && (layer.bandDelta <= -FTV_BAND_MIN || layer.blueDelta <= -FTV_BLUE_MIN)).map(layer => layer.id),
        bothAxis: layers.filter(layer => BOTH_IDS.includes(layer.id) && layer.bandDelta <= -FTV_BAND_MIN).map(layer => layer.id),
        debugAxis: layers.filter(layer => DEBUG_IDS.includes(layer.id) && layer.bandDelta <= -FTV_BAND_MIN).map(layer => layer.id),
        xrAxis: layers.filter(layer => XR_IDS.includes(layer.id) && layer.bandDelta <= -FTV_BAND_MIN).map(layer => layer.id),
        packAxis: layers.filter(layer => PACK_IDS.includes(layer.id) && layer.bandDelta <= -FTV_BAND_MIN).map(layer => layer.id),
      };
    });
    return {
      ...
      ftvDeltasUseEmptyBaseline: report.ftvDeltasUseEmptyBaseline,
      ...
      viewport: { ...VIEWPORT },
      protocol: 'world-ftv-buffer-not-original-golden',
      baselineHashes,
      pageErrors,
    };
  } finally {
...
export async function verifyWorldFtv(browser, url) {
  const report = await measureWorldFtv(browser, url);
  const out = process.env.WORLD_FTV_OUTPUT ?? fromRoot('artifacts', 'world-ftv');
  const results = await retainWorldFtv(report, out);
...
if (process.argv[1] && await realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    console.log(JSON.stringify(await verifyWorldFtv(browser, process.env.SMOKE_URL ?? 'http://127.0.0.1:8080/?qa=1'), null, 2));
  } finally {
    await browser.close();
  }
}
