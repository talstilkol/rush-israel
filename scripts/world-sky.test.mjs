...
import {
  BAND_PIXELS, DISC_IDS, SKY_BLUE_MIN, SKY_FRAMES, SKY_L2_MIN, SKY_LAYERS, SKY_LUMA_MIN,
  FAILURE_LIMIT, HEMI_IDS, IBL_IDS, LUMA_IDS, NEUTRAL_SKY_HEX, PIXEL_THRESHOLD, PRODUCT_EXPOSURE,
  PRODUCT_SKY_HEX, skyBufferIsNotOriginalGolden, dominantSky, luma, retainWorldSky,
  bakeIsNotSkyMismatch, worldSkyFixture, worldSkyResults,
} from './world-sky-browser.mjs';
import { worldBakeFixture } from './world-bake-browser.mjs';
...
test('dominant 200px bake report without 0x3a9ae0 vs 0x808080 is not sky mismatch', () => {
  assert.equal(bakeIsNotSkyMismatch({ frames: [] }), true);
  assert.equal(bakeIsNotSkyMismatch(worldBakeFixture()), true);
  assert.equal(bakeIsNotSkyMismatch(worldSkyFixture()), false);
  assert.throws(() => worldSkyResults(worldBakeFixture()), /combined bakeEnv background is not 0x3a9ae0 vs 0x808080 mismatch/);
});

test('sky buffer without original-golden protocol identity is not original-golden', () => {
  assert.equal(skyBufferIsNotOriginalGolden(worldSkyFixture()), false);
  assert.equal(skyBufferIsNotOriginalGolden({
    ...worldSkyFixture(), originalGoldenComparisons: 4, protocol: 'original-golden',
  }), true);
  assert.equal(PIXEL_THRESHOLD, 0.12);
  assert.equal(FAILURE_LIMIT, 0.08);
  assert.equal(SKY_L2_MIN, 8);
  assert.equal(SKY_LUMA_MIN, 8);
  assert.equal(SKY_BLUE_MIN, 8);
  ...
  assert.deepEqual(SKY_LAYERS, ['sky', 'hemi', 'disc', 'intensity', 'fill']);
  assert.deepEqual([...LUMA_IDS], ['intensity', 'fill']);
  assert.deepEqual([...IBL_IDS], ['sky']);
  ...
  assert.throws(() => worldSkyResults({ ...worldSkyFixture(), productGroundHex: 0 }), /product ground color retuned/);
  assert.throws(() => worldSkyResults({ ...worldSkyFixture(), productExposure: 1 }), /product exposure retuned/);
  assert.throws(() => worldSkyResults({ ...worldSkyFixture(), lumaTermsIncludeHemi: true }), /luma terms include hemi/);
  assert.throws(() => worldSkyResults({ ...worldSkyFixture(), skyIsCombined: true }), /sky isolation still combined/);
  assert.throws(() => worldSkyResults({ ...worldSkyFixture(), skyClearsGain: true }), /sky isolation clears environmentIntensity/);
  assert.throws(() => worldSkyResults({ ...worldSkyFixture(), skyClearsIntensity: true }), /sky isolation clears intensity/);
  assert.throws(() => worldSkyResults({ ...worldSkyFixture(), hemiClearsIntensity: true }), /hemi isolation clears intensity/);
  assert.throws(() => worldSkyResults({ ...worldSkyFixture(), intensityClearsIbl: true }), /intensity isolation clears scene.environment/);
});
...
test('locked sky poses match original-golden cameras and PNG hashes', () => {
  assert.deepEqual(SKY_FRAMES.map(row => row.id), ['g01', 'g05', 'g07', 'g08']);
  assert.deepEqual(SKY_FRAMES.map(row => row.file), ORIGINAL_GOLDEN_FILES);
  assert.equal(SKY_FRAMES[3].night, true);
  assert.equal(dominantSky(worldSkyFixture().frames[0].layers), 'sky');
});

test('sky-attribution evidence yields five protocol passes', () => {
  assert.deepEqual(worldSkyResults(worldSkyFixture()).map(row => row.status), Array(5).fill('passed'));
});
...
]) test(`world-sky evidence fails closed: ${name}`, () => {
  const r = worldSkyFixture();
  mutate(r);
  assert.throws(() => worldSkyResults(r));
});

test('baseline PNG hash drift fails closed', () => {
  const r = worldSkyFixture();
  r.baselineHashes['ayalon-day-g01.png'] = '0'.repeat(64);
  assert.throws(() => worldSkyResults(r), /baseline hash drift/);
});

test('zero full-frame present mismatch remains failed', () => {
  const r = worldSkyFixture();
  r.frames[0] = { ...r.frames[0], presentPct: 0, presentMismatched: 0 };
  const row = worldSkyResults(r).find(item => item.case.includes('still mismatches'));
  assert.equal(row.status, 'failed');
  assert.equal(row.failures, 1);
});

test('a rest-camera miss remains failed during sky sampling', () => {
  const r = worldSkyFixture();
  r.frames[1] = { ...r.frames[1], follow: 9.2, height: 2.28 };
  assert.throws(() => worldSkyResults(r));
});

test('invalid world-sky report is retained before validation throws', async () => {
  const out = await mkdtemp(join(tmpdir(), 'world-sky-invalid-'));
  try {
    const r = worldSkyFixture({ originalGoldenComparisons: 4 });
    await assert.rejects(retainWorldSky(r, out));
...
test('smoke keeps world-layer residual mismatch bias scene region column slice extra material factor rgb shade tone term beam ray ibl gain cube probe bake and sky probes', () => {
